...
interface EpsilonComputerProps {
...
export function EpsilonComputer({
  toolMessages,
  currentIndex,
  onNavigate,
  messages,
  ...
  streamingText,
  sandboxId,
}: EpsilonComputerProps) {
  log.log('[EpsilonComputer] Render - toolMessages:', toolMessages.length, 'currentIndex:', currentIndex);
  log.log('[EpsilonComputer] project:', project ? { id: project.id, name: project.name, hasSandbox: !!project.sandbox } : 'undefined');
  log.log('[EpsilonComputer] sandboxId prop:', sandboxId);
  ...
  log.log('[EpsilonComputer] currentPair:', currentPair ? 'has pair' : 'undefined');
  log.log('[EpsilonComputer] currentPair.toolMessage:', currentPair?.toolMessage?.message_id || 'null');
  log.log('[EpsilonComputer] currentPair.assistantMessage:', currentPair?.assistantMessage?.message_id || 'null');
  
  const { toolCall, toolResult, isSuccess, assistantTimestamp, toolTimestamp } = useMemo(() => {
    if (!currentPair?.toolMessage) {
      log.log('[EpsilonComputer] No toolMessage in currentPair, returning null');
      return { toolCall: null, toolResult: null, isSuccess: false, assistantTimestamp: undefined, toolTimestamp: undefined };
    }
    log.log('[EpsilonComputer] Calling extractToolCallAndResult');
