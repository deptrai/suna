...
import EpsilonSymbolIcon from '@/assets/brand/epsilon-symbol.svg';
...
/**
 * Check if a model ID corresponds to a Epsilon mode (Basic or Advanced)
 */
export function isEpsilonMode(modelId: string): boolean {
  // New Epsilon registry IDs
  if (modelId === 'epsilon/basic' || modelId === 'epsilon/power' ||
      modelId === 'epsilon-basic' || modelId === 'epsilon-power') {
    return true;
  }
  // Legacy: Epsilon Basic (Haiku 4.5)
  if (modelId.includes('claude-haiku-4-5') || modelId.includes('heol2zyy5v48')) {
    return true;
  }
  // Legacy: Epsilon Advanced Mode (Sonnet 4.5)
...
export function getModelProvider(modelId: string): ModelProvider {
  // Check for Epsilon modes first
  if (isEpsilonMode(modelId)) {
    return 'epsilon';
  }
  ...
  if (parts.length > 1) {
    ...
    if (['openai', 'anthropic', 'google', 'xai', 'moonshotai', 'bedrock', 'openrouter', 'epsilon'].includes(provider)) {
...
export function getModelProviderIcon(modelId: string): React.FC<SvgProps> {
  ...
  const iconMap: Record<ModelProvider, React.FC<SvgProps>> = {
    epsilon: EpsilonSymbolIcon, // Epsilon modes use the Epsilon symbol
