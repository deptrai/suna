...
// The opencode pack has no files yet, so it falls back to epsilon.
// ---------------------------------------------------------------------------

const EPSILON_ASSETS: Partial<Record<SoundEvent, AVPlaybackSource>> = {
  completion: require('@/assets/sounds/epsilon/completion.mp3'),
  send: require('@/assets/sounds/epsilon/send.mp3'),
};

function resolveAsset(pack: string, event: SoundEvent): AVPlaybackSource | null {
  if (pack === 'epsilon') {
    return EPSILON_ASSETS[event] ?? EPSILON_ASSETS.completion ?? null;
  }
...
export async function previewSound(event: SoundEvent) {
  const { preferences } = useSoundStore.getState();
  const pack = preferences.pack === 'off' ? 'epsilon' : preferences.pack;
