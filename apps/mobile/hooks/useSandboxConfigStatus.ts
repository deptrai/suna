 ...
 * exposes a mutation that creates + starts a Epsilon task to repair the
...
export function useSandboxConfigStatus() {
  ...
  const projectsQuery = useQuery<SandboxProjectSummary[]>({
    ...
    queryFn: async () => {
      const data = await opencodeFetch<unknown>(sandboxUrl!, '/epsilon/projects');
  ...
  const startFixTaskMutation = useMutation({
    mutationFn: async () => {
      ...
      const targetProject = configFixProject ?? await opencodeFetch<SandboxProjectSummary>(sandboxUrl, '/epsilon/projects', {
        method: 'POST',
        body: JSON.stringify({
          name: 'Workspace',
          path: '/workspace',
      ...
      const task = await opencodeFetch<{ id: string }>(sandboxUrl, '/epsilon/tasks', {
        method: 'POST',
        body: JSON.stringify({
      ...
      await opencodeFetch(sandboxUrl, `/epsilon/tasks/${encodeURIComponent(task.id)}/start`, {
        method: 'POST',
      });
