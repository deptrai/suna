#!/bin/sh
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Epsilon Frontend — Docker Entrypoint                                       ║
# ║                                                                            ║
...
# Support both NEXT_PUBLIC_* (legacy) and EPSILON_PUBLIC_* (current) naming.
RUNTIME_SUPABASE_URL="${NEXT_PUBLIC_SUPABASE_URL:-${EPSILON_PUBLIC_SUPABASE_URL:-}}"
RUNTIME_ANON_KEY="${NEXT_PUBLIC_SUPABASE_ANON_KEY:-${EPSILON_PUBLIC_SUPABASE_ANON_KEY:-}}"
RUNTIME_BACKEND_URL="${NEXT_PUBLIC_BACKEND_URL:-${EPSILON_PUBLIC_BACKEND_URL:-}}"
RUNTIME_BILLING="${NEXT_PUBLIC_BILLING_ENABLED:-${EPSILON_PUBLIC_BILLING_ENABLED:-false}}"
...
# e.g. http://127.0.0.1:54321 -> https://e2e-test.epsilon.cloud (not http://https://...)
