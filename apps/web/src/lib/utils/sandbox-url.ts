...
export interface SubdomainUrlOptions {
  /** Sandbox ID (e.g. 'epsilon-sandbox' for local, Daytona ID for cloud) */
  sandboxId: string;
  /** Backend port (e.g. 8008) — the port epsilon-api listens on */
  backendPort: number;
  /**
   * The public-facing API base URL (e.g. 'https://e2e-test.epsilon.cloud/v1').
   * When the user's browser is NOT on localhost, path-based proxy URLs are
   * generated from this:
   *   https://e2e-test.epsilon.cloud/v1/p/{sandboxId}/{port}/{path}
   *
...
const EXCLUDED_PORTS = new Set([
  ...
  4096, // OpenCode API (proxied by Epsilon Master)
  parseInt(SANDBOX_PORTS.EPSILON_MASTER, 10), // Epsilon Master itself
]);
 ...
 * from inside the sandbox (Epsilon Master). These are rewritten to appear as
...
export function parseLocalhostUrl(
  ...
  try {
    ...
    // Special case: localhost:8000/proxy/{port}/... (Epsilon Master proxy URL).
    // Extract the real port and remaining path so detection/rewriting works.
    const epsilonMasterPort = parseInt(SANDBOX_PORTS.EPSILON_MASTER, 10);
    if (port === epsilonMasterPort) {
      const proxyMatch = parsed.pathname.match(EPSILON_MASTER_PROXY_REGEX);
      if (proxyMatch) {
...
/**
 * Detect whether the epsilon-api backend is on the user's own machine
 ...
 * only reaches epsilon-api if epsilon-api is ALSO running on the user's machine.
 ...
 * that matters is whether epsilon-api is running on the user's local machine,
 ...
 *     when epsilon-api is on 127.0.0.1.
 *
 *   - **Path-based** (backend remote): `{apiBaseUrl}/p/{sandboxId}/{port}/{path}`
 *     Goes through Caddy → API → sandbox. Used for all deployed setups.
 *
 * @example
 *   // Local dev:      → 'http://p3210-epsilon-sandbox.localhost:8008/viewer.html'
 *   // Deployed back:  → 'https://e2e-test.epsilon.cloud/v1/p/epsilon-sandbox/3210/viewer.html'
 */
export function rewriteLocalhostUrl(
  ...
  if (!isBackendOnLocalhost(subdomainOpts.apiBaseUrl)) {
    // apiBaseUrl is like "https://e2e-test.epsilon.cloud/v1" — strip trailing slash.
    const base = subdomainOpts.apiBaseUrl.replace(/\/+$/, '');
    return `${base}/p/${subdomainOpts.sandboxId}/${port}${safePath}`;
  }

  // Subdomain proxy — only used in true local-dev where epsilon-api is on
 ...
 *
 * Subdomain: http://p3210-epsilon-sandbox.localhost:8008/viewer.html
 * Path:      https://e2e-test.epsilon.cloud/v1/p/epsilon-sandbox/3210/viewer.html
 */
...
/**
 * Build a web proxy URL that routes through the Epsilon Master (port 8000)
 * which hosts the /web-proxy/ forward proxy.
 *
 * The web proxy lives on Epsilon Master, NOT the OpenCode server, so we
...
export function buildWebProxyUrl(
  ...
  try {
    ...
    const kmPort = parseInt(SANDBOX_PORTS.EPSILON_MASTER, 10); // 8000
