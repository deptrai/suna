/**
 * LSS (Local Semantic Search) API — semantic search via the Epsilon Master endpoint.
 *
 * Calls the `/lss/search` route on Epsilon Master, which spawns the `lss` CLI
...
export async function searchLss(
  ...
  // Build search URL — the server URL points to Epsilon Master (port 8000),
