...
const REPO_URL = 'https://github.com/epsilon-ai/chainlens';
const REPO_SCRIPT_URL = `${REPO_URL}/blob/main/scripts/get-epsilon.sh`;
const RAW_SCRIPT_URL = 'https://raw.githubusercontent.com/epsilon-ai/chainlens/main/scripts/get-epsilon.sh';
const LOCAL_SCRIPT_CANDIDATES = [
  path.join(process.cwd(), '../../scripts/get-epsilon.sh'),
  path.join(process.cwd(), '../scripts/get-epsilon.sh'),
  path.join(process.cwd(), 'scripts/get-epsilon.sh'),
];
...
function scriptHeaders(upstreamHeaders: Headers): Headers {
  ...
  headers.set('X-Epsilon-Install-Source', RAW_SCRIPT_URL);
  return headers;
}
...
export async function GET(request: NextRequest) {
  ...
  const upstream = await fetch(RAW_SCRIPT_URL, {
    headers: {
      'User-Agent': 'epsilon-install-route',
    },
    next: {
      revalidate: 300,
    },
