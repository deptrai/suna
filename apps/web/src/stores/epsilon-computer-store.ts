...
export const useEpsilonComputerStore = create<EpsilonComputerState>()(
  devtools(
    (set, get) => ({
      ...initialState,
      ...
      setSandboxContext: (sandboxId: string | null) => {
        ...
        if (currentSandboxId !== sandboxId) {
          console.log('[EpsilonComputerStore] Sandbox context changed:', currentSandboxId, '->', sandboxId);
      ...
      setIsSidePanelOpen: (open: boolean) => {
        ...
        const update: Partial<EpsilonComputerState> = { isSidePanelOpen: open };
        if (sessionId) {
      ...
      openSidePanel: () => {
        const sessionId = get()._activeSessionId;
        const update: Partial<EpsilonComputerState> = { isSidePanelOpen: true };
        if (sessionId) {
      ...
      closeSidePanel: () => {
        const sessionId = get()._activeSessionId;
        const update: Partial<EpsilonComputerState> = { isSidePanelOpen: false, isExpanded: false };
        if (sessionId) {
      ...
      reset: () => {
        console.log('[EpsilonComputerStore] Full reset');
        useFilesStore.getState().reset();
        set(initialState);
      },
    }),
    {
      name: 'epsilon-computer-store',
    }
  )
);
...
// Sandbox context
export const useEpsilonComputerSandboxId = () =>
  useEpsilonComputerStore((state) => state.currentSandboxId);

export const useSetSandboxContext = () =>
  useEpsilonComputerStore((state) => state.setSandboxContext);
...
export const useIsSidePanelOpen = () =>
  useEpsilonComputerStore((state) => state.isSidePanelOpen);

export const useSetIsSidePanelOpen = () =>
  useEpsilonComputerStore((state) => state.setIsSidePanelOpen);

export const useIsExpanded = () =>
  useEpsilonComputerStore((state) => state.isExpanded);

export const useToggleExpanded = () =>
  useEpsilonComputerStore((state) => state.toggleExpanded);
