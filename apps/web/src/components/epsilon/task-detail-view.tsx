...
import {
  useEpsilonTask,
  useEpsilonTaskEvents,
  useEpsilonTaskStatus,
  useUpdateEpsilonTask,
  useStartEpsilonTask,
  useApproveEpsilonTask,
  useDeleteEpsilonTask,
  type EpsilonTaskEvent,
} from '@/hooks/epsilon/use-epsilon-tasks';
import { useEpsilonProject } from '@/hooks/epsilon/use-epsilon-projects';
import { openTabAndNavigate } from '@/stores/tab-store';
import { StatusPill } from '@/components/epsilon/task-pills';
import {
  shortTaskId,
  relativeTime,
  fullDate,
} from '@/lib/epsilon/task-meta';
import type { EpsilonTaskStatus } from '@/hooks/epsilon/use-epsilon-tasks';
...
export function TaskDetailView({
  ...
  const { data: task, isLoading } = useEpsilonTask(taskId || '', {
    enabled: !!taskId && pollingEnabled,
    pollingEnabled: !!taskId && pollingEnabled,
  });
  const { data: events } = useEpsilonTaskEvents(taskId || '', {
    enabled: !!taskId && pollingEnabled,
    pollingEnabled: !!taskId && pollingEnabled,
  });
  const { data: liveStatus } = useEpsilonTaskStatus(taskId || '', {
    enabled: !!taskId && pollingEnabled,
    pollingEnabled: !!taskId && pollingEnabled,
  });
  const { data: project } = useEpsilonProject(task?.project_id || '');
  const updateTask = useUpdateEpsilonTask();
  const startTask = useStartEpsilonTask();
  const approveTask = useApproveEpsilonTask();
  const deleteTaskMutation = useDeleteEpsilonTask();
...
const EVENT_META: Record<EpsilonTaskEvent['type'], EventMeta> = {
  progress: {
    icon: Activity,
    color: 'text-blue-500',
...
function Timeline({ events }: { events: EpsilonTaskEvent[] }) {
  return (
