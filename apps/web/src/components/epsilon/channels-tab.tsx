...
/**
 * Project Channels tab — filtered view of /epsilon/channels scoped to
...
async function projectChannelsFetch(serverUrl: string, path: string, opts?: RequestInit): Promise<any> {
  try {
    const res = await authenticatedFetch(`${serverUrl}/epsilon/channels${path}`, opts);
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-muted-foreground truncate">
          ...
          {ch.default_agent && ch.default_agent !== 'epsilon' ? ` · ${ch.default_agent}` : ''}
        </p>
      </div>
