...
const SSHTerminal = dynamic(
  () => import('@/components/thread/epsilon-computer/components/SSHTerminal').then(mod => ({ default: mod.SSHTerminal })),
  { ssr: false }
);
...
export function TerminalTabContent({ ptyId, tabId, hidden = false }: TerminalTabContentProps) {
  const currentSandboxId = useEpsilonComputerStore((s) => s.currentSandboxId);
