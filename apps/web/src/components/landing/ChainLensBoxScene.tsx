...
interface EpsilonBoxModelProps {
...
function EpsilonBoxModel({ progressRef, isOn, setIsOn }: EpsilonBoxModelProps) {
  const groupRef = useRef<THREE.Group>(null!);
  const innerRef = useRef<THREE.Group>(null!);
  const { scene } = useGLTF('/models/epsilon_box.glb');
  ...
  useFrame((state, delta) => {
    ...
    // Start front-on (LED visible), tilt to reveal top (Epsilon engraving) on scroll
    ...
    <div className="absolute inset-0 pointer-events-auto">
      ...
      >
        ...
        <Suspense fallback={null}>
          <EpsilonBoxModel progressRef={scrollProgressRef} isOn={isOn} setIsOn={setIsOn} />
          <ContactShadows
            position={[0, -1.5, 0]}
            opacity={0.4}
            scale={10}
...
useGLTF.preload('/models/epsilon_box.glb');
