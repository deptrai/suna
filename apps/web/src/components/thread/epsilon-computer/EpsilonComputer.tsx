...
import { isHiddenTool } from '@epsilon/shared/tools';
...
import { 
  ...
  useEpsilonComputerPendingToolNavIndex,
  useEpsilonComputerClearPendingToolNav,
...
interface EpsilonComputerProps {
  ...
  /** When true, EpsilonComputer skips rendering its own PanelHeader (tabs are provided externally, e.g. by SessionLayout). */
  hideTopBar?: boolean;
...
const FLOATING_LAYOUT_ID = 'epsilon-computer-float';

export const EpsilonComputer = memo(function EpsilonComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = useEpsilonComputerPendingToolNavIndex();
  const clearPendingToolNav = useEpsilonComputerClearPendingToolNav();
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[EpsilonComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
