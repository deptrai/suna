...
export function ChannelSettingsDialog({ channel, open, onOpenChange, onUpdated }: ChannelSettingsDialogProps) {
  ...
  useEffect(() => {
    if (channel) {
      ...
      setAgentName(channel.default_agent || 'epsilon');
      ...
      if (channel.default_model && channel.default_model.includes('/')) {
        ...
        setSelectedModel({ providerID: 'epsilon', modelID: channel.default_model });
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await authenticatedFetch(`${url}/epsilon/channels/${channel.id}`, {
        method: 'PATCH',
