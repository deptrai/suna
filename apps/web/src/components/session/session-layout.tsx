...
import { EpsilonComputer } from '@/components/thread/epsilon-computer';
...
} from '@/stores/epsilon-computer-store';
import {
  useOpenCodeMessages,
  useOpenCodeSession,
...
import {
  ...
  adaptAgentStatus,
} from '@/lib/adapters/opencode-to-epsilon-computer';
...
export const SessionLayout = memo(function SessionLayout({
  ...
  // open session tab.
  const isSidePanelOpen = useEpsilonComputerStore((s) => s.isSidePanelOpen);
  const setIsSidePanelOpen = useEpsilonComputerStore((s) => s.setIsSidePanelOpen);
  const setActiveSession = useEpsilonComputerStore((s) => s.setActiveSession);
  const shouldOpenPanel = useEpsilonComputerStore((s) => s.shouldOpenPanel);
  const clearShouldOpenPanel = useEpsilonComputerStore((s) => s.clearShouldOpenPanel);
  const isExpanded = useEpsilonComputerStore((s) => s.isExpanded);
  const toggleExpanded = useEpsilonComputerStore((s) => s.toggleExpanded);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div ref={panelGroupRef} className="flex-1 min-h-0 flex overflow-hidden">
        ...
        >
          ...
          {/* Side panel (EpsilonComputer — Actions only) */}
          <ResizablePanel
            ref={sidePanelRef}
