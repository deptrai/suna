...
async function cmdUpdate(sandboxId: string, targetVersion: string) {
  ...
  if (exists.stdout?.trim() === 'cached') {
    ...
    await execOnHost(endpoint, `systemd-run --unit=epsilon-image-pull docker pull ${targetImage}`, 15);
    for (let i = 0; i < 60; i++) {
...
for db in glob.glob('/workspace/.local/share/opencode/*.db'):
 c=sqlite3.connect(db);c.execute('PRAGMA wal_checkpoint(TRUNCATE)');c.close()" 2>/dev/null || true`, 10);
  ...
  await execOnHost(endpoint, `echo '${b64}' | base64 -d > /tmp/epsilon-update.sh && chmod +x /tmp/epsilon-update.sh`, 5);
  const unitName = `epsilon-test-${Date.now()}`;
  await execOnHost(endpoint, `systemctl reset-failed epsilon-test-restart 2>/dev/null || true`, 5);
  const restart = await execOnHost(endpoint, `systemd-run --unit=${unitName} /tmp/epsilon-update.sh`, 15);
