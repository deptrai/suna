...
cat > /usr/local/bin/epsilon-authorized-keys <<'EOF'
#!/bin/bash
set -euo pipefail
...
cat > /usr/local/bin/epsilon-container-shell <<'EOF'
#!/bin/bash
set -euo pipefail
...
cat > /etc/ssh/sshd_config.d/epsilon-sandbox.conf <<'EOF'
Match User abc
...
const result = await execOnHost(
  endpoint,
  `printf '%s' '${scriptB64}' | base64 -d > /tmp/epsilon-ssh-bridge.sh && bash /tmp/epsilon-ssh-bridge.sh`,
  60,
);
