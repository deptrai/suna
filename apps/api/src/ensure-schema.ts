 ...
 *
 * In production (INTERNAL_EPSILON_ENV=prod), schema is managed by external
...
export async function ensureSchema(): Promise<void> {
  ...
  if (process.env.EPSILON_SKIP_ENSURE_SCHEMA === '1') {
    console.log('[schema] EPSILON_SKIP_ENSURE_SCHEMA=1 — skipping');
    return;
  }

  // Production: schema managed externally (CI/CD migrations)
  if (config.INTERNAL_EPSILON_ENV === 'prod') {
