...
describe('sandbox container port config', () => {
  ...
  test('buildContainerConfig inherits default ports', () => {
    const config = buildContainerConfig({ image: 'epsilon/computer:0.8.20' });
  ...
  test('foreground run command omits detached flag for systemd-managed sandboxes', () => {
    const config = buildContainerConfig({ image: 'epsilon/computer:0.8.20', envFile: JUSTAVPS_ENV_FILE, containerName: 'justavps-workload' });
  ...
  test('managed service start script reuses persisted env when already present', () => {
    ...
    expect(script).toContain('grep -Eq "^(INTERNAL_SERVICE_KEY|EPSILON_TOKEN|EPSILON_API_URL)="');
    expect(script).toContain('exec docker run --rm');
    expect(script).toContain('--privileged');
    expect(script).toContain('EPSILON_ENABLE_INNER_DOCKER=0');
