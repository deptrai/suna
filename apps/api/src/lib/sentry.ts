/**
 * Sentry error tracking for Epsilon API.
 *
...
const ENV = process.env.INTERNAL_EPSILON_ENV || 'dev';
...
if (SENTRY_DSN) {
  Sentry.init({
    ...
    beforeSend(event) {
      ...
      if (event.request?.headers) {
        ...
        for (const key of ['authorization', 'cookie', 'x-epsilon-token', 'x-api-key']) {
          if (headers[key]) {
  ...
  console.log(`[sentry] Initialized (env=${ENV}, release=epsilon-api@${VERSION})`);
} else {
