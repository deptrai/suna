...
/**
 * Epsilon API key prefixes.
 *
 *   epsilon_      — user-created API key (for external programmatic access)
 *   epsilon_sb_   — sandbox-managed key (auto-created per sandbox, used by agents)
...
/**
 * Check if a token is a Epsilon-issued key (user or sandbox).
 * Single check for the router — no branching on multiple prefixes.
 */
export function isEpsilonToken(token: string): boolean {
  return token.startsWith(KEY_PREFIX);
}

/**
 * Generate a public/secret key pair for a user-created API key.
 * Secret key: epsilon_<32 chars>  (shown once, only hash stored)
 ...
 * Secret key: epsilon_sb_<32 chars>  (injected as EPSILON_TOKEN into sandbox)
 * Public key: pk_<32 chars>          (safe to store/display)
 */
 ...
 * Token: epsilon_tnl_<32 chars> (shown once during tunnel creation, only hash stored)
 */
