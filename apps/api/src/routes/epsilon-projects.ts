/**
 * Epsilon API proxy handler — /v1/epsilon/*
 *
...
export async function epsilonProxyHandler(c: Context): Promise<Response> {
  // /v1/epsilon/projects/xxx → /epsilon/projects/xxx
  const sandboxPath = c.req.path.replace(/^\/v1/, '').replace(/\/+$/, '') || '/epsilon';
