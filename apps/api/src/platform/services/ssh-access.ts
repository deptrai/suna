...
export function resolvePublicSSHHost(c: Context): string {
  ...
  for (const raw of headerCandidates) {
    ...
    if (!host) continue;
    if (host === 'host.docker.internal' || host === 'epsilon-api') continue;
    return host;
  }

  const configCandidates = [config.EPSILON_URL, config.FRONTEND_URL];
...
export function buildSSHConnectionInfo(opts: {
  ...
  const keyName = `epsilon_${hostToken}`;
  const hostAlias = `epsilon-${hostToken}`;
  ...
  const sshConfigCommand = `mkdir -p ~/.ssh && touch ~/.ssh/config && chmod 600 ~/.ssh/config && grep -q "Host ${hostAlias}" ~/.ssh/config 2>/dev/null || cat >> ~/.ssh/config << 'EPSILON_SSH_CONFIG'\n\n${sshConfigEntry}\nEPSILON_SSH_CONFIG`;
...
export function buildSSHSetupPayload(opts: {
  ...
  const setupCommand = `mkdir -p ~/.ssh && cat > ~/.ssh/${opts.connection.key_name} << 'EPSILON_KEY'\n${pk}\nEPSILON_KEY\nchmod 600 ~/.ssh/${opts.connection.key_name} && ${opts.connection.reconnect_command}`;
  const agentPrompt = `Set up SSH access to my remote Epsilon instance and connect via Remote-SSH.\n\n1. Save the SSH key:\n${setupCommand}\n\n2. Add to ~/.ssh/config:\n${opts.connection.ssh_config_entry}\n\n3. Connect via Remote-SSH to host "${opts.connection.host_alias}"`;
