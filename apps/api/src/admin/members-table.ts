 ...
 * (columns: user_id, account_id, account_role). Newer Epsilon-native
 * deployments use `epsilon.account_members` (same shape + created_at).
 *
...
type MembersTable = 'epsilon.account_members' | 'basejump.account_user';
...
async function probe(): Promise<MembersTable> {
  ...
  try {
    await db.execute(sql`SELECT 1 FROM epsilon.account_members LIMIT 1`);
    return 'epsilon.account_members';
  } catch {
