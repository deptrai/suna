...
export interface ProxyServiceConfig {
  ...
  /** Alternate upstream base URL for Epsilon-managed requests (Mode 1). Falls back to targetBaseUrl. */
  epsilonTargetBaseUrl?: string;
  /** Epsilon-owned API key for this upstream service */
  getEpsilonApiKey: () => string;
  /** How to inject the API key into upstream requests (passthrough) */
  keyInjection: KeyInjectionMethod;
  /** Alternate key injection for Epsilon-managed requests (Mode 1). Falls back to keyInjection. */
  epsilonKeyInjection?: KeyInjectionMethod;
  /** Only these routes are allowed when using Epsilon's key (prevents cost abuse) */
...
export function getProxyServices(): Record<string, ProxyServiceConfig> {
  return {
    tavily: {
      ...
      getEpsilonApiKey: () => config.TAVILY_API_KEY,
    ...
    serper: {
      ...
      getEpsilonApiKey: () => config.SERPER_API_KEY,
    ...
    firecrawl: {
      ...
      getEpsilonApiKey: () => config.FIRECRAWL_API_KEY,
    ...
    replicate: {
      ...
      getEpsilonApiKey: () => config.REPLICATE_API_TOKEN,
    ...
    context7: {
      ...
      getEpsilonApiKey: () => config.CONTEXT7_API_KEY,
      keyInjection: { type: 'header', headerName: 'Authorization', prefix: 'Bearer ' },
      allowedRoutes: [
    ...
    //
    // Dual-mode:
    // - Epsilon-managed (Mode 1): uses Epsilon-owned provider keys.
    ...
    //
    // Mode 1 (Epsilon token in auth): inject provider key configured in service
    // Mode 2 (user key + X-Epsilon-Token): passthrough to real provider
    //
    // The proxy handler picks targetBaseUrl for Mode 2/3 and
    // epsilonTargetBaseUrl for Mode 1 (when present).

    anthropic: {
      name: 'anthropic',
      targetBaseUrl: config.ANTHROPIC_API_URL,   // https://api.anthropic.com/v1
      getEpsilonApiKey: () => config.ANTHROPIC_API_KEY,
      keyInjection: { type: 'header', headerName: 'x-api-key' },
      allowedRoutes: [
    ...
    openai: {
      ...
      getEpsilonApiKey: () => config.OPENAI_API_KEY,
      keyInjection: { type: 'header', headerName: 'Authorization', prefix: 'Bearer ' },
      allowedRoutes: [
    ...
    xai: {
      ...
      epsilonTargetBaseUrl: config.OPENROUTER_API_URL,
      getEpsilonApiKey: () => config.OPENROUTER_API_KEY,
      keyInjection: { type: 'header', headerName: 'Authorization', prefix: 'Bearer ' },
      allowedRoutes: [
        { path: '/chat/completions', methods: ['POST'] },
      ],
