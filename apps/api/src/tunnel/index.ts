 ...
 * This file wires in Epsilon-specific business logic: DB persistence,
...
function startTunnelService(): void {
  ...
  tunnelRelay.on('agent:connect', async ({ tunnelId, metadata }) => {
    ...
    try {
      ...
      const { tunnelConnections, tunnelPermissions } = await import('@epsilon/db');
      const { db } = await import('../shared/db');
  ...
  permissionCleanupInterval = setInterval(async () => {
    try {
      ...
      const { tunnelPermissions } = await import('@epsilon/db');
      const { db } = await import('../shared/db');
      ...
      const { tunnelDeviceAuthRequests } = await import('@epsilon/db');
      await db
