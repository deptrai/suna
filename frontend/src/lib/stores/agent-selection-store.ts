...
interface AgentSelectionState {
  ...
  isChainlensAgent: (agents: Agent[]) => boolean;
}
  ...
  persist(
    (set, get) => ({
      ...
      initializeFromAgents: (agents: Agent[], threadAgentId?: string, onAgentSelect?: (agentId: string | undefined) => void) => {
        ...
        if (!selectedId) {
          ...
          if (current && agents.some(a => a.agent_id === current)) {
            ...
            const defaultChainlensAgent = agents.find(agent => agent.metadata?.is_chainlens_default);
            selectedId = defaultChainlensAgent ? defaultChainlensAgent.agent_id : agents[0].agent_id;
          }
        }
      ...
      autoSelectAgent: (agents: Agent[], onAgentSelect?: (agentId: string | undefined) => void, currentSelectedAgentId?: string) => {
        ...
        const defaultChainlensAgent = agents.find(agent => agent.metadata?.is_chainlens_default);
        const agentToSelect = defaultChainlensAgent || agents[0];
      ...
      isChainlensAgent: (agents: Agent[]) => {
        const { selectedAgentId } = get();
        const currentAgent = selectedAgentId 
          ? agents.find(agent => agent.agent_id === selectedAgentId)
          : null;
        return currentAgent?.metadata?.is_chainlens_default || selectedAgentId === undefined;
      },
    }),
    {
...
export const useAgentSelection = () => {
  ...
  return {
    ...
    isChainlensAgent: store.isChainlensAgent,
  };
}; 