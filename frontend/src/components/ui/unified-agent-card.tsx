...
const CardAvatar: React.FC<{ 
  ...
  const isChainLensAgent = data.metadata?.is_chainlens_default === true;
...
const AgentBadges: React.FC<{ data: BaseAgentData, isChainLensAgent: boolean }> = ({ data, isChainLensAgent }) => (
  <div className="flex gap-1">
    {!isChainLensAgent && data.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {data.current_version.version_name}
      </Badge>
    )}
    {!isChainLensAgent && data.is_public && (
...
export const UnifiedAgentCard: React.FC<UnifiedAgentCardProps> = ({
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        case 'marketplace':
          return <MarketplaceBadge isEpsilonTeam={data.is_epsilon_team} isOwner={isOwner} />;
        case 'template':
          return <TemplateBadge isPublic={data.is_public} />;
        case 'agent':
          return <AgentBadges data={data} isChainLensAgent={isChainLensAgent} />;
        default:
          return null;
      }
    };
