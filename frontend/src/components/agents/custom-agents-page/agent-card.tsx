...
const MarketplaceBadge: React.FC<{ 
  isEpsilonTeam?: boolean; 
  isOwner?: boolean;
}> = ({ isEpsilonTeam, isOwner }) => {
  return (
...
const AgentBadges: React.FC<{ agent: AgentData, isChainlensAgent: boolean }> = ({ agent, isChainlensAgent }) => (
  <div className="flex gap-1">
    {!isChainlensAgent && agent.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {agent.current_version.version_name}
      </Badge>
    )}
    {!isChainlensAgent && agent.is_public && (
...
export const AgentCard: React.FC<AgentCardProps> = ({
  ...
  const isChainlensAgent = mode === 'agent' && (data as AgentData).metadata?.is_chainlens_default === true;
    ...
    switch (mode) {
      case 'marketplace':
        return <MarketplaceBadge 
          isEpsilonTeam={(data as MarketplaceData).is_epsilon_team} 
          isOwner={isOwner}
        />;
      case 'template':
        return <TemplateBadge isPublic={(data as TemplateData).is_public} />;
      case 'agent':
        return <AgentBadges agent={data as AgentData} isChainlensAgent={isChainlensAgent} />;
      default:
        return null;
    }
  };
