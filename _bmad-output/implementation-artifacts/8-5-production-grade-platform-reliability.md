...
**AC3 — Provisioning Status Accuracy (Issue 3) — CONFIRMED BUG, CẦN FIX**

- **Given** `ensure-sandbox.ts` `provisionAsync()` là code path cho Daytona provider — nó update `externalId`, `baseUrl`, `metadata` nhưng **KHÔNG set `status='active'`** sau khi provision hoàn tất. Sandbox bị kẹt mãi ở `'provisioning'`.
- **When** Daytona sandbox provisioning thành công (epsilon/health trả 200, `create()` resolve).
- **Then** `provisionAsync()` success path phải thêm explicit DB update:
  ```typescript
  await db.update(sandboxes)
    .set({ status: 'active', updatedAt: new Date() })
    .where(eq(sandboxes.sandboxId, row.sandboxId));
  ```
- **And** Các path khác đã đúng: `tryClaimFromPool()` (INSERT trực tiếp `status: 'active'`), `tryReactivateStaleSandbox()` (UPDATE sang `'active'`), `provisionSync()` (UPDATE sau `provider.create()` thành công).
- **And** Add unit test assertion: mock `provider.create()` resolve → assert DB row `status === 'active'` (không phải `'provisioning'`).

---

### Sprint 2 — Security (Issues 4, 5)

**AC4 — serviceKey Encryption at Rest (Issue 4) — REVISED: ENCRYPTION ĐÃ CÓ SẴN**

> ⚠️ **Architect note (2026-05-19)**: Story ban đầu mô tả implement `pgcrypto` + `BYTEA` column mới. Tuy nhiên `apps/api/src/shared/sandbox-secrets.ts` đã implement **AES-256-GCM encryption** hoàn chỉnh tại application layer. Thêm `pgp_sym_encrypt` sẽ tạo 2 encryption schemes song song — avoid. AC4 được rewrite thành "enforce + audit".

- **Given** `sandbox-secrets.ts` đã có `setSandboxServiceKeyInConfig()` (encrypt AES-256-GCM vào `serviceKeyEnc` JSONB field) và `getSandboxServiceKeyFromConfig()` (decrypt với fallback sang plaintext `serviceKey` cho migration).
- **When** audit toàn bộ codebase.
- **Then** tất cả code paths ghi serviceKey vào DB PHẢI dùng `setSandboxServiceKeyInConfig()` — không direct JSONB write với key `serviceKey` plaintext.
  - Tìm vi phạm: `grep -rn '"serviceKey"' apps/api/src/ --include="*.ts"` — kết quả chỉ được phép là trong `sandbox-secrets.ts` constants.
- **And** `SANDBOX_SERVICE_KEY_ENCRYPTION_SECRET` phải là **required** trong cloud mode (Daytona/JustAVPS). Thêm runtime validation trong `apps/api/src/config.ts`:
  ```typescript
  if ((config.DAYTONA_API_KEY || config.JUSTAVPS_API_KEY) && !config.SANDBOX_SERVICE_KEY_ENCRYPTION_SECRET) {
    throw new Error('[config] SANDBOX_SERVICE_KEY_ENCRYPTION_SECRET required in cloud sandbox mode');
  }
  ```
- **And** Audit DB: verify không còn row nào có `config->>'serviceKey'` plaintext (những row cũ chưa migrate):
  ```sql
  SELECT COUNT(*) FROM epsilon.sandboxes WHERE config->>'serviceKey' IS NOT NULL AND config->>'serviceKeyEnc' IS NULL;
  -- Should be 0. If not: run setSandboxServiceKeyInConfig migration script.
  ```
- **And** KHÔNG thêm column mới, KHÔNG dùng pgcrypto — encryption đã đủ ở application layer.
...
**AC7 — Multi-Replica Safe Dedup via Postgres Advisory Lock (Issue 7) — REVISED: SESSION-LEVEL LOCK**

> ⚠️ **Architect note (2026-05-19)**: Story ban đầu dùng `pg_try_advisory_xact_lock` (transaction-scoped). Daytona `create()` timeout là 960 giây — giữ DB transaction mở 16 phút sẽ exhaust connection pool. Phải dùng session-level lock thay vì xact-level.

- **Given** `provisioningSubscriptions: Set<string>` (line 17 `sandbox-provisioner.ts`) là in-memory — không chia sẻ giữa API replicas.
- **When** scale API lên 2+ replicas (Docker Swarm).
- **Then** Replace in-memory Set bằng **session-level** `pg_try_advisory_lock` + explicit unlock:
  ```typescript
  ...
  try {
    // ... existing provision logic (có thể mất hàng phút) ...
  } finally {
    await db.execute(sql`SELECT pg_advisory_unlock(${lockKey}::bigint)`);
  }
  ```
- **And** KHÔNG wrap provision logic trong DB transaction (sẽ giữ connection open 16 phút). Lock và provision là 2 operation riêng.
- **And** `finally` block đảm bảo lock luôn được release kể cả khi provision throw.
...
### Sprint 1 — Task 3: Fix Missing Status Transition trong provisionAsync() (AC3)

> **BUG CONFIRMED (Winston review 2026-05-19)**: `provisionAsync()` là Daytona path — KHÔNG set `status='active'` sau khi xong. Cần add explicit update.

- [ ] **apps/api/src/platform/services/ensure-sandbox.ts** `provisionAsync()`: Tìm success path cuối hàm (sau khi `create()` resolve và DB update `externalId`/`baseUrl`/`metadata`), thêm:
  ```typescript
  // Transition to active — provisionAsync() completes async after Daytona create() resolves
  await db.update(sandboxes)
    .set({ status: 'active', updatedAt: new Date() })
    .where(eq(sandboxes.sandboxId, row.sandboxId));
  console.log(`[ensure-sandbox] provisionAsync: sandbox ${row.sandboxId} status → active`);
  ```
- [ ] Verify các paths khác đã đúng (chỉ đọc, không sửa):
  - `tryClaimFromPool()` → INSERT trực tiếp `status: 'active'` ✓
  - `tryReactivateStaleSandbox()` → UPDATE `status='active'` ✓
  - `provisionSync()` → UPDATE sau `provider.create()` thành công ✓
- [ ] Add unit test trong `ensure-sandbox.test.ts`: mock Daytona `provider.create()` resolve → assert DB row `status === 'active'` (không phải `'provisioning'`)

---

### Sprint 2 — Task 4: serviceKey Encryption Audit & Enforce (AC4)

> Encryption đã có trong `sandbox-secrets.ts` — task này là AUDIT + ENFORCE, không phải implement mới.

- [ ] **Audit plaintext writes**: Chạy và verify kết quả:
  ```bash
  grep -rn '"serviceKey"' apps/api/src/ --include="*.ts"
  # Chỉ được phép xuất hiện trong apps/api/src/shared/sandbox-secrets.ts (constants)
  # Nếu tìm thấy chỗ khác → fix để dùng setSandboxServiceKeyInConfig()
  ```
- [ ] **apps/api/src/config.ts**: Thêm runtime validation cloud mode:
  ```typescript
  // Sau khi parse config, trước khi export:
  if ((env.DAYTONA_API_KEY || env.JUSTAVPS_API_KEY) && !env.SANDBOX_SERVICE_KEY_ENCRYPTION_SECRET) {
    throw new Error('[config] SANDBOX_SERVICE_KEY_ENCRYPTION_SECRET required in cloud sandbox mode');
  }
  ```
- [ ] **DB audit**: Kiểm tra production DB có row nào còn plaintext không:
  ```sql
  SELECT COUNT(*) FROM epsilon.sandboxes
  WHERE config->>'serviceKey' IS NOT NULL AND config->>'serviceKeyEnc' IS NULL;
  ```
  Nếu count > 0: chạy migration script (dùng `setSandboxServiceKeyInConfig` logic tương đương):
  ```sql
  -- Không dùng pgcrypto. Migration script chạy tại application layer qua admin CLI.
  -- Xem sandbox-secrets.ts buildSandboxServiceKeyConfigPatch() để generate patch.
  ```
- [ ] **Dokploy env**: Verify `SANDBOX_SERVICE_KEY_ENCRYPTION_SECRET` đã set trong production Dokploy env (nếu chưa → add ngay)
- [ ] KHÔNG thêm BYTEA column mới, KHÔNG enable pgcrypto extension
...
### Sprint 3 — Task 7: Postgres Advisory Lock — SESSION-LEVEL (AC7)

- [ ] **apps/api/src/platform/services/sandbox-provisioner.ts**: Replace in-memory Set với session-level lock:
  ```typescript
  ...
  export async function provisionSandboxFromCheckout(opts: {...}) {
    ...
    // Session-level lock (KHÔNG phải xact-level) — provision có thể mất tới 16 phút
    const [{ acquired }] = await db.execute(
      sql`SELECT pg_try_advisory_lock(${lockKey}::bigint) as acquired`
    );
    if (!acquired) {
    ...
    try {
      // ... existing provision logic (findBySubscription, pool.grab, createSandbox, etc.) ...
    } finally {
      await db.execute(sql`SELECT pg_advisory_unlock(${lockKey}::bigint)`);
    }
  }
  ```
- [ ] Remove `provisioningSubscriptions.add/has/delete` calls (toàn bộ)
- [ ] KHÔNG wrap provision logic trong `db.transaction()` — lock và provision là 2 operation độc lập
- [ ] Add unit test: 2 concurrent calls với cùng subscriptionId → chỉ 1 call acquire lock, call còn lại return `{ row: null, created: false }` ngay lập tức
...
- **2026-05-19 Architect Review (Winston)**: 3 corrections sau khi đọc codebase thực tế:
  - **AC3**: BUG CONFIRMED — `provisionAsync()` (Daytona path) không set `status='active'`. Rewritten từ "audit" → "fix missing transition".
  - **AC4**: `sandbox-secrets.ts` đã implement AES-256-GCM encryption hoàn chỉnh tại application layer. Task rewritten từ "implement pgcrypto + BYTEA column" → "audit + enforce existing encryption". Không cần DB schema change.
  - **AC7**: `pg_try_advisory_xact_lock` không phù hợp vì Daytona provision timeout 960s — changed sang session-level `pg_advisory_lock/unlock` với explicit try/finally.
  - **AC9 prerequisite**: Confirmed `chainlens.net` đã add vào Cloudflare — không blocked.
