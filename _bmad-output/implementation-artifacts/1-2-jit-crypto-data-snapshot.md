...
Status: done (review patches applied 2026-05-09)
...
_Code review run: 2026-05-09 (Blind Hunter + Edge Case Hunter + Acceptance Auditor)_

#### Decision-needed (3)

- [x] [Review][Decision][Resolved=B-pragmatic] **Billing atomicity: synchronous deduct vs deferred microtask** — Hiện tại `deductToolCredits` chạy trong `queueMicrotask` SAU khi response đã trả về (`apps/api/src/router/routes/jit-sync.ts:95-103`). Nếu deduct fail → chỉ `console.warn`, user nhận data không trả phí. Spec dòng 47 cho phép "log warn vẫn return data" nhưng AC5 nhấn mạnh NFR8 atomic. Lựa chọn: (A) deduct synchronously trước `c.json` (đúng tinh thần atomic, +5–20ms latency), (B) giữ async + retry queue/DLQ + structured metric, (C) giữ nguyên (chấp nhận revenue-leak nhỏ trên billing outage).
- [x] [Review][Decision][Resolved=Remove] **`metrics` parameter — implement hay loại bỏ** — Schema accept `metrics: ['tvl','apy','volume','fees']` (`types.ts`) nhưng `fetchProtocolSnapshot` không bao giờ honor. APY luôn `null`, volume/fees không fetch. Tool description hứa "TVL, APY, chain data" → LLM bị mislead. Lựa chọn: (A) loại metrics khỏi public schema + sửa tool description (MVP đúng spec Dev Notes), (B) implement metrics filter ngay (kéo `/yields/poolsOld` vào — vượt scope MVP, thuộc Story 2.1).
- [x] [Review][Decision][Resolved=Header+Metric] **Stale fallback observability** — Stale-fallback (DeFiLlama down, có cache) trả HTTP 200 đúng spec AC6 nhưng dashboards/SLO sẽ không thấy upstream outage. Có thêm `X-Cache-Status: stale-fallback` header + log structured metric không?

#### Patches (18)

- [x] [Review][Patch] **AC3 FAIL — server-side timeout 900ms ≠ spec 1200ms** [`apps/api/src/router/services/defillama.ts:4`] — `JIT_SYNC_TIMEOUT_MS = 900` phải là `1200`.
- [x] [Review][Patch] **AC6 — cache key thiếu `metrics_sorted_csv`** [`apps/api/src/router/services/jit-cache.ts:13-15`] — `cacheKey(slug, chain)` cần signature `cacheKey(slug, chain, metrics)` và include sorted-CSV theo spec subtask 2.4 + AC6. Cache hiện tại pollute giữa các metric subsets.
- [x] [Review][Patch] **AC5 / subtask 5.11 — `cost: 0` hardcoded** [`apps/api/src/router/routes/jit-sync.ts:78, 90`] — Phải populate từ `getToolCost('jit_sync', 0)` (hoặc return value của `deductToolCredits`). Hiện tại response nói `cost: 0` trong khi billing charge ~$0.0015 → ledger mismatch.
- [x] [Review][Patch] **`Partial<JitSyncProxyResponse>` cast che giấu schema violation** [`apps/api/src/router/routes/jit-sync.ts:79`] — Tool consumer (`jit_sync.ts`) declare các field non-optional (`tvl_usd`, `name`, `chains`) → access `.tvl_usd.toFixed(...)` sẽ NPE. Định nghĩa explicit `JitSyncErrorResponse` và return union `JitSyncProxyResponse | JitSyncErrorResponse`. Update tool interface tương ứng.
- [x] [Review][Patch] **Slug normalization — case mismatch giữa cache key và DeFiLlama URL** [`apps/api/src/router/routes/jit-sync.ts:33` + `services/defillama.ts:28`] — Route lowercase trong `cacheKey()` nhưng pass slug raw vào `fetchProtocolSnapshot` → URL `/protocol/Uniswap` 404 trong khi cache key đã là `uniswap:all`. Lowercase ngay tại route entry: `const slug = parseResult.data.protocol_slug.trim().toLowerCase();`.
- [x] [Review][Patch] **Slug validation — security/DoS** [`apps/api/src/types.ts` `JitSyncRequestSchema`] — Schema chỉ có `min(1)`. Thêm `.regex(/^[a-z0-9-]+$/).max(64)`. Direct API caller có thể inject path traversal hoặc 10KB string.
- [x] [Review][Patch] **session_id không max length** [`apps/api/src/types.ts` `JitSyncRequestSchema`] — Adversarial 10MB session_id → billing log/DB DoS. Thêm `.max(128)`.
- [x] [Review][Patch] **Chain filter case-insensitive + filter-result reporting** [`apps/api/src/router/services/defillama.ts:54-56, 71`] — User pass `chain="ethereum"` nhưng DeFiLlama keys PascalCase (`"Ethereum"`) → fallback về global TVL nhưng `chains: [chain]` lie. Build lowercased index của `chainTvls`, hoặc throw 404 khi không match. Update `chains` array để phản ánh thực tế filter.
- [x] [Review][Patch] **Missing `data.tvl` → silent zero** [`apps/api/src/router/services/defillama.ts:58`] — `tvlArr?.[…] ?? 0` trả `tvl_usd: 0` cho LLM khi DeFiLlama đổi shape → agent confidently nói protocol có $0. Nếu `tvlArr` empty/missing → `throw new Error('DeFiLlama returned no TVL data')` để trigger stale fallback.
- [x] [Review][Patch] **`tvl_change_24h_pct` nullable + timestamp-aware** [`apps/api/src/router/services/defillama.ts:58-60` + `types.ts ProtocolSnapshot`] — (1) `prev = last` khi length<2 → `0%` indistinguishable với "thực sự không đổi". (2) `prev = 0` (protocol mới) → `0%` thay vì "infinity/new". (3) Last-2-points không guarantee 24h apart. Đổi field type sang `number | null`. Compute null khi <2 points hoặc `prev === 0`. Lý tưởng: filter array tìm entry gần `now - 86400000` ms.
- [x] [Review][Patch] **Lesson 7 — wrap `response.json()` ở service** [`apps/api/src/router/services/defillama.ts:50`] — DeFiLlama có thể trả 200 với HTML error page. `await response.json()` SyntaxError hiện tại bubble out ngoài try block. Wrap: `try { data = await response.json(); } catch { throw new Error('DeFiLlama returned non-JSON'); }`.
- [x] [Review][Patch] **Cache eviction race + dead code** [`apps/api/src/router/services/jit-cache.ts:29-42`] — (1) Re-set existing key khi `size>1000` vẫn trigger eviction → trim 100 entries không cần thiết. Guard: `if (!cache.has(key) && cache.size >= 1000) { … }`. (2) Biến `oldest` capture rồi không dùng — xóa.
- [x] [Review][Patch] **`getCachedAny` cap max age** [`apps/api/src/router/services/jit-cache.ts:25-27`] — DeFiLlama down >24h → serve entry cũ với label "24h change" vô nghĩa. Cap ở 24h: nếu age > 24h → return null (caller sẽ rơi vào "no cached data" branch).
- [x] [Review][Patch] **`getCached` NaN guard** [`apps/api/src/router/services/jit-cache.ts:20`] — `new Date(invalid).getTime()` = `NaN` → `age > TTL` false → entry treated fresh forever (cache poisoning). `if (Number.isNaN(t)) return null;`.
- [x] [Review][Patch] **`dedupedFetch` race: setCache after inflight.delete** [`apps/api/src/router/services/jit-cache.ts:55` + `routes/jit-sync.ts:55-56`] — `.finally(() => inflight.delete(key))` chạy trước khi route handler resume tới `setCache`. Concurrent burst kế tiếp trong gap đó sẽ start live fetch mới. Move `setCache` vào trong `dedupedFetch` wrapper trước khi delete inflight.
- [x] [Review][Patch] **`creditCheck.message` echo verbatim** [`apps/api/src/router/routes/jit-sync.ts:37`] — Nếu billing service trả message chứa user-controlled data → leak vào HTTPException response. Dùng fixed string `'Insufficient credits'`, log details server-side.
- [x] [Review][Patch] **`formatUsd` / `formatSnapshot` Number.isFinite guards** [`apps/api/src/router/services/jit-cache.ts:62-77`] — `formatUsd(NaN)` → `"$NaN"`, `Infinity` → `"$Infinity"`, `apy_avg.toFixed` không guard NaN, `tvl_change_24h_pct.toFixed` cùng vấn đề, `chains.join('/')` với array rỗng → `**Name** ()`. Defensive: `if (!Number.isFinite(n)) return '$?';`, `if (Number.isFinite(data.apy_avg))`, fallback `'multi-chain'` cho chains rỗng.
- [x] [Review][Patch] **Tool slug regex chấp nhận leading/trailing dash** [`core/epsilon-master/opencode/tools/jit_sync.ts` slug regex] — `/^[a-z0-9-]+$/` cho phép `"-"` hoặc `"--"` → DeFiLlama 404, vẫn deduct credit. Đổi: `/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/`.

#### Deferred (10) — pre-existing/cross-cutting/lower-priority

- [x] [Review][Defer] **AbortSignal.timeout không cover body read** [`apps/api/src/router/services/defillama.ts:34-50`] — deferred, headers nhanh + body chậm là edge hiếm; Bun fetch thường fast.
- [x] [Review][Defer] **Console-based logging thay vì structured logger** [`defillama.ts:63`, `jit-sync.ts:100`] — deferred, project-wide convention.
- [x] [Review][Defer] **In-memory cache + horizontal scaling divergence** [`jit-cache.ts:10`] — deferred, by spec design (single-process), Story 2.1 sẽ Redis-back.
- [x] [Review][Defer] **`new Date(...).getTime()` parse mỗi cache hit** [`jit-cache.ts:20`] — deferred, micro-perf.
- [x] [Review][Defer] **Negative cache cho upstream failures (thundering herd)** [`jit-cache.ts:49-58`] — deferred, không xảy ra với DeFiLlama free-tier traffic hiện tại.
- [x] [Review][Defer] **Test mocks ignore arguments — schema regression silent pass** [`apps/api/src/__tests__/e2e-jit-sync.test.ts:50-56`] — deferred, test-quality follow-up.
- [x] [Review][Defer] **`combinedAuth` test mock — auth chưa được integration-tested** [`e2e-jit-sync.test.ts:29-31`] — deferred, cần Supabase test fixture.
- [x] [Review][Defer] **Hono body-size limit cho route** [`apps/api/src/router/routes/jit-sync.ts:21`] — deferred, project-wide middleware.
- [x] [Review][Defer] **`fetched_at` ISO trong snapshot — relative time tốt hơn cho LLM** [`jit-cache.ts:77`] — deferred, opinion/UX polish.
- [x] [Review][Defer] **Tool URL parse robustness, args.chain trim, response.text size cap, args.metrics enum-validate** [`core/epsilon-master/opencode/tools/jit_sync.ts`] — deferred, defensive guards thấp priority.
