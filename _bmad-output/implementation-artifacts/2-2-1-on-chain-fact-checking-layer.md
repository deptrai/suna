...
- [ ] **Task 0: Config and environment docs (AC1)**
  - [ ] 0.1 — Add `ONCHAIN_FACT_CHECK_WORKER_ENABLED`, `ONCHAIN_FACT_CHECK_INTERVAL_MS`, `ONCHAIN_FACT_CHECK_LOOKBACK_HOURS`, `ONCHAIN_FACT_CHECK_DUMP_THRESHOLD_PCT`, `ONCHAIN_FACT_CHECK_MAX_WALLETS_PER_TOKEN`, `ONCHAIN_FACT_CHECK_MAX_TRANSFERS_PER_WALLET`, `ONCHAIN_FACT_CHECK_PROVIDER`, `ONCHAIN_FACT_CHECK_CHAINS`, and per-chain `ONCHAIN_FACT_CHECK_RPC_URL_<CHAIN>` values to `apps/api/src/config.ts`.
  - [ ] 0.2 — Update `apps/api/.env.example` with an "On-chain Fact Checking Worker" section.
  - [ ] 0.3 — Ensure startup logs skip reasons when disabled or providers are unconfigured.

- [ ] **Task 1: DB schema and migration (AC2, AC3)**
  - [ ] 1.1 — Add `projectWalletWatchlist` and `onchainFactChecks` to `packages/db/src/schema/epsilon.ts`.
  - [ ] 1.2 — Export new tables from `packages/db/src/index.ts`.
  - [ ] 1.3 — Create next migration with no collision against Story 2.1.1/2.1.2 migrations.
  - [ ] 1.4 — Include indexes and unique constraints exactly as specified.

- [ ] **Task 2: Fact-check service module (AC4, AC5)**
  - [ ] 2.1 — Create `apps/api/src/router/services/onchain-fact-check.ts`.
  - [ ] 2.2 — Implement QuickNode JSON-RPC adapter for `eth_getLogs`, `eth_call`, and latest block reads using configured per-chain RPC URLs.
  - [ ] 2.3 — Implement wallet-scoped ERC-20 transfer reconstruction from `Transfer` logs, bounded by token contract, wallet, and lookback window.
  - [ ] 2.4 — Implement Etherscan-compatible/Blockscout fallback only when QuickNode is unavailable or explicitly disabled.
  - [ ] 2.5 — Implement current balance lookup when available; otherwise mark `transfer_only` evidence.
  - [ ] 2.6 — Implement deterministic dump scoring and risk factor generation.
  - [ ] 2.7 — Sanitize provider errors and raw responses before storing or returning.

- [ ] **Task 3: Worker queue (AC6, AC7)**
  - [ ] 3.1 — Create `apps/api/src/queue/bullmq/workers/onchain-fact-check-worker.ts`.
  - [ ] 3.2 — Implement `fact-check-discover-item` job.
  - [ ] 3.3 — Implement `refresh-recent-positive-feed` retry/backfill job.
  - [ ] 3.4 — Use `upsertJobScheduler` with the configured interval.
  - [ ] 3.5 — Add worker `error` and `failed` handlers.

- [ ] **Task 4: Discover feed integration (AC6, AC9)**
  - [ ] 4.1 — Extend `apps/api/src/queue/bullmq/workers/discover-feed.ts` to classify positive/promotional items before insert.
  - [ ] 4.2 — Resolve token/chain only from reliable source data or configured mappings.
  - [ ] 4.3 — Run fact check with timeout and graceful fallback.
  - [ ] 4.4 — Mutate title/summary/warning fields only when result is confidently `flagged`.
  - [ ] 4.5 — Ensure idempotency still works after summary/title changes.

- [ ] **Task 5: API route (AC8)**
  - [ ] 5.1 — Create `apps/api/src/router/routes/onchain-fact-check.ts`.
  - [ ] 5.2 — Mount under `router.use('/onchain-fact-check/*', combinedAuth)` and `router.route('/onchain-fact-check', onchainFactCheck)` in `apps/api/src/router/index.ts`.
  - [ ] 5.3 — Implement cache-first behavior and `force_refresh` billing gate.
  - [ ] 5.4 — Use tool cost key `onchain_fact_check` if pricing config exists; otherwise add it deliberately with product-approved default.

- [ ] **Task 6: Queue lifecycle wiring (AC7)**
  - [ ] 6.1 — Export `startOnchainFactCheckWorker`, `setupOnchainFactCheckJobs`, `stopOnchainFactCheckWorker` from `apps/api/src/queue/index.ts`.
  - [ ] 6.2 — Start/setup in `startBackgroundServices()` in `apps/api/src/index.ts`.
  - [ ] 6.3 — Stop in shutdown handler alongside existing workers.

- [ ] **Task 7: Frontend/shared type update if richer badge is added (AC9)**
  - [ ] 7.1 — Prefer existing `isAnomaly`/`warningLevel` first.
  - [ ] 7.2 — If adding structured fact-check fields to `/v1/discover`, add shared types before frontend changes.
  - [ ] 7.3 — Ensure no raw wallet metadata or provider payload leaks into public Discover response.

- [ ] **Task 8: Tests (AC10)**
  - [ ] 8.1 — Add unit tests for scoring and status mapping.
  - [ ] 8.2 — Add provider adapter tests with mocked Etherscan/Blockscout responses.
  - [ ] 8.3 — Add route tests for cache/live/billing paths.
  - [ ] 8.4 — Add worker test for Discover mutation behavior.
  - [ ] 8.5 — Run relevant `bun test` and API typecheck.
