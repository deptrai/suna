...
> ⚠️ **[F1] Version verification required:** Các version numbers trên (16.2.6, 19.0.6) là targets — verify latest stable trên npmjs.com trước khi chạy. Sau upgrade, chạy `pnpm install --dry-run` để detect peer dep conflicts với `@sentry/nextjs`, `framer-motion`, `react-hook-form`, `@logtail/next` trước khi commit.
...
> ℹ️ **[F2] Pre-verified:** Tất cả dynamic route pages đã dùng async params pattern (`use(params)` hoặc `await params`). Client components dùng `useParams()`/`useSearchParams()` (share, files, terminal, tunnel, templates, auth, admin, checkout) được miễn — không cần migrate. AC này sẽ pass ngay sau upgrade.
...
> ℹ️ **[F3] 7 files cần migrate (pre-identified):** `ui/resizable.tsx`, `ui/command.tsx`, `ui/phone-input.tsx`, `home/github-button.tsx`, `epsilon/mention-textarea.tsx`, `session/question-prompt.tsx`, `session/pty-terminal.tsx`.
...
> ℹ️ **[F4] Pre-verified:** `mermaid-renderer.tsx` đã dùng `await import('mermaid')` bên trong async handlers — không có static top-level import. Verify `code-block.tsx` không có static mermaid import trước khi đánh dấu AC này done.
...
> ⚠️ **[F5] File naming correction:** `SpreadsheetViewerInner` chưa tồn tại. Implementation: rename `SpreadsheetViewer.tsx` → `SpreadsheetViewerInner.tsx`, tạo `SpreadsheetViewer.tsx` mới là dynamic wrapper. Caller là `SpreadsheetToolview.tsx` và `SpreadsheetApp.tsx` — không cần thay đổi import path nếu wrapper giữ tên `SpreadsheetViewer`.
...
> ℹ️ **[F6] Pre-verified:** `ChainLensBoxScene.tsx` chỉ được dùng trong `(home)/variant-2/page.tsx` (WIP page, không phải production landing). Caller đã dùng `dynamic()`. Main landing `(home)/page.tsx` không dùng three.js. Verify với `ANALYZE=true bun run build` — three.js likely already absent from initial bundle.
...
> ⚠️ **[F7] File reference correction:** PDF renderer là `components/file-renderers/pdf-renderer.tsx`. File này import `react-pdf` (wrapper library) chứ không import `pdfjs-dist` trực tiếp — `pdfjs-dist` được bundle bởi `react-pdf`. Ngoài ra có `@react-pdf/renderer` (separate package cho PDF generation). Cần wrap `pdf-renderer.tsx` bằng `dynamic()` tại caller, và verify `@react-pdf/renderer` cũng không xuất hiện trong initial bundle.
...
> ℹ️ **[F8] Pre-verified:** `sqlite-renderer.tsx` đã dùng `await import('sql.js')` bên trong async function — không có static top-level import. AC này sẽ pass ngay. Verify với bundle analyzer để confirm.
...
> ℹ️ **[F9] Scope note:** Project dùng 20+ `@tiptap/*` extension packages. Cân nhắc thêm tất cả `@tiptap/*` extensions vào `optimizePackageImports` thay vì chỉ `@tiptap/core` và `@tiptap/react` để tối ưu đầy đủ.
...
> ⚠️ **[F12] Setup required trước AC này:** `@next/bundle-analyzer` chưa được install và chưa được configure trong `next.config.ts`. Bước setup: `pnpm add -D @next/bundle-analyzer`, sau đó wrap `nextConfig` với `withBundleAnalyzer({ enabled: process.env.ANALYZE === 'true' })` trong `next.config.ts`.
...
> ℹ️ **[F10] Pre-verified:** `(dashboard)/markets/loading.tsx` đã tồn tại. Verify nội dung đáp ứng yêu cầu skeleton (chart placeholder + table rows) — update nếu cần thay vì tạo mới.
...
> ⚠️ **[F11] Dependency required trước AC này:** `babel-plugin-react-compiler` chưa được install. Bước setup: `pnpm add -D babel-plugin-react-compiler` trước khi thêm `reactCompiler: true` vào `next.config.ts`.
