...
@router.post("/chainlens-agents/install-user/{account_id}")
async def admin_install_chainlens_for_user(
    account_id: str,
    replace_existing: bool = False,
    _: bool = Depends(verify_admin_api_key)
):
    """Install ChainLens agent for a specific user."""
    logger.debug(f"Admin installing ChainLens agent for user: {account_id}")
    
    service = ChainLensDefaultAgentService()
    agent_id = await service.install_chainlens_agent_for_user(account_id, replace_existing)
    
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed ChainLens agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install ChainLens agent for user {account_id}"
        )
