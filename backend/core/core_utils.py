...
async def check_agent_count_limit(client, account_id: str) -> Dict[str, Any]:
    ...
    Returns:
        ...
        - current_count: int - current number of custom agents (excluding Chainlens defaults)
    ...
    try:
        ...
        non_chainlens_agents = []
        for agent in agents_result.data or []:
            metadata = agent.get('metadata', {}) or {}
            is_chainlens_default = metadata.get('is_chainlens_default', False)
            if not is_chainlens_default:
                non_chainlens_agents.append(agent)
                
        current_count = len(non_chainlens_agents)
        logger.debug(f"Account {account_id} has {current_count} custom agents (excluding Chainlens defaults)")
