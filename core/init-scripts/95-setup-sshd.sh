...
SECRETS_DIR="$(dirname "${SECRET_FILE_PATH:-${EPSILON_PERSISTENT_ROOT:-/persistent}/secrets/.secrets.json}")"
mkdir -p "$SECRETS_DIR"
...
if ! grep -q 'epsilon-shell-defaults' /config/.bashrc 2>/dev/null; then
  cat >> /config/.bashrc <<'BASHRC'

# ── epsilon-shell-defaults ──
# Common aliases
