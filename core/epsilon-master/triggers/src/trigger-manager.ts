...
const EPSILON_MASTER_URL = "http://localhost:8000"
...
export class TriggerManager {
  ...
  constructor(
    ...
    // Reload handler: external CRUD paths (epsilon-master's /epsilon/triggers
    ...
    // Run handler: epsilon-master's /epsilon/triggers/:id/run forwards to
  ...
  private resolveDbPath(directory: string): string {
    // Use the central epsilon.db if it exists, otherwise create in .epsilon/
    const centralDb = `${directory}/.epsilon/epsilon.db`
    return centralDb
  }
  ...
  private rebuildWebhookRoutes(): void {
    ...
    for (const trigger of triggers) {
      ...
      const route: WebhookRoute = {
        agentName: trigger.agent_name ?? "epsilon",
        trigger: {
          name: trigger.name,
          source: {
  ...
  private async migrateFromOldSystem(): Promise<void> {
    ...
    const oldDir = path.join(directory, ".epsilon", "agent-triggers")
