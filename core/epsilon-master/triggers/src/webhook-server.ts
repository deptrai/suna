...
export class WebhookTriggerServer {
  ...
  /** Register a callback for POST /internal/reload. Used by epsilon-master's
   ...
   *  epsilon-master's HTTP /run hits this; without it, /run only logs a
   ...
   *  DB trigger state into .epsilon/triggers.yaml. epsilon-master's seed path
  ...
  async start(): Promise<void> {
    ...
    this.server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (pathname === "/health") {
        ...
        res.end(JSON.stringify({ ok: true, service: "epsilon-triggers", routes: this.routes.size }))
        return
      }

      // Internal run — epsilon-master's /epsilon/triggers/:id/run forwards here
      ...
      // .epsilon/triggers.yaml. Seed path calls this right after inserting
      ...
      // Internal reload — epsilon-master hits this after a /epsilon/triggers
      ...
      if (route.trigger.source.secret) {
        const supplied = headers["x-epsilon-opencode-trigger-secret"] ?? headers["x-epsilon-trigger-secret"] ?? ""
