...
function getWorkspaceDbPath(): string {
  const root = process.env.WORKSPACE_DIR || process.env.EPSILON_WORKSPACE || "/workspace"
  return join(root, ".epsilon", "epsilon.db")
}
    ...
    try {
      ...
      if (!exists) {
        throw new Error(
          ...
          `Fix via PATCH /epsilon/triggers/<id> or recreate with the correct project_id.`,
        )
      }
