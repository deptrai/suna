...
function getTriggerManagerRegistry(): TriggerManagerRegistry {
  const g = globalThis as typeof globalThis & { __epsilonTriggerManagers?: TriggerManagerRegistry }
  if (!g.__epsilonTriggerManagers) g.__epsilonTriggerManagers = new Map()
  return g.__epsilonTriggerManagers
}
...
function createLogger(client: PluginContextShape["client"], fallback?: TriggerPluginOptions["logger"]) {
  return (level: "info" | "warn" | "error", message: string) => {
    ...
    try {
      client.app?.log?.({ body: { service: "epsilon-triggers", level, message } }).catch(() => undefined)
    } catch {}
  }
}
...
Examples:
  ...
  triggers action=create name="Daily Report" source_type=cron cron_expr="0 0 9 * * *" action_type=prompt prompt="Generate the daily report" agent_name=epsilon
                  ...
                  if (actionType === "prompt") {
                    ...
                    if (args.project_id || args.ticket_id) {
                      try {
                        ...
                        const root = process.env.WORKSPACE_DIR || process.env.EPSILON_WORKSPACE || normalizedOptions.directory || "/workspace"
                        const dbPath = path.join(root, ".epsilon", "epsilon.db")
              ...
              switch (args.action) {
                ...
                case "setup": {
                  ...
                  const trigger = manager.createTrigger({
                    ...
                    action_type: "prompt",
                    action_config: { prompt: args.prompt },
                    agent_name: args.agent_name ?? "epsilon",
                    model_id: args.model_id,
                    session_mode: args.session_mode ?? "new",
