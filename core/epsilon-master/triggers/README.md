...
```
.epsilon/triggers.yaml → TriggerStore (DB) → CronScheduler / WebhookServer → ActionDispatcher → prompt | command | http
```

## Architecture

- **Config** lives in `.epsilon/triggers.yaml` (git-versionable)
- **Runtime state** lives in `epsilon.db` (is_active, last_run, executions)
- **API** at `/epsilon/triggers/*` — full CRUD
