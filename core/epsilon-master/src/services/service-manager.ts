...
const WORKSPACE_ROOT = process.env.EPSILON_WORKSPACE || "/workspace";
const SERVICE_STATE_DIR = join(WORKSPACE_ROOT, ".epsilon", "services");
...
const RECOVERY_THROTTLE_MS = Number(
  process.env.EPSILON_SERVICE_RECOVERY_THROTTLE_MS || 4_000,
);
const PORT_MIN = 10_000;
const PORT_MAX = 60_000;
const PERSISTED_SOURCE_ROOT = WORKSPACE_ROOT;
const ECONNRESET_GUARD_PATH = "/ephemeral/epsilon-master/econnreset-guard.cjs";
const INNER_DOCKER_ENABLED = process.env.EPSILON_ENABLE_INNER_DOCKER !== "0";
...
const BUILTIN_SERVICES: RegisteredServiceSpec[] = [
  // opencode-serve is spawn — managed directly by Epsilon Master (not s6)
  {
    id: "opencode-serve",
    ...
    sourceType: "files",
    sourceRef: null,
    startCommand: "bash /ephemeral/epsilon-master/scripts/run-opencode-serve.sh",
    installCommand: null,
    buildCommand: null,
    envVarKeys: [],
    deps: [],
...
export class ServiceManager {
  ...
  private async cleanupLegacyOrphans(): Promise<void> {
    const patterns = [
      ...
      "bash /ephemeral/epsilon-master/scripts/run-opencode-serve.sh",
