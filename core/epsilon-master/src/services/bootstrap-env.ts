 ...
 *
 * Core vars (EPSILON_TOKEN, EPSILON_YOLO_API_KEY, EPSILON_API_URL, INTERNAL_SERVICE_KEY,
 * TUNNEL_TOKEN) are the
 ...
 * AND the s6 env dir is empty, the sandbox can't authenticate to epsilon-api.
 * The SecretStore can't help because EPSILON_TOKEN is its own encryption key.
 *
 ...
 * the API rotates or re-issues EPSILON_TOKEN. The /env API and injectSandboxToken
 ...
 *
 * Older builds allowed EPSILON_TOKEN, EPSILON_YOLO_API_KEY, INTERNAL_SERVICE_KEY,
 * and TUNNEL_TOKEN to drift apart. The refactored model uses one sandbox token
 * for all four.
...
export function normalizeBootstrapAuthAliases(): number {
  const canonical = process.env.EPSILON_TOKEN || ''
  if (!canonical) return 0

  let updated = 0
  for (const key of ['EPSILON_YOLO_API_KEY', 'INTERNAL_SERVICE_KEY', 'TUNNEL_TOKEN'] as const) {
  ...
  if (updated > 0) {
    console.log(`[Bootstrap] Normalized ${updated} auth alias(es) to EPSILON_TOKEN`)
  }
  return updated
}
