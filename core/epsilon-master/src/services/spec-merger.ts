/**
 * OpenAPI Spec Merger — combines epsilon-master and OpenCode specs into one.
 *
 * Strategy:
 * - epsilon-master spec is generated via hono-openapi generateSpecs()
 * - OpenCode spec is fetched live from localhost:{OPENCODE_PORT}/doc
 * - Paths are merged (epsilon-master wins conflicts like /file, /file/content)
...
// Sandbox Gateway tags (from epsilon-master describeRoute)
const GATEWAY_TAGS: Array<{ name: string; description: string }> = [
  ...
  { name: 'GW: Secrets',      description: 'Environment variable / secret management (encrypted at rest via EPSILON_TOKEN)' },
...
// Map from old epsilon-master tag names to new GW: prefixed names
...
export async function buildMergedSpec(
  ...
  const openCodeSpec = await fetchOpenCodeSpec()
   ...
   // Start with epsilon-master as the base
  const merged: OpenAPISpec = {
    openapi: '3.1.0',
    info: {
      title: 'Epsilon Sandbox API',
      version: '1.0.0',
      description:
        'Unified API reference for the Epsilon sandbox.\n\n' +
        '**Sandbox Gateway** — the sandbox infrastructure layer (epsilon-master). ' +
        ...
        '### Epsilon API Key (programmatic access)\n\n' +
        'Create keys from **Settings > API Keys**. Each key is a public/secret pair:\n\n' +
        '| Key | Format | Length |\n' +
        '|-----|--------|--------|\n' +
        '| **Secret key** | `epsilon_` + 32 alphanumeric chars | 39 chars |\n' +
        ...
        '```\nAuthorization: Bearer epsilon_<your-secret-key>\n```\n\n' +
        ...
        'Each sandbox gets an auto-generated token (`epsilon_sb_` prefix, 42 chars) injected as the `EPSILON_TOKEN` environment variable. ' +
        ...
        '2. Proxy validates token (Epsilon API key → HMAC-SHA256 hash lookup, or Supabase JWT → `getUser()` verification)\n' +
    ...
    servers: [{ url: 'https://YOUR_EPSILON_API/v1/p/{sandboxId}/8000', description: 'Replace with your Epsilon API base URL' }],
    tags: [],
    paths: {},
    components: {
      schemas: {},
      securitySchemes: {
        EpsilonApiKey: {
          ...
          scheme: 'bearer',
          description:
            'Epsilon API key (`epsilon_` prefix, 39 characters total). ' +
            'Create one from Settings > API Keys in the dashboard. ' +
            'Pass as: `Authorization: Bearer epsilon_<your-key>`',
        },
        SupabaseJWT: {
          type: 'http',
          scheme: 'bearer',
  ...
  // 1) Add all epsilon-master paths first (they take priority)
  //    Remap their tags from "System" → "GW: System" etc.
  for (const [path, methods] of Object.entries(epsilonSpec.paths || {})) {
  ...
  // 2) Add OpenCode paths (skip conflicts where epsilon-master already has them)
  if (openCodeSpec) {
    for (const [path, methods] of Object.entries(openCodeSpec.paths || {})) {
      if (merged.paths[path]) {
        // Conflict — epsilon-master wins, but merge any missing methods
        for (const [method, details] of Object.entries(methods as Record<string, any>)) {
    ...
    const kmSchemas = epsilonSpec.components?.schemas || {}
