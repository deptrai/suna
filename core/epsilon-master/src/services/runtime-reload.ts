/**
 * Runtime Reload — Nuclear restart for the entire Epsilon stack.
 *
 * Two modes:
 ...
 *   svc-epsilon-master      — This process (port 8000)
 */
...
export function getSafeFullReloadFallback(options?: {
  ...
  if (envMode === 'local' && uid !== 0) {
    return 'Full restart is not supported from the local sandbox app process; performed a safe OpenCode dispose instead to avoid a epsilon-master restart loop'
  }
...
export async function initiateRuntimeReload(mode: ReloadMode): Promise<ReloadResult> {
  ...
  result.steps.push('Restarting epsilon-master last — s6 will respawn with fresh state')
  result.success = true

  // Self-restart — deferred so HTTP response goes out first
  setTimeout(() => {
    console.log('[runtime-reload] Full restart: killing epsilon-master — s6 will respawn')

    const restart = runS6Svc(['-r', '/run/service/svc-epsilon-master'])
    if (!restart.ok) {
      console.warn(`[runtime-reload] Failed to restart svc-epsilon-master via '${restart.command}': ${restart.stderr || `exit ${restart.exitCode}`}`)
