...
export function buildOwnerPrompt(project: ProjectRow, task: TaskRow): string {
  return [
    ...
    `- id: ${project.id}`,
    `- path: ${project.path}`,
    `- context: ${project.path}/.epsilon/CONTEXT.md`,
    '',
    'Task:',
    `- id: ${task.id}`,
    `- title: ${task.title}`,
    ...
    `- SECOND: read ${project.path}/.epsilon/CONTEXT.md before making changes. Treat it as the shared project memory spine.`,
    ...
    '- Durable project docs (.epsilon/CONTEXT.md) are maintained automatically by the hidden project-maintainer after each task event; you do not need to update them yourself.',
    '',
    'Completion contract:',
    '- The autowork loop stops when — and only when — you emit a <epsilon_autowork_complete> tag with two children:',
  ...
  if (task.owner_session_id) {
    try {
      const resumePrompt = [
        ...
        'When complete, emit <epsilon_autowork_complete> with <verification> and <requirements_check> children.',
      ].join('\n')
