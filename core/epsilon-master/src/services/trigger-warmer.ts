 ...
 * the epsilon-system plugin (which owns the cron trigger scheduler) loads
 ...
 * hit at 19:31.
 *
 * What we do: read projects from .epsilon/epsilon.db and `GET /session`
...
function listProjectPaths(): string[] {
  const workspace = process.env.EPSILON_WORKSPACE?.trim() || '/workspace'
