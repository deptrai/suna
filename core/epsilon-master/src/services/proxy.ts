  ...
  try {
    ...
    if (response.status >= 500) {
      try {
        ...
        console.error(`[Epsilon Master] OpenCode ${response.status} on ${c.req.method} ${url.pathname}: ${errMsg}`)
      } catch {
        ...
        if (text.includes('__bunfallback')) {
          ...
          if (b64Match) {
            console.error(`[Epsilon Master] OpenCode ${response.status} on ${c.req.method} ${url.pathname}: Bun startup crash (module resolution or compile error — check OpenCode logs)`)
          } else {
            console.error(`[Epsilon Master] OpenCode ${response.status} on ${c.req.method} ${url.pathname}: Bun error page returned (check OpenCode logs)`)
          }
        } else {
          console.error(`[Epsilon Master] OpenCode ${response.status} on ${c.req.method} ${url.pathname}: ${text || '(empty response)'}`)
        }
      }
    }
    ...
    if (error instanceof DOMException && (error.name === 'AbortError' || error.name === 'TimeoutError')) {
      if (!acceptsSSE) {
        ...
        note(
          ...
          `[Epsilon Master] OpenCode timeout on ${c.req.method} ${url.pathname} after ${FETCH_TIMEOUT_MS / 1000}s`,
        )
    ...
    if (isConnRefused) {
      note(
        ...
        `[Epsilon Master] OpenCode unreachable on ${c.req.method} ${url.pathname}: ${errMsg} — is OpenCode running on ${config.OPENCODE_HOST}:${config.OPENCODE_PORT}?`,
      )
      ...
      if (recovered) {
        try {
          ...
          note(
            ...
            `[Epsilon Master] OpenCode retry after recovery failed on ${c.req.method} ${url.pathname}: ${retryMsg}`,
          )
      ...
      note(
        ...
        `[Epsilon Master] Proxy error on ${c.req.method} ${url.pathname}: ${errMsg}`,
      )
    }
