...
function credentialKeyPath(): string {
  ...
  const root = process.env.EPSILON_PERSISTENT_ROOT || '/persistent'
...
function workspaceDbPathForKeyCheck(): string {
  const ws = process.env.EPSILON_WORKSPACE?.trim() || '/workspace'
  return join(ws, '.epsilon', 'epsilon.db')
}
 ...
 *
 * Saw this in prod 2026-04-28: 25 credentials on chainlens-on-call-agent
