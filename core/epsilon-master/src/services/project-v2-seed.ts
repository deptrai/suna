 ...
 * native keys (name, description, mode, model) AND epsilon metadata
 * (slug, tool_groups, execution_mode, default_assignee_columns). OpenCode
 * ignores unknown keys; the epsilon plugin reads the epsilon ones from our DB
 * mirror, not the file.
 */
...
export const TEAM_SECTION_START = '<!-- EPSILON:TEAM:START -->'
export const TEAM_SECTION_END = '<!-- EPSILON:TEAM:END -->'
export const MILESTONES_SECTION_START = '<!-- EPSILON:MILESTONES:START -->'
export const MILESTONES_SECTION_END = '<!-- EPSILON:MILESTONES:END -->'
...
export function resolveDefaultModel(opts?: { override?: string | null }): string {
  ...
  if (process.env.EPSILON_TOKEN && process.env.EPSILON_API_URL) {
    return 'epsilon/minimax-m27'
  }
  return 'epsilon-yolo/think'
}
...
export interface AgentFileMeta {
  ...
  model?: string | null
  // Epsilon metadata — unknown to opencode, read by our plugin hooks from DB.
  tool_groups?: string[]
 ...
 *
 * The frontmatter mixes opencode-native keys (top half) with epsilon metadata
 * (bottom half). OpenCode reads what it knows, ignores the rest. Our plugin
 * reads the epsilon keys from the DB mirror — the file is opencode's source
...
export function renderAgentFile(meta: AgentFileMeta, body: string): string {
  ...
  // Epsilon bookkeeping — opencode ignores, plugin reads via DB
...
- Need a secret / API key / env var? BEFORE blocking, check these
  ...
  4. \`<project.path>/.epsilon/.env\` (project-local secrets).
...
export async function seedV2Project(
  ...
  if (dashboardTicketId) {
    try {
      ...
      // TWO stores: the DB + .epsilon/triggers.yaml. We must:
...
async function pokeTriggerManagerReload(): Promise<void> {
  const port = process.env.EPSILON_TRIGGER_WEBHOOK_PORT || '8099'
  try {
 ...
 * epsilon-system mention convention that actually triggers a human
...
export async function syncTeamSection(
  ...
  const ctxPath = path.join(project.path, '.epsilon', 'CONTEXT.md')
...
export async function tryReadContext(projectPath: string): Promise<string> {
  const ctxPath = path.join(projectPath, '.epsilon', 'CONTEXT.md')
