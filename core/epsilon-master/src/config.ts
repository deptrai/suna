...
function parsePortMap(): Record<string, string> {
  ...
  try {
    ...
    console.warn('[Epsilon Master] Failed to parse SANDBOX_PORT_MAP:', raw)
    return {}
  }
}

export const config = {
  // Epsilon Master port (main entry point)
  PORT: parseInt(process.env.EPSILON_MASTER_PORT || '8000'),
  ...
  // ─── Epsilon Backend ─────────────────────────────────────────────────────────
  // EPSILON_API_URL: base URL of epsilon-api. Source of truth is the secrets-manager-
  // backed s6 env file when present; process.env/.env are fallbacks for native dev.
  get EPSILON_API_URL() { return getEnv('EPSILON_API_URL') || 'http://localhost:8008' },

  // EPSILON_TOKEN — direction: sandbox → epsilon-api.
  ...
  get EPSILON_TOKEN() { return getEnv('EPSILON_TOKEN') || '' },

  // Feature flag: enable or disable local deployment routes (/epsilon/deploy/*)
  EPSILON_DEPLOYMENTS_ENABLED: process.env.EPSILON_DEPLOYMENTS_ENABLED === 'true',
  ...
  // sandbox advertises features.projectsEnabled=false on /epsilon/health so
  ...
  PROJECTS_ENABLED: process.env.EPSILON_PROJECTS_ENABLED === 'true',

  // Secret storage
  SECRET_FILE_PATH: process.env.SECRET_FILE_PATH || `${process.env.EPSILON_PERSISTENT_ROOT || '/persistent'}/secrets/.secrets.json`,
  SALT_FILE_PATH: process.env.SALT_FILE_PATH || `${process.env.EPSILON_PERSISTENT_ROOT || '/persistent'}/secrets/.salt`,
  ENCRYPTION_KEY_PATH: process.env.ENCRYPTION_KEY_PATH || `${process.env.EPSILON_PERSISTENT_ROOT || '/persistent'}/secrets/.encryption-key`,
  ...
  // This is how epsilon-api (and other external callers) authenticates TO the sandbox.
  ...
  // In normal operation, epsilon-api injects the key as a Docker env var.
  get INTERNAL_SERVICE_KEY(): string {
    const s6EnvDir = process.env.S6_ENV_DIR || '/run/s6/container_environment'
    // Always re-read from s6 env dir first — epsilon-api may have written it
    ...
    const tokenAlias = getEnv('EPSILON_TOKEN') || process.env.EPSILON_TOKEN || ''
    if (tokenAlias) {
    ...
    if (!process.env.INTERNAL_SERVICE_KEY) {
      console.warn(
        '[Epsilon Master] WARNING: No INTERNAL_SERVICE_KEY or EPSILON_TOKEN available.\n' +
