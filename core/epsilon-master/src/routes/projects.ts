/**
 * Epsilon Projects API
 *
 * Reads/writes from the shared .epsilon/epsilon.db (same DB the orchestrator plugin uses).
 * This is the frontend's source of truth for project data — NOT the OpenCode SDK.
 *
 * Mounted at /epsilon/projects in epsilon-master.
 */
...
function getDb(): Database {
  ...
  const workspace = process.env.EPSILON_WORKSPACE?.trim()
...
projectsRouter.post('/:id/pm-session', async (c) => {
  ...
  try {
    ...
    // Bind session ↔ PM in the epsilon DB so ticketToolGateHook resolves
...
const SMALL_MODELS = new Set<string>(['epsilon-yolo/think', 'epsilon-yolo/code'])
...
function buildOnboardingPrompt(name: string, handle: string, description: string, seededModel: string): string {
  ...
  return [
    ...
    'pick verbatim.',
    isSmallSeed
      ? `\n**MODEL ADVISORY** — the seeded model \`${seededModel}\` is a small/cheap tier and tends to misfire on real engineering work (dropped tool calls, patchy diff edits, weak QA evidence). Inside Q2 (stack), include ONE short sentence flagging this and offer a larger option (e.g. \`epsilon/minimax-m27\`, \`anthropic/claude-sonnet-4-6\` if the human's API key is loaded). If they confirm the small model anyway, ship it — don't keep asking.`
      : null,
    '',
