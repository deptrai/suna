...
/**
 * Derive the v1 API base URL from EPSILON_API_URL.
 * EPSILON_API_URL may contain a path suffix (e.g. "/v1/router") used by other
 * subsystems. We only need the origin (scheme + host + port) for direct API calls.
 */
function getApiV1(): string {
  try {
    return new URL(config.EPSILON_API_URL).origin + '/v1'
  } catch {
    // Fallback: strip any path and append /v1
    return config.EPSILON_API_URL.replace(/\/+$/, '').replace(/\/v1\/.*$/, '') + '/v1'
  }
}

/**
 * Build X-Pipedream-* headers from sandbox env vars.
 * These let the sandbox send its own Pipedream credentials to epsilon-api,
...
/**
 * Guard: ensure EPSILON_TOKEN is available before proxying to epsilon-api.
...
pipedreamRouter.use('*', async (c, next) => {
  if (!config.EPSILON_TOKEN) {
    console.error('[Pipedream] EPSILON_TOKEN is not set — cannot authenticate to epsilon-api.')
    return c.json({
      error: 'Pipedream unavailable: EPSILON_TOKEN is not configured. Restart the sandbox.',
    }, 503)
  }

  // Local PIPEDREAM_* env vars are optional overrides.
  // If present, getPipedreamHeaders() sends them as X-Pipedream-* headers.
  // If absent, epsilon-api uses its own defaults (env or per-account DB creds).
  await next()
})

/**
 * POST /api/pipedream/token
 * Proxy to epsilon-api: POST /v1/pipedream/token
...
pipedreamRouter.post('/token',
  describeRoute({
    ...
    description: 'Proxies to epsilon-api to fetch an OAuth token for a connected third-party integration. Used by agent tools.',
    responses: {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/pipedream/token`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${config.EPSILON_TOKEN}`,
          ...getPipedreamHeaders(),
        },
...
pipedreamRouter.get('/credentials', async (c) => {
  try {
    const apiUrl = getApiV1()
    const res = await fetch(`${apiUrl}/pipedream/credentials`, {
      headers: { Authorization: `Bearer ${config.EPSILON_TOKEN}` },
      signal: AbortSignal.timeout(15_000),
    })
    const data = await res.json()
    if (!res.ok) return c.json(data, res.status as any)
...
export async function pushPipedreamCredsToApi(): Promise<void> {
  ...
  if (!config.EPSILON_TOKEN || !config.EPSILON_API_URL) {
    console.log('[Pipedream] No EPSILON_TOKEN/API_URL — skipping cred push')
    return
  }
...
async function upsertPipedreamConnector(app: string, appName?: string): Promise<string> {
  ...
  const root = process.env.EPSILON_WORKSPACE || '/workspace'
  mkdirSync(`${root}/.epsilon`, { recursive: true })
  const db = new Database(`${root}/.epsilon/epsilon.db`)
...
// Pipedream integrations from epsilon-api and upserts each into epsilon.db.

export async function syncExistingConnectorsFromApi(): Promise<void> {
  if (!config.EPSILON_TOKEN || !config.EPSILON_API_URL) {
    console.log('[Pipedream] Connector boot-sync: no EPSILON_TOKEN/API_URL — skipping')
    return
  }
  try {
    const apiUrl = getApiV1()
