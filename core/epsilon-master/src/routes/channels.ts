 ...
 *
 * GET    /epsilon/channels              — list all channels
 * POST   /epsilon/channels/verify-telegram  — verify a Telegram bot token
 * POST   /epsilon/channels/setup/telegram   — full Telegram bot setup
 * POST   /epsilon/channels/setup/slack      — full Slack bot setup
 * POST   /epsilon/channels/slack-manifest   — generate Slack app manifest
 * GET    /epsilon/channels/:id          — get channel details
 * POST   /epsilon/channels/:id/enable   — enable channel
 * POST   /epsilon/channels/:id/disable  — disable channel
 * DELETE /epsilon/channels/:id          — remove channel
 * PATCH  /epsilon/channels/:id          — update settings
 */
...
function buildSlackManifest(displayName: string, webhookUrl: string) {
  return {
    display_information: {
      name: displayName,
      description: 'Epsilon AI instance',
...
channelsRouter.post('/slack-manifest', async (c) => {
  try {
    ...
    const displayName = botName?.trim() || 'Epsilon Slack'
