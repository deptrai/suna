...
const EPSILON_MARKETPLACE_NAMESPACE = 'epsilon'
const EPSILON_MARKETPLACE_REGISTRY_URL = 'https://epsilon-registry-6om.pages.dev'

function getWorkspacePaths() {
  const workspaceRoot = process.env.EPSILON_WORKSPACE || '/workspace'
  const opencodeDir = path.join(workspaceRoot, '.opencode')
  return {
    workspaceRoot,
    opencodeDir,
    legacyOpencodeDir: path.join(workspaceRoot, '.epsilon', '.opencode'),
...
async function ensureEpsilonRegistry(): Promise<void> {
  const { ocxConfigPath } = getWorkspacePaths()
  const configText = await readFile(ocxConfigPath, 'utf8').catch(() => '')
  const hasEpsilonRegistry =
    configText.includes('"epsilon"') && configText.includes(EPSILON_MARKETPLACE_REGISTRY_URL)

  if (!hasEpsilonRegistry) {
    await runOcx([
      'registry',
      'add',
      EPSILON_MARKETPLACE_REGISTRY_URL,
      '--name',
      EPSILON_MARKETPLACE_NAMESPACE,
    ])
  }
}
...
async function listInstalledMarketplaceComponents(): Promise<string[]> {
  ...
  return components
    .filter((component) => component.name.startsWith(`${EPSILON_MARKETPLACE_NAMESPACE}/`))
    .map((component) => component.name.replace(/^epsilon\//, ''))
...
marketplaceRouter.get('/status', async (c) => {
  try {
    ...
    const registryConfigured =
      ocxConfigText.includes('"epsilon"') && ocxConfigText.includes(EPSILON_MARKETPLACE_REGISTRY_URL)
    ...
    return c.json({
      ...
      status: {
        ...
        ocxConfigPath,
        opencodeConfigPath,
        registryUrl: EPSILON_MARKETPLACE_REGISTRY_URL,
        registryConfigured,
        installedComponents,
      },
    })
...
marketplaceRouter.post('/install', async (c) => {
  try {
    ...
    const namespace = body.namespace || EPSILON_MARKETPLACE_NAMESPACE
