...
// POST /env/rotate-token — rotate EPSILON_TOKEN.
// Encryption is decoupled from EPSILON_TOKEN, so this just updates the token
// value and restarts services. No re-encryption needed.
envRouter.post('/rotate-token',
  describeRoute({
    tags: ['Secrets'],
    summary: 'Rotate EPSILON_TOKEN',
    description: 'Rotates the EPSILON_TOKEN used for sandbox↔API authentication. Encryption is decoupled — secrets are NOT re-encrypted. Always restarts services.',
    responses: {
  ...
  async (c) => {
    try {
      ...
      console.log(`[ENV API] EPSILON_TOKEN rotated. ${result.rotated} secret(s) unaffected (encryption decoupled).`)
