 ...
 * Replaces both the old /epsilon/triggers (read-only list) and /epsilon/cron/* routes.
 *
...
function getWorkspaceRoot(): string {
  return process.env.WORKSPACE_DIR || process.env.EPSILON_WORKSPACE || '/workspace'
}

function getStore(): TriggerStore {
  if (!store) {
    store = new TriggerStore(join(getWorkspaceRoot(), '.epsilon', 'epsilon.db'))
 ...
 * runtime (cron jobs + webhook routes). epsilon-master's HTTP CRUD path
...
triggersRouter.post('/',
  ...
  async (c) => {
    ...
    // Handle legacy flat format (from old /epsilon/cron API)
