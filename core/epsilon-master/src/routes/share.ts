/**
 * Public URL sharing — /epsilon/share/:port
 *
 ...
 * /s/{token}/* on epsilon-master, which validates the token before proxying.
 *
 * Defaults:
 ...
 *
 * Also supports:
 *   GET  /epsilon/share            — list all active shares
 *   DELETE /epsilon/share/:token   — revoke a specific share
 *
...
/**
 * Build the public base URL that reaches port 8000 (epsilon-master) from outside.
...
export function getMasterPublicBaseUrl(): string {
  ...
  const proxyDomain = process.env.JUSTAVPS_PROXY_DOMAIN || 'epsilon.cloud'
  ...
  // Cloud without JustAVPS: via epsilon-api proxy
  const sandboxId = process.env.SANDBOX_ID || ''
  const epsilonApiUrl = (process.env.EPSILON_API_URL || '').replace(/\/v1\/router\/?$/, '')
  if (envMode === 'cloud' && sandboxId && epsilonApiUrl) {
    return `${epsilonApiUrl}/v1/p/${sandboxId}/8000`
  }
...
// GET /epsilon/share/:port — create a new share
...
// DELETE /epsilon/share/:token — revoke a share
