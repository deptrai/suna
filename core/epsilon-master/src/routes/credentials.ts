 ...
 *
 * Routes are mounted under /epsilon/projects/:projectId/credentials. List
 ...
 *
 * Actor attribution comes from X-Epsilon-Actor-Type + X-Epsilon-Actor-Id
...
function getDb(): Database {
  const workspace = process.env.WORKSPACE_DIR || process.env.EPSILON_WORKSPACE || '/workspace'
  const dbPath = join(workspace, '.epsilon', 'epsilon.db')
  if (!existsSync(dbPath)) throw new Error('epsilon.db not found')
...
function actorFromHeaders(c: { req: { header: (k: string) => string | undefined } }): { type: ActorType; id: string | null } {
  const rawType = c.req.header('x-epsilon-actor-type') ?? c.req.header('X-Epsilon-Actor-Type')
  const rawId = c.req.header('x-epsilon-actor-id') ?? c.req.header('X-Epsilon-Actor-Id')
...
// GET /epsilon/projects/:projectId/credentials — list (no values)
...
// POST /epsilon/projects/:projectId/credentials — upsert {name, value, description?}
...
// GET /epsilon/projects/:projectId/credentials/:name — reveal decrypted value
...
// DELETE /epsilon/projects/:projectId/credentials/:name
...
// GET /epsilon/projects/:projectId/credentials/:name/events — per-credential audit
...
// HEAD /epsilon/projects/:projectId/credentials/:name — existence check without reveal
