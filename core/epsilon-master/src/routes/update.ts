...
const EPSILON_DATA_DIR = '/workspace/.epsilon';
const UPDATE_STATUS_FILE = EPSILON_DATA_DIR + '/update-status.json';
...
// POST /epsilon/update — Trigger a Docker image-based update
// The actual image pull + container recreate is handled by the epsilon-api
// (local-docker provider). This endpoint just signals readiness and version info.
updateRoutes.post(
  '/',
  describeRoute({
    tags: ['update'],
    description: 'Signal sandbox update. Actual image pull + recreate is handled by epsilon-api.',
    responses: {
  ...
  async (c) => {
    ...
    await setStatus({
      ...
      message: `Update to ${targetVersion} pending — container will be recreated by epsilon-api`,
      targetVersion,
    ...
    return c.json({
      success: true,
      message: `Update to ${targetVersion} acknowledged. Container recreate will be triggered by epsilon-api.`,
      currentVersion,
      targetVersion,
      action: 'recreate',
    });
...
// GET /epsilon/update/status — Check update status
updateRoutes.get(
  '/status',
  describeRoute({
    tags: ['update'],
...
// GET /epsilon/update/version — Get current version info
updateRoutes.get(
  '/version',
  describeRoute({
    tags: ['update'],
