 ...
 * webhooks are handled directly by epsilon-master.
 */
...
async function fetchAvailableModels(): Promise<string[]> {
  try {
    const res = await fetch('http://localhost:8000/epsilon/preferences/models', {
...
async function handleChannelCommand(
  ...
  switch (command) {
    ...
    case 'info': {
      ...
      return {
        ...
        text: [
          ...
          `Agent: ${channel.default_agent || 'epsilon'}`,
    ...
    case 'agent': {
      if (!arg) {
        return { handled: true, text: `Current agent: ${channel.default_agent || 'epsilon'}\nUsage: ${p}agent <name>` }
      }
...
async function resolveChannelDirectory(channel: ChannelConfig): Promise<string | undefined> {
  ...
  try {
    ...
    const dbPath = path.join(process.env.EPSILON_WORKSPACE || '/workspace', '.epsilon', 'epsilon.db')
  ...
  const modelOverride = channel.default_model
    ? (() => {
        ...
        return parts.length >= 2
          ...
          : { providerID: 'epsilon', modelID: channel.default_model }
      })()
    : undefined
