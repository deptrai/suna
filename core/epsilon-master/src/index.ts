...
import { epsilonUserContextMiddleware } from './services/epsilon-user-middleware'
...
process.on('uncaughtException', (err) => {
  console.error('[Epsilon Master] UNCAUGHT EXCEPTION:', err)
})
process.on('unhandledRejection', (reason) => {
  console.error('[Epsilon Master] UNHANDLED REJECTION:', reason)
})
...
// to ensure they're always available regardless of how the process was started.
{
  ...
  const CORE_VARS = ['EPSILON_TOKEN', 'EPSILON_YOLO_API_KEY', 'EPSILON_YOLO_URL', 'EPSILON_API_URL', 'INTERNAL_SERVICE_KEY', 'TUNNEL_TOKEN'] as const
  let synced = 0
  for (const key of CORE_VARS) {
    // Use injected env var, but fall back to a sane default for EPSILON_API_URL
    // so OpenCode's {env:EPSILON_API_URL} substitution always produces a valid URL.
    let val: string | undefined = process.env[key]
    if (!val && key === 'EPSILON_API_URL') {
      val = 'http://localhost:8008'
    }
    if (!val && key === 'EPSILON_YOLO_API_KEY') {
      val = process.env.EPSILON_TOKEN
    }
    ...
    if (val) {
      try {
        ...
        console.warn(`[Epsilon Master] Failed to write ${key} to s6 env dir:`, err)
      }
    }
  }
  if (synced > 0) {
...
if (!authSyncDisabled) {
  await syncAuthToSecrets(secretStore).catch(err =>
    console.error('[Epsilon Master] auth-sync boot error:', err)
  )
...
// Project paradigm overlay: when EPSILON_PROJECTS_ENABLED=false (default) we
...
// epsilon-master start so flipping the flag at runtime takes effect on the
...
function syncAgentOverlayForProjectFlag() {
  ...
  try {
    ...
    console.log(
      `[Epsilon Master] Agent overlay written: PROJECTS_ENABLED=${config.PROJECTS_ENABLED}; ` +
      `disabled-when-off: ${projectOnlyAgents.join(', ')}`,
    )
  } catch (err) {
    console.warn('[Epsilon Master] Failed to write agent overlay:', err)
  }
}
syncAgentOverlayForProjectFlag()

if (process.env.EPSILON_DISABLE_CORE_SUPERVISOR !== 'true') {
  void serviceManager.start().catch(err =>
    console.error('[Epsilon Master] service manager start error:', err)
  )
}
...
void warmTriggerPluginForAllProjects().catch((err) =>
  console.warn('[Epsilon Master] trigger warmup failed:', err),
)
...
const defaultCorsOrigins = [
  ...
  'http://localhost:8008', 'http://127.0.0.1:8008',   // epsilon-api (local)
...
// Parses the signed `X-Epsilon-User-Context` header the Epsilon API preview
// proxy attaches to every authenticated user request. Attaches the parsed
// context to `c.get('epsilonUser')` for downstream route handlers. Purely
...
app.use('*', epsilonUserContextMiddleware())
...
// without needing INTERNAL_SERVICE_KEY. External callers (epsilon-api, frontend
...
app.use('*', async (c, next) => {
  ...
  if (pathname === '/epsilon/health') return next()
  ...
  // authenticate via the X-Epsilon-Trigger-Secret header (per-trigger secret).
...
function inspectOpencodeState() {
  ...
  const persistentRoot = process.env.EPSILON_PERSISTENT_ROOT || null
  ...
  return {
    ...
    opencodeStateGuardAvailable: existsSync('/usr/local/bin/epsilon-opencode-state'),
...
// OpenAPI JSON spec endpoint — merges epsilon-master + OpenCode specs at runtime
app.get('/docs/openapi.json',
  describeRoute({ hide: true, responses: { 200: { description: 'OpenAPI spec' } } }),
  async (c) => {
    // Generate epsilon-master's own spec
    const epsilonSpec = await generateSpecs(app)
    // Merge with OpenCode's spec (fetched from localhost, cached 30s)
    const merged = await buildMergedSpec(epsilonSpec as any)
    ...
    // public base URL (e.g. "http://localhost:8008/v1/p/epsilon-sandbox/8000").
...
if (config.EPSILON_DEPLOYMENTS_ENABLED) {
  app.route('/epsilon/deploy', deployRouter)
}

// Services — unified "what's running" for the frontend
app.route('/epsilon/services', servicesRouter)

// Triggers — unified CRUD for all trigger types (cron, webhook, etc.)
app.route('/epsilon/triggers', triggersRouter)
// Legacy compat: /epsilon/cron/* → forwards to /epsilon/triggers/*
app.route('/epsilon/cron', triggersRouter)

// Channels — SQLite-backed channel management (Telegram, Slack)
import { channelsRouter } from './routes/channels'
app.route('/epsilon/channels', channelsRouter)

// Core supervisor management
app.route('/epsilon/core', coreRouter)

// Full reload — restarts OpenCode instance and optionally all services
app.route('/epsilon/reload', reloadRouter)

// Marketplace — skill/component install from registry
app.route('/epsilon/marketplace', marketplaceRouter)

// Preferences — default model management
app.route('/epsilon/preferences', preferencesRouter)

// Projects + Tasks — Epsilon project management. Gated behind PROJECTS_ENABLED
// so the multi-project paradigm is fully off by default. When the flag is off
// every /epsilon/projects/*, /epsilon/tickets/*, and /epsilon/tasks/* request
...
if (config.PROJECTS_ENABLED) {
  ...
  app.route('/epsilon/projects', projectsRouter)
  app.route('/epsilon/projects/', projectsRouter)
  app.route('/epsilon/tasks', tasksRouter)
  app.route('/epsilon/tasks/', tasksRouter)

  // v2 board — tickets, columns, fields, templates, project_agents
  // ticketProjectsRouter layers project-scoped config under /epsilon/projects/:id/*
  // so it's mounted at /epsilon/projects after the base projects router (last writer wins
  // for path-specific handlers; base projects router has no sub-paths like /columns).
  app.route('/epsilon/tickets', ticketsRouter)
  app.route('/epsilon/tickets/', ticketsRouter)
  app.route('/epsilon/projects', ticketProjectsRouter)
  app.route('/epsilon/projects/', ticketProjectsRouter)

  // Milestones — outcome-level grouping under /epsilon/projects/:projectId/milestones
  app.route('/epsilon/projects', milestonesRouter)
  app.route('/epsilon/projects/', milestonesRouter)

  // Project-scoped credentials under /epsilon/projects/:projectId/credentials
  app.route('/epsilon/projects', credentialsRouter)
  app.route('/epsilon/projects/', credentialsRouter)
} else {
  const projectsDisabledHandler = (c: any) => c.json({
    error: 'projects_disabled',
    message: 'The multi-project paradigm is disabled on this sandbox. Set EPSILON_PROJECTS_ENABLED=true to enable.',
  }, 503)
  app.all('/epsilon/projects/*', projectsDisabledHandler)
  app.all('/epsilon/projects', projectsDisabledHandler)
  app.all('/epsilon/tickets/*', projectsDisabledHandler)
  app.all('/epsilon/tickets', projectsDisabledHandler)
  app.all('/epsilon/tasks/*', projectsDisabledHandler)
  app.all('/epsilon/tasks', projectsDisabledHandler)
  console.log('[epsilon-master] PROJECTS_ENABLED=false — /epsilon/{projects,tickets,tasks}/* return 503')
}

// Public URL sharing — /epsilon/share/:port returns the public URL for a sandbox port
app.route('/epsilon/share', shareRouter)

import { Database as _ScopeDb } from 'bun:sqlite'
import { getUserScopes } from './services/user-scope-cache'

app.get('/epsilon/internal/session-scopes/:sessionId', async (c) => {
  const sessionId = c.req.param('sessionId')
  try {
    const workspace =
      process.env.EPSILON_WORKSPACE?.trim() ||
      process.env.OPENCODE_CONFIG_DIR?.replace(/\/opencode\/?$/, '') ||
      '/workspace'
    const dbPath = `${workspace}/.epsilon/epsilon.db`
...
app.route('/epsilon/connectors', connectorsRouter)

// Pipedream integration proxy — forwards to epsilon-api
app.route('/api/pipedream', pipedreamRouter)

// [channels v2] Channel webhooks — handles /hooks/telegram/<id> and /hooks/slack/<id>
// directly in epsilon-master. These are looked up in the channels SQLite DB,
...
// Auth is skipped (see auth middleware) — per-trigger secret via X-Epsilon-Trigger-Secret header.
app.all('/hooks/*', async (c) => {
  ...
  try {
    ...
    const secret = c.req.header('x-epsilon-trigger-secret') ?? c.req.header('x-epsilon-opencode-trigger-secret')
    if (secret) headers['x-epsilon-trigger-secret'] = secret
    ...
    console.error(`[Epsilon Master] Webhook proxy error for ${pathname}:`, err)
...
app.post('/events/pipedream/:listenerId', async (c) => {
  ...
  try {
    ...
    console.error(`[Epsilon Master] Pipedream event forward error for ${listenerId}:`, err)
...
console.log(`[Epsilon Master] Starting on port ${config.PORT}`)

// Push Pipedream creds to API after boot (async, non-blocking)
setTimeout(() => pushPipedreamCredsToApi(), 5_000)
// Reconcile Pipedream connectors from API into local epsilon.db (self-heals
// sandboxes that missed the fire-and-forget connector-sync call).
setTimeout(() => syncExistingConnectorsFromApi(), 8_000)
console.log(`[Epsilon Master] Proxying to OpenCode at ${config.OPENCODE_HOST}:${config.OPENCODE_PORT}`)
console.log(`[Epsilon Master] API docs available at http://localhost:${config.PORT}/docs`)
...
function resetIdleTimer(ws: { data: WsProxyData; close: (code?: number, reason?: string) => void }) {
  ...
  ws.data.idleTimer = setTimeout(() => {
    console.warn(`[Epsilon Master] WS idle timeout for port ${ws.data.targetPort}`)
    ...
    open(ws: { data: WsProxyData; send: (data: any) => void; close: (code?: number, reason?: string) => void }) {
      ...
      ws.data.connectTimer = setTimeout(() => {
        if (ws.data.upstream?.readyState === WebSocket.CONNECTING) {
          console.warn(`[Epsilon Master] WS upstream connect timeout for port ${targetPort}`)
      ...
      try {
        ...
        upstream.addEventListener('error', (e: Event) => {
          ...
          console.warn(`[Epsilon Master] WS upstream error for port ${targetPort} (path: ${targetPath})`)
          if (!ws.data.closed) {
            try { ws.close(1011, 'upstream error') } catch { /* already closed */ }
          }
        })
      } catch (err) {
        console.error(`[Epsilon Master] WS proxy failed to connect to port ${targetPort}:`, err)
    ...
    message(ws: { data: WsProxyData; close: (code?: number, reason?: string) => void }, message: string | Buffer) {
      ...
      if (upstream && upstream.readyState === WebSocket.OPEN) {
        ...
        if (ws.data.bufferBytes + size > WS_BUFFER_MAX_BYTES) {
          console.warn(`[Epsilon Master] WS buffer overflow for port ${ws.data.targetPort}, closing`)
