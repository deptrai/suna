...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "manifest": {
      ...
      const manifest = {
        display_information: { name: flags.name || `Epsilon ${["Atlas","Nova","Sage","Echo","Bolt","Iris","Dash","Cleo","Finn","Luna","Juno","Axel","Niko","Zara","Milo","Ruby","Hugo","Aria","Leo","Ivy"][Math.floor(Math.random()*20)]}`, description: "Epsilon AI instance", background_color: "#1a1a2e" },
        features: { bot_user: { display_name: flags.name || "Epsilon", always_online: true } },
