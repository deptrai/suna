#!/usr/bin/env bun
/**
 * cli-lib.ts — Shared utilities for all Epsilon CLI tools.
 *
 * Provides:
...
export function getEnv(key: string): string | undefined {
  ...
  const workspace = process.env.EPSILON_WORKSPACE || process.cwd()
  try {
...
export function masterUrl(): string {
  return (getEnv("EPSILON_MASTER_URL") || "http://localhost:8000").trim()
}
