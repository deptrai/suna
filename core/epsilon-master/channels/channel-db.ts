 ...
 * Lives in .epsilon/epsilon.db alongside connectors, triggers, etc.
 *
...
function resolveDbPath(): string {
  const root = process.env.EPSILON_WORKSPACE?.trim()
  ...
  for (const candidate of [
    path.join(root, ".epsilon", "epsilon.db"),
    "/workspace/.epsilon/epsilon.db",
  ]) {
    const dir = path.dirname(candidate)
    if (existsSync(dir)) return candidate
  }
  const dbDir = path.join(root, ".epsilon")
  mkdirSync(dbDir, { recursive: true })
  return path.join(dbDir, "epsilon.db")
}
...
export function generateChannelName(createdBy?: string): string {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const name = `Epsilon ${adj}${suffix}`
    if (!usedNames.has(name)) return name
  }
  // Fallback with random number
  return `Epsilon ${Date.now().toString(36)}${createdBy ? ` (by ${createdBy})` : ""}`
}
...
export function createChannel(opts: {
  ...
  `).run(
    ...
    opts.default_agent || "epsilon", opts.default_model || "", opts.bridge_instructions || null,
