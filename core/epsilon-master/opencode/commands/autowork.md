...
2. In **planning phase**, it waits for a well-formed `<epsilon_autowork_plan>` tag that defines status quo, target end state, ambiguity resolution, work plan, and verification gates.
...
7. In execution phase, the plugin checks for a well-formed `<epsilon_autowork_complete>` tag.
...
10. In verifier phase, the plugin waits for a well-formed `<epsilon_autowork_verified>` tag with a final rerun of the verification commands and a final audit checklist.
...
- Execution does not begin until a valid `<epsilon_autowork_plan>` is accepted.
...
- Clean completion does not happen until a valid `<epsilon_autowork_verified>` is accepted.
