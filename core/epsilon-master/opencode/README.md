...
Epsilon OpenCode config directory — agents, tools, skills, commands, and plugins.

This directory lives inside `core/epsilon-master/opencode/` and is the OpenCode config dir. In the container it's at `/ephemeral/epsilon-master/opencode/` (set via `OPENCODE_CONFIG_DIR`).

## Structure

```
opencode.jsonc          <- OpenCode config (plugins, model, MCP servers)
package.json            <- minimal (deps managed by parent epsilon-master/package.json)
...
skills/                 <- built-in skills (natively discovered)
  plugin/                 <- plugins loaded individually via opencode.jsonc
    epsilon-system/        <- unified Epsilon plugin (projects, tasks, sessions, connectors, autowork, triggers, worktree)
...
```bash
ln -s computer/core/epsilon-master/opencode .opencode
```
