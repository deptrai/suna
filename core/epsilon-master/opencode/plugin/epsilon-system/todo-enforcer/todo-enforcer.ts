/**
 * Epsilon Todo Enforcer — native todo continuation.
 *
...
import { wrapInEpsilonSystemTags } from "../lib/message-transform"
...
function wrapSystemPrompt(text: string): string {
	return wrapInEpsilonSystemTags(text, { type: "passive-continuation", source: "epsilon-native-todo-enforcing" })
}
...
function isInternalMessage(text: string): boolean {
	...
	if (text.includes("<epsilon_system")) return true
	return false
}
...
const TodoEnforcerPlugin: Plugin = async ({ client }) => {
	...
	const log = (level: "info" | "warn" | "error", message: string) => {
		try {
			client.app.log({ body: { service: "epsilon-todo-enforcing", level, message } }).catch(() => {})
		} catch {
	...
	return {
		"chat.message": async (input: any, output: any) => {
			try {
				...
				if (messageText.includes("EPSILON_AUTOWORK")) autoworkActiveSessions.add(sessionId)
				if (messageText.includes("EPSILON_AUTOWORK_CANCEL")) {
