 ...
 * `<epsilon_autowork_plan>` before execution begins. Later, completion is only
 * accepted when the latest `<epsilon_autowork_complete>` is backed by real,
 ...
 * `<epsilon_autowork_request>` so the worker cannot drift across long loops.
 */
 ...
 * The wrapper tag `<epsilon_autowork_system>` surrounds every injected prompt,
 * and `<epsilon_autowork_complete>` is the worker's completion signal — both
...
const AutoworkPlugin: Plugin = async ({ client }) => {
	...
	const log = (level: "info" | "warn" | "error", message: string) => {
		try {
			client.app.log({ body: { service: "epsilon-autowork", level, message } }).catch(() => {})
		} catch {
			// ignore
		}
	}
