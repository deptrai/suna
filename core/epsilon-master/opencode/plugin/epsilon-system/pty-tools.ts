...
function buildPtyOutput(
  ...
  return `<epsilon_system type="pty-output" source="opencode-pty">\n${inner}\n</epsilon_system>`
}
    ...
    tool: {
      pty_spawn: tool({
        ...
        async execute(args, ctx) {
          ...
          return `<epsilon_system type="pty-spawn" source="opencode-pty">\n<pty_spawned>\n${inner}\n</pty_spawned>\n</epsilon_system>`
        },
      }),
      ...
      pty_write: tool({
        ...
        async execute(args) {
          ...
          const displayPreview = preview
            ...
            .replace(/\n/g, '\\n')
            .replace(/\r/g, '\\r')
          return `<epsilon_system type="pty-write" source="opencode-pty">\nSent ${args.data.length} bytes to ${args.id}: "${displayPreview}"\n</epsilon_system>`
        },
      }),
      ...
      pty_list: tool({
        ...
        async execute() {
          ...
          if (sessions.length === 0) {
            return `<epsilon_system type="pty-list" source="opencode-pty">\n<pty_list>\nNo active PTY sessions.\n</pty_list>\n</epsilon_system>`
          }
      ...
      pty_kill: tool({
        ...
        async execute(args) {
          ...
          return `<epsilon_system type="pty-kill" source="opencode-pty">\n${inner}\n</epsilon_system>`
        },
      }),
    },
    event: async ({ event }) => {
