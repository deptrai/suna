/**
 * Epsilon Projects — project CRUD + session-project linking + file gating.
 *
 * SQLite (epsilon.db) is the single source of truth.
...
const TASK_SUMMARY_START = "<!-- EPSILON:TASK-SUMMARY:START -->"
const TASK_SUMMARY_END = "<!-- EPSILON:TASK-SUMMARY:END -->"

function projectContextPath(projectPath: string): string {
	return path.join(projectPath, ".epsilon", "CONTEXT.md")
}
...
export class ProjectManager {
	...
	async createProject(name: string, desc: string, customPath: string): Promise<ProjectRow> {
		...
		await fs.mkdir(path.join(pp, ".epsilon"), { recursive: true })
		ensureGlobalMemoryFiles(import.meta.dir)
		await wm(path.join(pp, ".epsilon", "CONTEXT.md"), `# ${name}\n\n${desc || "No description."}\n`)
	...
	/**
	 * Single-project paradigm: a sandbox owns exactly one Epsilon project,
	...
	return {
		...
		project_get: tool({
			...
			async execute(args: { name: string }): Promise<string> {
				...
				const contextPath = path.join(p.path, ".epsilon", "CONTEXT.md")
				return [
					`## ${p.name}`, ``, `**Path:** \`${p.path}\``,
		...
		try {
			...
			for (let i = messages.length - 1; i >= 0; i--) {
				if (messages[i]?.info?.role === "user") {
					...
					messages[i].parts.push({ type: "text", text: `<epsilon_system type="project-status" source="epsilon-system">${statusXml}</epsilon_system>` })
					break
				}
			}
		} catch {}
