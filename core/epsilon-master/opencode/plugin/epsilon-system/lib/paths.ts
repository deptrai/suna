...
export function resolveEpsilonWorkspaceRoot(anchorDir?: string): string {
	const explicitWorkspace = process.env.EPSILON_WORKSPACE?.trim()
...
export function resolveEpsilonDir(anchorDir?: string): string {
	const explicitDir = process.env.EPSILON_DIR?.trim()
	if (explicitDir) return normalizeAbsolutePath(explicitDir)
	return join(resolveEpsilonWorkspaceRoot(anchorDir), ".epsilon")
}

export function ensureEpsilonDir(anchorDir?: string): string {
	const epsilonDir = resolveEpsilonDir(anchorDir)
...
/**
 * Ensure global USER.md + MEMORY.md exist at the workspace root .epsilon/ directory.
 * Deeper notes can live in .epsilon/memory/*.md and be referenced from MEMORY.md.
 */
export function ensureGlobalMemoryFiles(anchorDir?: string): { userPath: string; memoryPath: string; memoryDir: string } {
	const epsilonDir = ensureEpsilonDir(anchorDir)
	const memoryDir = join(epsilonDir, "memory")
	mkdirSync(memoryDir, { recursive: true })
	return {
		userPath: ensureFile(join(epsilonDir, "USER.md"), [
			"# User Profile",
			"",
			"## Preferences",
			"",
		...
		memoryPath: ensureFile(join(epsilonDir, "MEMORY.md"), [
			"# Global Memory",
			"",
			"Keep this file concise. Put deeper notes in `.epsilon/memory/*.md` and reference them here.",
			"",
			"## Environment",
			"",
			"## Stack",
...
export function renderProjectContext(projectPath: string): string {
	const ctxPath = join(projectPath, ".epsilon", "CONTEXT.md")
