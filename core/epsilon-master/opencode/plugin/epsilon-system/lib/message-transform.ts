// ============================================================================
// Epsilon System XML Tags — wrap internal/system content for conditional rendering
// ============================================================================
//
...
//
// Examples:
//   <epsilon_system>AGENTS.md instruction content here</epsilon_system>
//   <epsilon_system reminder="project-gate">Project must be selected first</epsilon_system>
//   <epsilon_system type="instruction" source="AGENTS.md">System prompt...</epsilon_system>

export const MEMORY_CONTEXT_MARKER = "<!-- epsilon-mem-context -->"
export const MEMORY_CONTEXT_MESSAGE_ID = "__epsilon_mem_context__"

export const EPSILON_SYSTEM_OPEN_TAG = "<epsilon_system"
export const EPSILON_SYSTEM_CLOSE_TAG = "</epsilon_system>"

/**
 * Check if text contains any epsilon_system XML tags.
 */
export function containsEpsilonSystemTags(text: string): boolean {
	return text.includes(EPSILON_SYSTEM_OPEN_TAG) && text.includes(EPSILON_SYSTEM_CLOSE_TAG)
}

/**
 * Wrap text content in epsilon_system XML tags with optional attributes.
 * 
 * @param content - The text content to wrap
 * @param attrs - Optional attributes (e.g., { type: "instruction", source: "AGENTS.md" })
 * @returns The wrapped text, e.g., '<epsilon_system type="instruction" source="AGENTS.md">content</epsilon_system>'
 */
export function wrapInEpsilonSystemTags(
	content: string,
	...
	return `${EPSILON_SYSTEM_OPEN_TAG}${attrString}>${content}${EPSILON_SYSTEM_CLOSE_TAG}`
}

/**
 * Strip all epsilon_system XML tags and their content from text.
 * Use this in the frontend to remove internal content before rendering.
 * 
 * @param text - Text containing epsilon_system tags
 * @returns Text with all epsilon_system content removed
 */
export function stripEpsilonSystemTags(text: string): string {
	if (!text) return ""
	
	// Pattern to match <epsilon_system ...>...</epsilon_system> with optional attributes
	// Handles multiline content
	const SYSTEM_TAG_REGEX = /<epsilon_system[^>]*>[\s\S]*?<\/epsilon_system>/gi
	
	return text.replace(SYSTEM_TAG_REGEX, "").trim()
}

/**
 * Extract all epsilon_system content blocks from text without removing them.
 * Useful for debugging or logging what system content was injected.
 * 
 * @param text - Text containing epsilon_system tags
 * @returns Array of content blocks found inside epsilon_system tags
 */
export function extractEpsilonSystemContent(text: string): string[] {
	if (!text) return []
...
/**
 * Wrap multiple lines or content blocks in epsilon_system tags.
 ...
 */
export function wrapLinesInEpsilonSystemTags(
	...
	return wrapInEpsilonSystemTags(content, attrs)
}
