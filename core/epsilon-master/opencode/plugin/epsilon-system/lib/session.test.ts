...
function makeStorage(): { storageBase: string; dbPath: string } {
	const root = mkdtempSync(path.join(tmpdir(), "epsilon-session-test-"))
	tempRoots.push(root)
...
function seedDb(dbPath: string): void {
	...
	db.query(`INSERT INTO message VALUES (?,?,?,?,?)`).run(
		"msg_1", "ses_root", 1100, 1100, JSON.stringify({ role: "user", content: "Please compare Hermes Agent vs Epsilon memory systems." }),
	)
...
describe("session helpers", () => {
	test("searchSessions returns session-level hits with reason and snippet", async () => {
		...
		expect(hits[0]?.snippet).toContain("Hermes Agent vs Epsilon")
	})
