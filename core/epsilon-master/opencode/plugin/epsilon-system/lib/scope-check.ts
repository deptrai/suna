const EPSILON_MASTER_URL = `http://127.0.0.1:${process.env.EPSILON_MASTER_PORT || '8000'}`
const TTL_MS = 30_000
...
async function getScopesForSession(sessionId: string): Promise<Set<string> | null> {
  ...
  try {
    const res = await fetch(
      `${EPSILON_MASTER_URL}/epsilon/internal/session-scopes/${encodeURIComponent(sessionId)}`,
