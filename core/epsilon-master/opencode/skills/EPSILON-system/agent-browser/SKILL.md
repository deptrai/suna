...
## Live Viewer URL (Required in Epsilon)

Epsilon keeps a persistent primary browser session named `epsilon` alive for the live viewer. Reuse that session instead of creating ad hoc sessions or overriding stream ports.

Mandatory launch policy in Epsilon:

1. Reuse the existing `epsilon` session unless the task explicitly requires isolation
...
```bash
agent-browser --session epsilon open <url>
```

Required behavior:

1. Determine the active session name (default to `epsilon` in the Epsilon sandbox)
...
# Then share:
# http://localhost:9224/?session=epsilon
```
