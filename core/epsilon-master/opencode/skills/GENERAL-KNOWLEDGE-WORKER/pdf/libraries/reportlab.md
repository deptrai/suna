...
Always set metadata on every PDF you create. The author MUST be `"Epsilon"` and the title MUST describe the document contents.

- **Canvas**: call `c.setTitle(...)` and `c.setAuthor("Epsilon")` immediately after creating the canvas
- **SimpleDocTemplate**: pass `title=...` and `author="Epsilon"` as constructor kwargs
