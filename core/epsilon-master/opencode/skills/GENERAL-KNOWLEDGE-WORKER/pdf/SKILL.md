...
- **Author** should default to `"Epsilon"` unless the user asks for a different organization or author name
- **Title** should be a descriptive name relevant to the document contents

Canvas API: `c.setTitle(...)`, `c.setAuthor("Epsilon")` right after creating the canvas.
SimpleDocTemplate: pass `title=...`, `author="Epsilon"` as constructor kwargs.
pdf-lib (JS): `doc.setTitle(...)`, `doc.setAuthor("Epsilon")`.
