...
app.post('/epsilon/deploy', async (c) => {
...
app.post('/epsilon/deploy/:id/stop', async (c) => {
...
app.get('/epsilon/deploy/:id/logs', async (c) => {
...
app.get('/epsilon/deploy/:id/status', async (c) => {
...
app.get('/epsilon/deploy', async (c) => {
...
describe('Deploy Routes — compatibility over ServiceManager', () => {
  it('returns 400 when deploymentId is missing', async () => {
    const res = await jsonPost('/epsilon/deploy', { sourceType: 'files' })
  ...
  it('deploys a simple Bun server via API', async () => {
    ...
    const res = await jsonPost('/epsilon/deploy', {
  ...
  it('lists managed deployments', async () => {
    const res = await jsonGet('/epsilon/deploy')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('returns status and logs for a running deployment', async () => {
    const list = await jsonGet('/epsilon/deploy')
    const deployments = (await list.json()).deployments as Array<{ id: string }>
    const id = deployments[0].id

    const statusRes = await jsonGet(`/epsilon/deploy/${id}/status`)
    expect(statusRes.status).toBe(200)
    const statusBody = await statusRes.json()
    expect(statusBody.status).toBe('running')

    await Bun.sleep(200)
    const logsRes = await jsonGet(`/epsilon/deploy/${id}/logs`)
  ...
  it('stops a managed deployment and reports stopped status', async () => {
    ...
    const stopRes = await jsonPost(`/epsilon/deploy/${id}/stop`, {})
