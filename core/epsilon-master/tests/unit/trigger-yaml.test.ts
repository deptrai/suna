...
describe('TriggerYaml', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(tempDir, '.epsilon'), { recursive: true })
    store = new TriggerStore(join(tempDir, '.epsilon', 'test.db'))
  ...
  describe('read', () => {
    ...
    it('parses a valid YAML file', () => {
      writeFileSync(join(tempDir, '.epsilon', 'triggers.yaml'), `
triggers:
  - name: "Daily Report"
    source:
      type: cron
...
`)
      ...
      expect(result.triggers[0].action.agent).toBe('epsilon')
...
`)
    ...
    it('handles malformed YAML gracefully', () => {
      writeFileSync(join(tempDir, '.epsilon', 'triggers.yaml'), '{{{{invalid yaml')
      const result = yamlSync.read()
      expect(result.triggers).toHaveLength(0)
    })
  })
  ...
  describe('write', () => {
    it('writes a YAML file', () => {
      ...
      const content = readFileSync(join(tempDir, '.epsilon', 'triggers.yaml'), 'utf8')
...
`)
    ...
    it('removes DB entries not in YAML', () => {
      ...
      writeFileSync(join(tempDir, '.epsilon', 'triggers.yaml'), `triggers: []`)
