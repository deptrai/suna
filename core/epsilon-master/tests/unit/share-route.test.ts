...
/**
 * Tests for the share route (GET /epsilon/share/:port).
 *
...
const ENV_KEYS = [
  ...
  'SANDBOX_ID', 'EPSILON_API_URL', 'SANDBOX_PORT_MAP', 'SHARE_STORE_PATH',
]
...
describe('GET /epsilon/share/:port (token-based)', () => {
  let shareRouter: any
  let Hono: any
  ...
  it('returns a share with token, url, expiresAt, ttl', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000')
  ...
  it('accepts ?ttl=30m', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=30m')
    const body = await res.json() as any
  ...
  it('accepts ?ttl=2h', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=2h')
    const body = await res.json() as any
  ...
  it('rejects TTL below minimum (5m)', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=1s')
  ...
  it('rejects TTL above maximum (365d)', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=400d')
  ...
  it('rejects invalid TTL format', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=banana')
  ...
  it('accepts ?label=demo', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?label=demo')
    const body = await res.json() as any
  ...
  it('adds hint for TTL > 24h', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=2d')
    const body = await res.json() as any
  ...
  it('no hint for TTL <= 24h', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=1h')
    const body = await res.json() as any
  ...
  it('cloud mode: URL goes through port 8000 CF Worker with __proxy_token', async () => {
    ...
    expect(body.url).toContain('8000--abc123.epsilon.cloud')
  ...
  it('returns 400 for port 0', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/0')
    expect(res.status).toBe(400)
  })

  it('returns 400 for port 99999', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/99999')
    expect(res.status).toBe(400)
  })

  // ── List shares ───────────────────────────────────────────────────────

  it('GET /epsilon/share lists active shares', async () => {
    const app = buildApp()
    await app.request('/epsilon/share/3000')
    await app.request('/epsilon/share/5000')

    const res = await app.request('/epsilon/share')
    const body = await res.json() as any
  ...
  it('DELETE /epsilon/share/:token revokes a share', async () => {
    const app = buildApp()
    const createRes = await app.request('/epsilon/share/3000')
    const { token } = await createRes.json() as any

    const delRes = await app.request(`/epsilon/share/${token}`, { method: 'DELETE' })
    expect(delRes.status).toBe(200)

    // Should no longer appear in list
    const listRes = await app.request('/epsilon/share')
  ...
  it('DELETE unknown token returns 404', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/nonexistent', { method: 'DELETE' })
