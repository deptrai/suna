...
#   B. Full ticket CRUD round-trip via /epsilon/projects/proj-workspace/tickets.
...
#      survive a epsilon-master + opencode kill cycle.
...
#
# Self-contained. Toggles EPSILON_PROJECTS_ENABLED at runtime, restores
# state on exit.
set -u
PASS=0; FAIL=0; total=0; FAILS=()
TOKEN=$(docker exec epsilon-sandbox cat /run/s6/container_environment/INTERNAL_SERVICE_KEY 2>/dev/null)
...
flip() {
  local val="$1"
  docker exec epsilon-sandbox sh -c "echo -n \"$val\" > /run/s6/container_environment/EPSILON_PROJECTS_ENABLED"
  PID=$(docker exec epsilon-sandbox sh -c "ps -ef | grep 'bun run /ephemeral/epsilon-master' | grep -v grep | awk '{print \$2}'" | head -1)
  [ -n "$PID" ] && docker exec epsilon-sandbox kill -TERM "$PID" 2>/dev/null
  for i in $(seq 1 30); do
    sleep 1
    H=$(curl -s --max-time 1 "$BASE/epsilon/health" 2>/dev/null)
    echo "$H" | grep -q "\"projectsEnabled\":$val" && break
  done
  sleep 2
  OPCDPID=$(docker exec epsilon-sandbox sh -c "ps -ef | grep 'opencode-epsilon serve' | grep -v grep | awk '{print \$2}'" | head -1)
  [ -n "$OPCDPID" ] && docker exec epsilon-sandbox kill -TERM "$OPCDPID" 2>/dev/null
  if [ "$val" = "true" ]; then
    for i in $(seq 1 90); do sleep 1; T=$(docker exec epsilon-sandbox curl -s --max-time 2 $INNER/experimental/tool/ids 2>/dev/null); echo "$T" | grep -q "project_get" && return 0; done
  else
    for i in $(seq 1 60); do sleep 1; T=$(docker exec epsilon-sandbox curl -s --max-time 2 $INNER/experimental/tool/ids 2>/dev/null); [ -n "$T" ] && ! echo "$T" | grep -q "project_get" && return 0; done
  fi
}
...
SESS_RESP=$(docker exec epsilon-sandbox curl -s -X POST $INNER/session -H 'Content-Type: application/json' -d '{"title":"e2e-comp-test"}')
...
if [ -n "$SESS_ID" ]; then
  GET_RESP=$(docker exec epsilon-sandbox curl -s "$INNER/session/$SESS_ID")
  ci "session.get returns the same id" "\"$SESS_ID\"" "$GET_RESP"

  LIST_RESP=$(docker exec epsilon-sandbox curl -s "$INNER/session")
  ci "session.list contains the session" "\"$SESS_ID\"" "$LIST_RESP"

  DEL=$(docker exec epsilon-sandbox curl -s -o /dev/null -w '%{http_code}' -X DELETE "$INNER/session/$SESS_ID")
  c "session.delete returns 200" "200" "$DEL"
fi

echo
echo "── B. Full ticket CRUD (flag on) ──"
TICKETS_BASE="$BASE/epsilon/tickets"
...
PROJ_MILESTONES_BASE="$BASE/epsilon/projects/proj-workspace/milestones"
...
CH_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/epsilon/channels" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "$CH_BODY")
total=$((total+1))
...
CH_LIST_CODE=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/epsilon/channels" -H "Authorization: Bearer $TOKEN")
c "channel list 200" "200" "$CH_LIST_CODE"

# Cleanup any leftover e2e-channel-null
docker exec epsilon-sandbox /opt/bun/bin/bun -e '
import { Database } from "bun:sqlite"
const db = new Database("/workspace/.epsilon/epsilon.db")
...
SCHEMA_PROBE=$(docker exec epsilon-sandbox /opt/bun/bin/bun -e '
import { Database } from "bun:sqlite"
const db = new Database("/workspace/.epsilon/epsilon.db", { readonly: true })
...
const sys = [
  fs.readFileSync("/ephemeral/epsilon-master/opencode/epsilon-system.md","utf8"),
  nofm("/ephemeral/epsilon-master/opencode/agents/orchestrator.md"),
].join("\n\n")
const need = ["task_create","task_deliver","verification_condition",".epsilon/CONTEXT.md","Plan → Delegate → Review"]
...
GEN_PERMS=$(docker exec epsilon-sandbox awk '/^---$/{c++; next} c==1' /ephemeral/epsilon-master/opencode/agents/general.md)
...
for url in /epsilon/projects /epsilon/projects/proj-workspace /epsilon/projects/proj-workspace/tickets \
           /epsilon/triggers /epsilon/channels /epsilon/connectors; do
  CODE=$(curl -s -o /dev/null -w '%{http_code}' "$BASE$url")
  c "GET $url (no token) → 401" "401" "$CODE"
done
# Health is unauthenticated by design
HCODE=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/epsilon/health")
c "GET /epsilon/health (no token) → 200" "200" "$HCODE"

echo
echo "── I. Process-restart persistence ──"
# Plant a session_projects row, restart epsilon-master + opencode, verify it survives
...
KMPID=$(docker exec epsilon-sandbox sh -c "ps -ef | grep 'bun run /ephemeral/epsilon-master' | grep -v grep | awk '{print \$2}'" | head -1)
docker exec epsilon-sandbox kill -TERM "$KMPID"
for i in $(seq 1 20); do sleep 1; CODE=$(curl -s --max-time 1 -o /dev/null -w '%{http_code}' "$BASE/epsilon/health" 2>/dev/null); [ "$CODE" = "200" ] && break; done

OPCDPID=$(docker exec epsilon-sandbox sh -c "ps -ef | grep 'opencode-epsilon serve' | grep -v grep | awk '{print \$2}'" | head -1)
docker exec epsilon-sandbox kill -TERM "$OPCDPID"
for i in $(seq 1 60); do sleep 1; T=$(docker exec epsilon-sandbox curl -s --max-time 2 $INNER/experimental/tool/ids 2>/dev/null); echo "$T" | grep -q "project_get" && break; done

CHECK=$(docker exec epsilon-sandbox /opt/bun/bin/bun -e "
import { Database } from 'bun:sqlite'
const db = new Database('/workspace/.epsilon/epsilon.db', { readonly: true })
...
PROJ_CHECK=$(docker exec epsilon-sandbox /opt/bun/bin/bun -e '
import { Database } from "bun:sqlite"
const db = new Database("/workspace/.epsilon/epsilon.db", { readonly: true })
...
for i in 1 2 3 4 5 6 7 8; do
  (
    R=$(curl -s -X POST "$BASE/epsilon/tickets" \
...
for retry in 1 2 3 4 5; do
  if docker exec epsilon-sandbox /opt/bun/bin/bun -e '
import { Database } from "bun:sqlite"
const db = new Database("/workspace/.epsilon/epsilon.db")
...
SEED_PROBE=$(docker exec epsilon-sandbox /opt/bun/bin/bun -e '
import { Database } from "bun:sqlite"
const db = new Database("/workspace/.epsilon/epsilon.db", { readonly: true })
