...
function makeWorkspace(): string {
  return mkdtempSync(join(tmpdir(), "epsilon-trigger-agent-e2e-"))
}

function runPluginScenario(workspaceRoot: string) {
  const script = String.raw`
const workspaceRoot = process.env.EPSILON_WORKSPACE
const pluginMod = await import(new URL("./opencode/plugin/epsilon-system/epsilon-system.ts?e2e=" + Date.now(), import.meta.url).href)
const promptCalls = []
...
const yamlBeforeDelete = await Bun.file(workspaceRoot + "/.epsilon/triggers.yaml").text()
...
const yamlAfterDelete = await Bun.file(workspaceRoot + "/.epsilon/triggers.yaml").text()
...
`
  ...
  const result = spawnSync("bun", ["-e", script], {
    cwd: "/Users/markokraemer/Projects/heyagi/chainlens/core/epsilon-master",
    env: {
      ...process.env,
      EPSILON_WORKSPACE: workspaceRoot,
      EPSILON_TRIGGER_WEBHOOK_PORT: "0",
      EPSILON_DISABLE_CORE_SUPERVISOR: "true",
    },
    encoding: "utf8",
  })
...
describe("Epsilon trigger tool agent e2e", () => {
  let workspaceRoot = ""
  ...
  test("epsilon-system exposes trigger tools and they work through the returned tool registry", () => {
  ...
  test("epsilon agent instructions explicitly direct the model to use trigger tools", () => {
    const epsilonPrompt = readFileSync(
      "/Users/markokraemer/Projects/heyagi/chainlens/core/epsilon-master/opencode/agents/epsilon.md",
      "utf8",
    )

    expect(epsilonPrompt).toContain("If a user asks you to create, inspect, pause, resume, run, or sync triggers, start with the `triggers` tool")
    expect(epsilonPrompt).toContain("triggers action=create")
    expect(epsilonPrompt).toContain("triggers action=list")
    expect(epsilonPrompt).toContain("Do **not** invent a `ktriggers` CLI")
  })
})
