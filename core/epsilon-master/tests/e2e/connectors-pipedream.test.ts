...
import { cleanupRuntimeFixture, createRuntimeFixture, startDummyOpenCode, startEpsilonMaster, waitForHttp, type RuntimeFixture, type StartedServer } from "./helpers"

const scriptPath = "/Users/markokraemer/Projects/heyagi/computer/core/epsilon-master/opencode/skills/EPSILON-system/connectors/integration.ts"
const pluginPath = "/Users/markokraemer/Projects/heyagi/computer/core/epsilon-master/opencode/plugin/connectors/connectors.ts"
...
describe("connectors plugin + pipedream script e2e", () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), "epsilon-connectors-"))
  })
  ...
  test("plugin discovers example connector from workspace and scaffolded connectors batch", async () => {
    ...
    const setup = await plugin.tool.connector_setup.execute({
      connectors: JSON.stringify([
        { name: "github", description: "epsilon-ai org", source: "cli", status: "pending" },
...
describe("integration.ts end-to-end through epsilon-master", () => {
  ...
  beforeAll(async () => {
    ...
    rmSync("/tmp/epsilon-agent-triggers/cron-state.json", { force: true })

    const app = new Hono()

    app.use("*", async (c, next) => {
      const auth = c.req.header("authorization")
      if (auth && auth !== "Bearer test-epsilon-token") {
    ...
    master = await startEpsilonMaster(masterPort, fx, {
      INTERNAL_SERVICE_KEY: key,
      EPSILON_TOKEN: "test-epsilon-token",
      EPSILON_API_URL: `http://127.0.0.1:${apiPort}`,
  ...
  async function run(cmd: string, json?: object) {
    const proc = Bun.spawn(["bun", "run", scriptPath, cmd, ...(json ? [JSON.stringify(json)] : [])], {
      env: {
        ...process.env,
        EPSILON_MASTER_URL: `http://127.0.0.1:${masterPort}`,
        INTERNAL_SERVICE_KEY: key,
      },
      stdout: "pipe",
      stderr: "pipe",
