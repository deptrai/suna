...
import { clearAllStartupAbortedSessions } from "../../opencode/plugin/epsilon-system/lib/startup-aborted-sessions"
import {
	assistantMessage,
	completedTool,
...
function makeStorage(): string {
	const root = mkdtempSync(path.join(tmpdir(), "epsilon-autowork-e2e-"))
	tempRoots.push(root)
...
describe("autowork lifecycle e2e", () => {
	test("enters planning first and does not execute without an approved plan", async () => {
		...
		expect(harness.prompts[0]?.text).toContain("<epsilon_autowork_plan>")
	...
	test("moves into verifier phase after a valid completion candidate", async () => {
		...
		expect(harness.prompts[1]?.text).toContain("<epsilon_autowork_verified>")
		expect(harness.isActive()).toBe(true)
