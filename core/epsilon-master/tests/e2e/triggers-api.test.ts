...
function createTestApp(dir: string) {
  ...
  mkdirSync(join(dir, '.epsilon'), { recursive: true })
  const store = new TriggerStore(join(dir, '.epsilon', 'test.db'))
...
describe('Triggers API E2E', () => {
  ...
  describe('full lifecycle: cron + prompt', () => {
    it('create → list → get → update → pause → resume → run → executions → delete', async () => {
      ...
      const { status: createStatus, json: createJson } = await req('POST', '/triggers', {
        ...
        action: { type: 'prompt', prompt: 'Generate the daily report', agent: 'epsilon' },
      })
  ...
  describe('YAML sync', () => {
    it('persists created triggers to triggers.yaml', async () => {
      ...
      const yamlPath = join(tempDir, '.epsilon', 'triggers.yaml')
    ...
    it('removes from YAML on delete', async () => {
      ...
      // Verify it's in YAML
      let content = readFileSync(join(tempDir, '.epsilon', 'triggers.yaml'), 'utf8')
      ...
      content = readFileSync(join(tempDir, '.epsilon', 'triggers.yaml'), 'utf8')
    ...
    it('sync endpoint reads YAML back into DB', async () => {
      ...
      writeFileSync(join(tempDir, '.epsilon', 'triggers.yaml'), yamlContent)
