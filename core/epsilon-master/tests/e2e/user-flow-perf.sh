...
#   3. Cold-start budget — epsilon-master + opencode respawn within an
#      acceptable window so a flag flip doesn't strand the user.
#
# All checks run via docker exec / curl against the live epsilon-sandbox.
set -u
...
# Establish baseline state: flag off, opencode + epsilon-master up
H=$(curl -s "$BASE/epsilon/health")
ci "/epsilon/health → projectsEnabled:false" '"projectsEnabled":false' "$H"
ci "/epsilon/health → status:ok" '"status":"ok"' "$H"
ci "/epsilon/health → runtimeReady:true" '"runtimeReady":true' "$H"

# Latency budget: health/router endpoints should be snappy.
for trial in 1 2 3; do
  s=$(now_ms); curl -s -o /dev/null "$BASE/epsilon/health"; e=$(now_ms)
  le "GET /epsilon/health latency trial $trial" "300" "$((e-s))"
done

# Tool registry — the LLM hits this once per turn, must be sub-second.
for trial in 1 2 3; do
  s=$(now_ms); docker exec epsilon-sandbox curl -s -o /dev/null $INNER/experimental/tool/ids; e=$(now_ms)
  le "GET /experimental/tool/ids latency trial $trial" "1000" "$((e-s))"
done

echo
echo "── 2. Default flow — general agent has the tools the user needs ──"
TOOLS=$(docker exec epsilon-sandbox curl -s $INNER/experimental/tool/ids)
...
for url in /epsilon/triggers /epsilon/cron /epsilon/channels /epsilon/connectors \
           /epsilon/preferences /epsilon/services; do
...
s=$(now_ms)
TR_RESP=$(curl -s -X POST "$BASE/epsilon/triggers" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "$TR_BODY")
e=$(now_ms)
...
if [ -n "$TR_ID" ]; then
  s=$(now_ms); GET_RESP=$(curl -s "$BASE/epsilon/triggers/$TR_ID" -H "Authorization: Bearer $TOKEN"); e=$(now_ms)
  ...
  s=$(now_ms); curl -s -o /dev/null -X DELETE "$BASE/epsilon/triggers/$TR_ID" -H "Authorization: Bearer $TOKEN"; e=$(now_ms)
...
docker cp "$SCRIPT_DIR/_helpers-prompt-leak-test.ts" epsilon-sandbox:/tmp/prompt-leak-test.ts >/dev/null 2>&1
LEAK=$(docker exec epsilon-sandbox /opt/bun/bin/bun /tmp/prompt-leak-test.ts 2>&1)
...
echo
echo "── 6. Cold-start: epsilon-master respawn under 10s ──"
# Measure end-to-end: kill epsilon-master → it's back serving health.
...
for i in $(seq 1 100); do
  sleep 0.1
  CODE=$(curl -s --max-time 1 -o /dev/null -w '%{http_code}' "$BASE/epsilon/health" 2>/dev/null)
  [ "$CODE" = "200" ] && break
done
e=$(now_ms)
le "epsilon-master cold-start" "10000" "$((e-s))"

echo
echo "── 7. Flag-on flow — flip latency budget ──"
# Time from env-flip to opencode having project_get registered.
docker exec epsilon-sandbox sh -c 'echo -n "true" > /run/s6/container_environment/EPSILON_PROJECTS_ENABLED'
s=$(now_ms)
KMPID=$(docker exec epsilon-sandbox sh -c "ps -ef | grep 'bun run /ephemeral/epsilon-master' | grep -v grep | awk '{print \$2}'" | head -1)
docker exec epsilon-sandbox kill -TERM "$KMPID"
for i in $(seq 1 30); do
...
for i in $(seq 1 90); do
  sleep 1
  T=$(docker exec epsilon-sandbox curl -s --max-time 2 $INNER/experimental/tool/ids 2>/dev/null)
...
for url in /epsilon/projects /epsilon/projects/proj-workspace /epsilon/tickets; do
  s=$(now_ms); CODE=$(curl -s -o /dev/null -w '%{http_code}' "$BASE$url" -H "Authorization: Bearer $TOKEN"); e=$(now_ms)
  if [ "$CODE" != "503" ]; then
...
TOOLS_FINAL=$(docker exec epsilon-sandbox curl -s $INNER/experimental/tool/ids)
