...
describe("Security Tests", () => {
  ...
  beforeAll(async () => {
    fixture = createRuntimeFixture("epsilon-security-");
    opencode = await startDummyOpenCode(9003);
    serverProcess = await startEpsilonMaster(8003, fixture, {
      EPSILON_TOKEN: "security-test-token",
      OPENCODE_PORT: "9003",
    });
  });
  ...
  test("different EPSILON_TOKEN should not break secret decryption", async () => {
    ...
    // Start server with different token on a different port.
      ...
      const newServerProcess = await startEpsilonMaster(8004, fixture, {
        EPSILON_TOKEN: "different-security-test-token",
        OPENCODE_PORT: "9004",
      });
