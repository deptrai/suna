#!/usr/bin/env bash
# E2E test: single-project paradigm in the epsilon sandbox.
#
# Paradigm:
...
#   - Feature flag EPSILON_PROJECTS_ENABLED gates the project-paradigm surface
...
#
# Runs entirely against the live epsilon-sandbox container via docker exec —
...
for p in /epsilon/projects /epsilon/tickets /epsilon/tasks /epsilon/projects/proj-workspace; do
...
TOOLS_OFF=$(docker exec epsilon-sandbox curl -s $INNER/experimental/tool/ids)
total=$((total+1))
...
TOOLS_ON=$(docker exec epsilon-sandbox curl -s $INNER/experimental/tool/ids)
...
# Even after a epsilon-master restart, project_create stays absent.
KMPID=$(docker exec epsilon-sandbox sh -c "ps -ef | grep 'bun run /ephemeral/epsilon-master' | grep -v grep | awk '{print \$2}'" | head -1)
docker exec epsilon-sandbox kill -TERM "$KMPID" 2>/dev/null
sleep 5
OPCDPID=$(docker exec epsilon-sandbox sh -c "ps -ef | grep 'opencode-epsilon serve' | grep -v grep | awk '{print \$2}'" | head -1)
docker exec epsilon-sandbox kill -TERM "$OPCDPID" 2>/dev/null
...
TOOLS_REPLAY=$(docker exec epsilon-sandbox curl -s $INNER/experimental/tool/ids)
...
H_OFF=$(curl -s "$BASE/epsilon/health")
ci "after flip OFF: features.projectsEnabled=false (default restored)" '"projectsEnabled":false' "$H_OFF"

TOOLS_OFF2=$(docker exec epsilon-sandbox curl -s $INNER/experimental/tool/ids)
...
OVERLAY=$(docker exec epsilon-sandbox cat /workspace/.opencode/opencode.jsonc)
...
TRIG_RESP=$(curl -s -X POST "$BASE/epsilon/triggers" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "$TRIG_BODY")
...
[ -n "$TRIG_ID" ] && curl -s -X DELETE "$BASE/epsilon/triggers/$TRIG_ID" -H "Authorization: Bearer $TOKEN" >/dev/null
