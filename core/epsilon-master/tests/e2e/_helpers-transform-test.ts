...
// in epsilon-system.ts is working, the wrapper around it returns early
// and nothing is appended.

import { projectStatusTransform, ProjectManager, initProjectsDb } from "/ephemeral/epsilon-master/opencode/plugin/epsilon-system/projects.ts";
import { config } from "/ephemeral/epsilon-master/src/config.ts";
import * as path from "node:path";

console.log("PROJECTS_ENABLED:", config.PROJECTS_ENABLED);
console.log("env EPSILON_PROJECTS_ENABLED:", process.env.EPSILON_PROJECTS_ENABLED);

// Build a real ProjectManager (it won't make outbound calls just by being constructed)
const ws = "/workspace";
const db = initProjectsDb(path.join(ws, ".epsilon/epsilon.db"));
const fakeClient = {} as any;
const mgr = new ProjectManager(fakeClient, ws, db);

// Test 1: if the gate is working, the wrapper in epsilon-system.ts only invokes
...
// The wrapper in epsilon-system.ts is what skips it. Now simulate the wrapper:
