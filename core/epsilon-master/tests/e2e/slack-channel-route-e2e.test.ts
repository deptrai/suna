...
describe('Slack channel webhook route', () => {
  const originalWorkspace = process.env.EPSILON_WORKSPACE
  ...
  beforeEach(async () => {
    ...
    process.env.EPSILON_WORKSPACE = workspace
    process.env.INTERNAL_SERVICE_KEY = 'epsilon_sb_test'
    process.env.EPSILON_TOKEN = 'epsilon_sb_test'
    fetchCalls.length = 0
  ...
  afterEach(async () => {
    ...
    process.env.EPSILON_WORKSPACE = originalWorkspace
  ...
  it('accepts a signed Slack event and returns 202 instead of crashing', async () => {
    ...
    mkdirSync(join(workspace, '.epsilon'), { recursive: true })
    const db = new Database(join(workspace, '.epsilon', 'epsilon.db'))
    db.exec(`
      CREATE TABLE IF NOT EXISTS channels (
        ...
        bot_id TEXT,
        bot_username TEXT,
        default_agent TEXT DEFAULT 'epsilon',
    ...
    db.prepare(`
      ...
      VALUES (?, 'slack', 'Slack Route Test', 1, 'xoxb-test', 'signing-secret', 'webhook-secret', ?, 'U0LAN0Z89', 'epsilon-bot', 'epsilon', 'openai/gpt-4.1-mini', 'Always answer with a brief confirmation first.', 'You are the Slack support bot for this workspace.', NULL, ?, ?)
    ...
    expect(createCall?.body?.agent).toBe('epsilon')
