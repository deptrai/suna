...
import {
  ...
  listAgents,
  getAgentBySlug,
  insertAgent as _insertAgentRaw,
  markAgentReady,
  getTicket,
  listTickets,
  extractMentions,
...
import { findAgentForSession, agentHasGroup, TOOL_GROUPS, ticketToolGateHook } from '../../opencode/plugin/epsilon-system/ticket-tools'

/**
 * Test shim: insertAgent + flip ready_at to now. Production code leaves a
 * freshly-created agent at ready_at=NULL until opencode's cache refresh
 * completes; tests don't go through the refresh cycle so we mark ready
 * directly here.
 */
function insertAgent(db: Database, projectId: string, input: AgentInput): ProjectAgentRow {
  const row = _insertAgentRaw(db, projectId, input)
  markAgentReady(db, row.id)
  return { ...row, ready_at: new Date().toISOString() }
}
...
function makeFixture(): Fixture {
  const dir = mkdtempSync(path.join(tmpdir(), 'epsilon-tickets-e2e-'))
  const projectPath = path.join(dir, 'workspace')
  const dbDir = path.join(projectPath, '.epsilon')
  mkdirSync(dbDir, { recursive: true })
  const dbPath = path.join(dbDir, 'epsilon.db')
...
describe('v2 project seeding', () => {
  test('seeds PM agent, default columns, structure_version=2, and Team section in CONTEXT.md', async () => {
    ...
    const ctx = await fs.readFile(path.join(fx.project.path, '.epsilon', 'CONTEXT.md'), 'utf8')
...
describe('ticket lifecycle', () => {
  ...
  test('assign_to on create routes to listed owner, skips column default, and is self-stripped for the creator', async () => {
    const fx = await setupSeededProject()
    const pm = getAgentBySlug(fx.db, fx.project.id, DEFAULT_PM_SLUG)!
    const engPersona = path.join(fx.project.path, '.epsilon', 'agents', 'engineer.md')
  ...
  test('status change to a column with default assignee auto-assigns and emits a trigger', async () => {
    const fx = await setupSeededProject()
    const qaPersona = path.join(fx.project.path, '.epsilon', 'agents', 'qa.md')
...
describe('per-ticket execution mode', () => {
  ...
  test('per_assignment mode spawns a new session each time', async () => {
    const fx = await setupSeededProject()
    const persona = path.join(fx.project.path, '.epsilon', 'agents', 'eng2.md')
...
describe('tool_group enforcement', () => {
  test('agent with only project_action cannot call team_create_agent', async () => {
    const fx = await setupSeededProject()
    const persona = path.join(fx.project.path, '.epsilon', 'agents', 'worker.md')
    await fs.mkdir(path.dirname(persona), { recursive: true })
...
describe('structure_version branching', () => {
  ...
  test('syncTeamSection updates the CONTEXT.md after a new agent is registered', async () => {
    const fx = await setupSeededProject()
    const engPath = path.join(fx.project.path, '.epsilon', 'agents', 'engineer.md')
...
describe('sub-tickets + parent_id permission', () => {
  ...
  test('createTicket rejects parent_id from a different project', async () => {
    ...
    const fx = await setupSeededProject()
    const otherDir = mkdtempSync(path.join(tmpdir(), 'epsilon-other-'))
