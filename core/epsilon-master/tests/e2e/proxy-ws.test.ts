...
async function startEpsilonMasterForTest(
  port: number,
  ...
  const proc = spawn('bun', ['run', 'src/index.ts'], {
    ...
    env: {
      ...
      EPSILON_WORKSPACE_ROOT: fixture.workspaceRoot,
      S6_ENV_DIR: fixture.s6EnvDir,
      EPSILON_MASTER_PORT: String(port),
      OPENCODE_HOST: '127.0.0.1',
      OPENCODE_PORT: String(port + 1000),
      EPSILON_DISABLE_CORE_SUPERVISOR: 'true',
      EPSILON_DISABLE_AUTH_SYNC: 'true',
      ...extraEnv,
    },
  })
...
describe('Epsilon Sandbox Proxy WebSocket E2E', () => {
  beforeAll(async () => {
    runtime = createRuntimeFixture('epsilon-proxy-ws-')
    opencode = await startDummyOpenCode(OPENCODE_TEST_PORT)
    master = await startEpsilonMasterForTest(MASTER_PORT, runtime, {
