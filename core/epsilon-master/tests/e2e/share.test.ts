...
const ENV_KEYS = [
  ...
  'SANDBOX_ID', 'EPSILON_API_URL', 'SHARE_STORE_PATH',
]
...
describe('Share system E2E', () => {
  ...
  test('each share gets a unique token', async () => {
    const app = buildApp()
    const a = await (await app.request('/epsilon/share/3000')).json() as any
    const b = await (await app.request('/epsilon/share/3000')).json() as any
  ...
  test('share proxy returns 502 when nothing listens on the port', async () => {
    ...
    const createRes = await app.request('/epsilon/share/19999')
  ...
  test('rejects TTL below 5m', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=1m')
    expect(res.status).toBe(400)
  })

  test('rejects TTL above 7d', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/3000?ttl=8d')
    expect(res.status).toBe(400)
  })
  ...
  test('list shows created shares', async () => {
    const app = buildApp()
    const { token } = await (await app.request('/epsilon/share/4444')).json() as any
  ...
  test('revoke makes token invalid', async () => {
    const app = buildApp()
    const { token } = await (await app.request('/epsilon/share/5555')).json() as any
  ...
  test('share proxy OPTIONS returns CORS headers', async () => {
    const app = buildApp()
    const { token } = await (await app.request('/epsilon/share/3000')).json() as any
  ...
  test('port 0 rejected', async () => {
    const app = buildApp()
    expect((await app.request('/epsilon/share/0')).status).toBe(400)
  })

  test('port 65535 accepted', async () => {
    const app = buildApp()
    const res = await app.request('/epsilon/share/65535')
    expect(res.status).toBe(200)
    const body = await res.json() as any
  ...
  test('URL contains /s/{token}/', async () => {
    const app = buildApp()
    const body = await (await app.request('/epsilon/share/3000')).json() as any
  ...
  test('explicit PUBLIC_BASE_URL wins over cloud fallback URLs', async () => {
    ...
    process.env.EPSILON_API_URL = 'https://api.epsilon.test/v1/router'
    process.env.PUBLIC_BASE_URL = 'https://8000--real-slug.epsilon.cloud?__proxy_token=pt_real'

    const app = buildApp()
    const body = await (await app.request('/epsilon/share/3000')).json() as any
    expect(body.url).toContain('https://8000--real-slug.epsilon.cloud')
