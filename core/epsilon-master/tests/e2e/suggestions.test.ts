...
describe('Session Suggestions', () => {
  ...
  beforeAll(async () => {
    fixture = createRuntimeFixture('epsilon-suggestions-')
    mkdirSync(join(fixture.workspaceRoot, '.epsilon'), { recursive: true })
    writeFileSync(join(fixture.workspaceRoot, '.epsilon', 'MEMORY.md'), [
      '# Global Memory',
      '',
      '## Environment',
      '',
      '- user is comparing Hermes and Epsilon memory designs',
      '- focus on session tools and merged prompt memory files',
      '',
    ].join('\n'))
    opencode = await startDummyOpenCode(9004)
    master = await startEpsilonMaster(8004, fixture, {
      EPSILON_TOKEN: 'suggestions-test-token',
      OPENCODE_PORT: '9004',
    })
  })
