...
import { cleanupRuntimeFixture, createRuntimeFixture, startDummyOpenCode, startEpsilonMaster, type RuntimeFixture, type StartedServer } from "./helpers";

/**
 * Epsilon Sandbox Proxy E2E Tests
 *
 * Tests the dynamic port proxy route (/proxy/:port/*) on epsilon-master.
...
describe("Epsilon Sandbox Proxy E2E", () => {
  // ------ Setup / Teardown ------
  beforeAll(async () => {
    runtime = createRuntimeFixture('epsilon-proxy-');
    opencode = await startDummyOpenCode(9000);
    master = await startEpsilonMaster(8000, runtime, {
      EPSILON_TOKEN: 'proxy-test-token',
      OPENCODE_PORT: '9000',
    });
  ...
  // ------ Health ------
  describe("Health", () => {
    test("epsilon-master health check returns ok", async () => {
      const { status, data } = await proxyJson<{ status: string }>("/epsilon/health");
