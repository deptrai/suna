...
function makeFixture(): Fixture {
  const dir = mkdtempSync(path.join(tmpdir(), 'epsilon-cred-e2e-'))
  const dbPath = path.join(dir, 'epsilon.db')
...
describe('credential-service — raw-file confidentiality', () => {
  test('plaintext value bytes never appear in the raw epsilon.db file', async () => {
    const fx = setup()
