...
#
# Web dev server expected on :3000 (pnpm --filter Epsilon-Computer-Frontend dev).
...
if ! curl -s --max-time 2 -o /dev/null "$WEB" 2>/dev/null; then
  ...
  echo "    pnpm --filter Epsilon-Computer-Frontend dev"
  exit 1
fi
...
ci "board page imports TicketBoard" "from '@/components/epsilon/ticket-board'" "$(cat "$BP")"
