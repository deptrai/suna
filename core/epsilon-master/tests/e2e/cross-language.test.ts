...
import { cleanupRuntimeFixture, createRuntimeFixture, startDummyOpenCode, startEpsilonMaster, type RuntimeFixture } from "./helpers";
...
describe("Cross-Language Environment Variable Access", () => {
  ...
  let serverProcess: Awaited<ReturnType<typeof startEpsilonMaster>> | null = null;
  let opencode: Awaited<ReturnType<typeof startDummyOpenCode>> | null = null;
  let fixture: RuntimeFixture;

  beforeAll(async () => {
    fixture = createRuntimeFixture("epsilon-cross-language-");
    opencode = await startDummyOpenCode(9001);
    serverProcess = await startEpsilonMaster(8001, fixture, {
      EPSILON_TOKEN: "e2e-test-token",
      OPENCODE_PORT: "9001",
    });
  });
