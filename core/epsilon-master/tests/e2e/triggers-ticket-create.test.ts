...
async function makeFixture(): Promise<Fixture> {
  const dir = mkdtempSync(path.join(tmpdir(), 'epsilon-trigtix-e2e-'))
  const projectPath = path.join(dir, 'demo-proj')
  mkdirSync(projectPath, { recursive: true })
  mkdirSync(path.join(projectPath, '.epsilon'), { recursive: true })

  // Point the ticket-create-action at our temp DB via env.
  process.env.EPSILON_WORKSPACE = dir
  mkdirSync(path.join(dir, '.epsilon'), { recursive: true })
  const db = new Database(path.join(dir, '.epsilon', 'epsilon.db'), { create: true, readwrite: true })
  db.exec(`
...
afterEach(() => {
  delete process.env.EPSILON_WORKSPACE
