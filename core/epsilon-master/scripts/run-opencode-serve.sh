...
# Called by epsilon-master core supervisor — NOT by s6 directly.
...
export XDG_DATA_HOME="${XDG_DATA_HOME:-${EPSILON_PERSISTENT_ROOT}}"
export OPENCODE_CONFIG_DIR=/ephemeral/epsilon-master/opencode
...
if [ -x "/usr/local/bin/opencode-epsilon" ]; then
  export OPENCODE_BIN_PATH="/usr/local/bin/opencode-epsilon"
fi
...
if [ "$XDG_DATA_HOME" != "$EPSILON_PERSISTENT_ROOT" ]; then
  echo "[opencode-serve] ERROR: XDG_DATA_HOME must equal EPSILON_PERSISTENT_ROOT ($EPSILON_PERSISTENT_ROOT), got $XDG_DATA_HOME"
  exit 1
fi
...
if ! command -v epsilon-opencode-state >/dev/null 2>&1; then
  echo "[opencode-serve] ERROR: epsilon-opencode-state is required but missing"
  exit 1
fi
...
# Pick up vars written by epsilon-api after container start.
...
TOKEN_FILE="/run/s6/container_environment/EPSILON_TOKEN"
API_URL_FILE="/run/s6/container_environment/EPSILON_API_URL"

if [ ! -s "$TOKEN_FILE" ] || [ ! -s "$API_URL_FILE" ]; then
  echo "[opencode-serve] Waiting for EPSILON_TOKEN and EPSILON_API_URL to be provisioned..."
  for i in $(seq 1 5); do
...
[ -s "$TOKEN_FILE" ] && \
  export EPSILON_TOKEN="$(cat "$TOKEN_FILE")"
[ -s "$API_URL_FILE" ] && \
  export EPSILON_API_URL="$(cat "$API_URL_FILE")"
...
# OPENROUTER_API_KEY, EPSILON_YOLO_{API_KEY,URL}. Without this, keys added
# post-boot never reach opencode, because s6-supervise's env snapshot
# was taken before those files existed.
for _pv in ANTHROPIC_API_KEY OPENAI_API_KEY OPENROUTER_API_KEY EPSILON_YOLO_API_KEY EPSILON_YOLO_URL; do
...
# Safety check: if EPSILON_API_URL is still unset or points to localhost, warn loudly.
# This catches pool sandboxes where env injection failed or was delayed.
if [ -z "$EPSILON_API_URL" ] || echo "$EPSILON_API_URL" | grep -q "localhost"; then
  echo "[opencode-serve] WARNING: EPSILON_API_URL is '${EPSILON_API_URL:-unset}' — LLM calls will fail in cloud mode!"
fi
...
if command -v epsilon-opencode-state >/dev/null 2>&1; then
  epsilon-opencode-state guard >/dev/null 2>&1 || echo "[opencode-serve] WARNING: state guard failed"
fi
