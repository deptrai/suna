...
resolve_latest_version() {
  ...
  gh_version=$(curl -sf --connect-timeout 5 \
    "https://api.github.com/repos/epsilon-ai/chainlens/releases/latest" 2>/dev/null \
...
EPSILON_LOCAL_IMAGES="${EPSILON_LOCAL_IMAGES:-0}"
EPSILON_LOCAL_TAG="${EPSILON_LOCAL_TAG:-latest}"
EPSILON_BUILD_LOCAL_IMAGES="${EPSILON_BUILD_LOCAL_IMAGES:-0}"
EPSILON_PULL_PARALLELISM="${EPSILON_PULL_PARALLELISM:-4}"
...
EPSILON_LOCAL_REPO_ROOT="${EPSILON_LOCAL_REPO_ROOT:-$REPO_ROOT}"
...
parse_args() {
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --local)
        EPSILON_LOCAL_IMAGES=1
        shift
        ;;
      --build-local)
        EPSILON_LOCAL_IMAGES=1
        EPSILON_BUILD_LOCAL_IMAGES=1
        shift
        ;;
      --local-tag)
...
Examples:
  bash get-epsilon.sh
  bash get-epsilon.sh --local
  bash get-epsilon.sh --local --build-local
  bash get-epsilon.sh --local --local-tag latest
  bash get-epsilon.sh --version 0.7.14
  bash get-epsilon.sh --query "v=0.7.14"
  EPSILON_VERSION=0.7.15 bash get-epsilon.sh
EOF
        exit 0
        ;;
      *)
...
if [ -z "$EPSILON_VERSION" ] && [ "$EPSILON_LOCAL_IMAGES" != "1" ]; then
  EPSILON_VERSION=$(resolve_latest_version)
fi
# Ensure EPSILON_VERSION always has a value (fallback to 'latest' tag)
EPSILON_VERSION="${EPSILON_VERSION:-latest}"

IMAGE_TAG="$EPSILON_VERSION"
SANDBOX_IMAGE_REPO="epsilon/computer"
if [ "$EPSILON_LOCAL_IMAGES" = "1" ]; then
  IMAGE_TAG="$EPSILON_LOCAL_TAG"
  SANDBOX_IMAGE_REPO="epsilon/computer"
fi

FRONTEND_IMAGE="${EPSILON_FRONTEND_IMAGE:-epsilon/epsilon-frontend:${IMAGE_TAG}}"
API_IMAGE="${EPSILON_API_IMAGE:-epsilon/epsilon-api:${IMAGE_TAG}}"
SANDBOX_IMAGE="${EPSILON_SANDBOX_IMAGE:-${SANDBOX_IMAGE_REPO}:${IMAGE_TAG}}"
FRONTEND_IMAGE_OVERRIDDEN="0"
API_IMAGE_OVERRIDDEN="0"
SANDBOX_IMAGE_OVERRIDDEN="0"
[ -n "${EPSILON_FRONTEND_IMAGE:-}" ] && FRONTEND_IMAGE_OVERRIDDEN="1"
[ -n "${EPSILON_API_IMAGE:-}" ] && API_IMAGE_OVERRIDDEN="1"
[ -n "${EPSILON_SANDBOX_IMAGE:-}" ] && SANDBOX_IMAGE_OVERRIDDEN="1"
...
# Different name + port range from dev (epsilon-sandbox / 14000) so both can
# run simultaneously on the same Docker daemon.
SANDBOX_CONTAINER_NAME="${SANDBOX_CONTAINER_NAME:-epsilon-hosted-sandbox}"
...
ensure_local_build_requirements() {
  [ -d "$EPSILON_LOCAL_REPO_ROOT/apps/web" ] || fatal "Local repo root not found at ${EPSILON_LOCAL_REPO_ROOT}."
  [ -x "$PNPM_BIN" ] || fatal "pnpm is required for --build-local."
}

rebuild_local_images() {
  ensure_local_build_requirements
  local build_script="$EPSILON_LOCAL_REPO_ROOT/scripts/build-local-images.sh"
  [ -f "$build_script" ] || fatal "Local build script not found: ${build_script}"

  info "Rebuilding local installer images from ${EPSILON_LOCAL_REPO_ROOT}"
  echo ""
  bash "$build_script" --tag "$EPSILON_LOCAL_TAG"
...
resolve_release_images() {
  [ "$EPSILON_LOCAL_IMAGES" = "1" ] && return 0

  dots "Resolving images for v${EPSILON_VERSION}"
  ...
  if ! docker_manifest_exists "$FRONTEND_IMAGE"; then
    ...
    local fallback_frontend="epsilon/epsilon-frontend:latest"
    if docker_manifest_exists "$fallback_frontend"; then
      ...
      fatal "Frontend image not found for ${EPSILON_VERSION}, and latest fallback is unavailable."
    fi
  fi
  ...
  if ! docker_manifest_exists "$API_IMAGE"; then
    ...
    local fallback_api="epsilon/epsilon-api:latest"
    if docker_manifest_exists "$fallback_api"; then
      ...
      fatal "API image not found for ${EPSILON_VERSION}, and latest fallback is unavailable."
    fi
  fi
  ...
  if ! docker_manifest_exists "$SANDBOX_IMAGE"; then
    ...
    if docker_manifest_exists "$fallback_sandbox"; then
      ...
      fatal "Sandbox image not found for ${EPSILON_VERSION}, and latest fallback is unavailable."
    fi
  fi
...
pull_images_parallel() {
  ...
  printf '%s\n' "${images[@]}" | python3 -c 'import sys; print("\n".join(sorted(set(line.strip() for line in sys.stdin if line.strip()))))' \
    | xargs -r -n1 -P "$EPSILON_PULL_PARALLELISM" docker pull
}

# Free ports used by Epsilon (local mode)
# Usage: free_epsilon_ports [project_name]
...
free_epsilon_ports() {
  ...
  if [ -n "$project_name" ]; then
    # Clean up containers from this compose project (e.g. epsilon-frontend-1).
    # EXCLUDES standalone sandbox containers (epsilon-sandbox, epsilon-hosted-sandbox)
    # which are managed by the API, not compose — killing them breaks dev mode.
    docker ps -a --format '{{.Names}}' 2>/dev/null \
      | grep -E "^${project_name}-" \
      | grep -v -E "^(epsilon-sandbox|epsilon-hosted-sandbox)$" \
  ...
  [ $freed -eq 1 ] && info "Freed Epsilon ports" || true
}
...
EOF
  ...
  printf "\n"
  printf "    ${FADED}v${EPSILON_VERSION}  ·  One-Click Installer  ·  Local or VPS${NC}\n"
  printf "\n"
}
...
# ─── Mode Selection ──────────────────────────────────────────────────────────
prompt_mode() {
  section "Where are you running Epsilon?"
...
ROLESEOF
  ...
  # Epsilon extensions and schemas
  cat > "$INSTALL_DIR/volumes/db/epsilon.sql" << 'EPSILONEOF'
-- Epsilon bootstrap: extensions and schemas
...
CREATE OR REPLACE FUNCTION epsilon.configure_scheduler(api_url TEXT, tick_secret TEXT)
RETURNS void AS $$
BEGIN
  PERFORM cron.unschedule('epsilon_global_tick')
    WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'epsilon_global_tick');
...
${db_ports}
    volumes:
      ...
      - ./volumes/db/epsilon.sql:/docker-entrypoint-initdb.d/init-scripts/99-epsilon.sql:Z
    environment:
  ...
  supabase-auth:
    ...
    environment:
      ...
      GOTRUE_SMTP_SENDER_NAME: Epsilon
...
"
    ...
    frontend_supabase_env="      - EPSILON_PUBLIC_SUPABASE_URL=\${SUPABASE_PUBLIC_URL}
      - EPSILON_PUBLIC_SUPABASE_ANON_KEY=\${SUPABASE_ANON_KEY}
      - EPSILON_PUBLIC_BACKEND_URL=\${API_PUBLIC_URL}/v1
      - EPSILON_PUBLIC_BILLING_ENABLED=false
      - EPSILON_PUBLIC_ENV_MODE=local
      - EPSILON_PUBLIC_APP_URL=\${PUBLIC_URL}
      - EPSILON_PUBLIC_SANDBOX_ID=\${SANDBOX_CONTAINER_NAME}
    ...
    frontend_supabase_env="      - EPSILON_PUBLIC_SUPABASE_URL=\${SUPABASE_URL}
      - EPSILON_PUBLIC_SUPABASE_ANON_KEY=\${SUPABASE_ANON_KEY}
      - EPSILON_PUBLIC_BACKEND_URL=\${API_PUBLIC_URL}/v1
      - EPSILON_PUBLIC_BILLING_ENABLED=false
      - EPSILON_PUBLIC_ENV_MODE=local
...
# Epsilon — auto-generated by get-epsilon.sh
...
${supabase_db_env}
      ...
      - EPSILON_LOCAL_IMAGES=\${EPSILON_LOCAL_IMAGES}
      - DOCKER_HOST=unix:///var/run/docker.sock
      - EPSILON_URL=http://epsilon-api:8008/v1/router
      ...
      - SANDBOX_VERSION=\${EPSILON_VERSION}
      - TUNNEL_SIGNING_SECRET=\${TUNNEL_SIGNING_SECRET}
      - EPSILON_ROUTER_INTERNAL_ENABLED=false
...
# ──────────────────────────────────────────────────────────────────────────────
# Epsilon — Environment Configuration
# Auto-generated by get-epsilon.sh on $(date -u '+%Y-%m-%d %H:%M:%S UTC')
# ──────────────────────────────────────────────────────────────────────────────
...
EPSILON_LOCAL_IMAGES=${EPSILON_LOCAL_IMAGES}
EPSILON_LOCAL_TAG=${EPSILON_LOCAL_TAG}
EPSILON_LOCAL_REPO_ROOT=${EPSILON_LOCAL_REPO_ROOT}
...
EPSILON_SANDBOX_VERSION=${EPSILON_VERSION}
ENVEOF
...
ALLOWED_SANDBOX_PROVIDERS=local_docker
EPSILON_LOCAL_IMAGES=${EPSILON_LOCAL_IMAGES}
EPSILON_URL=${API_PUBLIC_URL}/v1/router
...
# ─── Write CLI ───────────────────────────────────────────────────────────────
write_cli() {
  cat > "$INSTALL_DIR/epsilon" << 'CLIPATH'
#!/usr/bin/env bash
set -euo pipefail
...
VERSION=$(grep -m1 '^EPSILON_VERSION=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "unknown")
FRONTEND_IMAGE=$(grep -m1 '^FRONTEND_IMAGE=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "epsilon/epsilon-frontend:${VERSION}")
API_IMAGE=$(grep -m1 '^API_IMAGE=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "epsilon/epsilon-api:${VERSION}")
SANDBOX_IMAGE=$(grep -m1 '^SANDBOX_IMAGE=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "epsilon/computer:${VERSION}")
SANDBOX_NAME=$(grep -m1 '^SANDBOX_CONTAINER_NAME=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "epsilon-hosted-sandbox")
LOCAL_IMAGES=$(grep -m1 '^EPSILON_LOCAL_IMAGES=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "0")
LOCAL_TAG=$(grep -m1 '^EPSILON_LOCAL_TAG=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "latest")
LOCAL_REPO_ROOT=$(grep -m1 '^EPSILON_LOCAL_REPO_ROOT=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "")
...
_refresh_installer_and_cli() {
  local tmp_script="$DIR/get-epsilon.sh.tmp"
  curl -fsSL "https://raw.githubusercontent.com/epsilon-ai/chainlens/main/scripts/get-epsilon.sh" -o "$tmp_script" || return 0
  chmod +x "$tmp_script"
  mv "$tmp_script" "$DIR/get-epsilon.sh"
  awk '
    capture && $0 == "CLIPATH" { exit }
    capture { print; next }
    /<< '\''CLIPATH'\''$/ { capture = 1 }
  ' "$DIR/get-epsilon.sh" > "$DIR/epsilon.new" && chmod +x "$DIR/epsilon.new" && mv "$DIR/epsilon.new" "$DIR/epsilon" || true
}
...
_show_update_notice() {
  ...
  printf "  ${Y}!${N}  ${Y}Update available:${N} v%s ${F}(run: epsilon update)${N}\n" "$latest"
}
...
_project_name() {
  if [ -f "$DIR/.env" ]; then
    grep -m1 '^COMPOSE_PROJECT_NAME=' "$DIR/.env" 2>/dev/null | cut -d= -f2- || echo "epsilon"
  else
    echo "epsilon"
  fi
}
...
_free_epsilon_ports() {
  ...
  docker ps -a --format '{{.Names}}' 2>/dev/null \
    | grep -E "^${project_name}-" \
    | grep -v -E "^(epsilon-sandbox|${SANDBOX_NAME})$" \
    | xargs -r docker rm -f 2>/dev/null || true
  ...
  [ $freed -eq 1 ] && _info "Freed Epsilon ports" || true
}
...
_rebuild_local_images() {
  [ -n "$LOCAL_REPO_ROOT" ] || { _err "EPSILON_LOCAL_REPO_ROOT is not set"; exit 1; }
...
_reset_stack() {
  ...
  [ "$(_mode)" = "local" ] && _free_epsilon_ports
...
_banner() {
  printf "\n"
  printf "  ${C}${B}Epsilon CLI${N}  ${F}v${VERSION}${N}\n"
  printf "  ${F}────────────────────────────────────────${N}\n"
  _show_update_notice
}
...
case "${1:-help}" in
  ...
  update)
    ...
    if _using_local_images; then
      ...
      if [ -n "$local_latest" ] && [ "$local_latest" != "$VERSION" ]; then
        ...
        FRONTEND_IMAGE="epsilon/epsilon-frontend:${VERSION}"
        API_IMAGE="epsilon/epsilon-api:${VERSION}"
        SANDBOX_IMAGE="epsilon/computer:${VERSION}"
        _env_set EPSILON_VERSION "$VERSION"
        _env_set EPSILON_SANDBOX_VERSION "$VERSION"
  ...
  uninstall)
    ...
    if echo "$del_volumes" | grep -qi '^y'; then
      docker compose down -v --remove-orphans 2>/dev/null || true
      docker volume rm "${SANDBOX_NAME}-data" 2>/dev/null || true
      docker volume rm epsilon_supabase-db-data 2>/dev/null || true
      docker volume rm supabase_db_epsilon-local 2>/dev/null || true
      _ok "Volumes removed."
    else
      docker compose down 2>/dev/null || true
    fi
    [ -L "/usr/local/bin/epsilon" ] && rm -f /usr/local/bin/epsilon 2>/dev/null || true
    rm -rf "$DIR"
    _ok "Epsilon uninstalled."
    echo ""
    ;;
  open)
    _open "$(_url)"
  ...
  *)
    ...
    printf "  ${C}logs${N}        Tail logs ${F}(epsilon logs epsilon-api)${N}\n"
    ...
    printf "  ${C}uninstall${N}   Remove Epsilon completely\n"
...
CLIPATH
  ...
  chmod +x "$INSTALL_DIR/epsilon"
}

# ─── Add to PATH ─────────────────────────────────────────────────────────────
setup_path() {
  if [ -d "/usr/local/bin" ] && [ -w "/usr/local/bin" ]; then
    ln -sf "$INSTALL_DIR/epsilon" /usr/local/bin/epsilon 2>/dev/null && {
      success "Linked 'epsilon' -> /usr/local/bin/epsilon"
      return
    }
  fi
  ...
  echo "" >> "$shell_rc"
  echo "# Epsilon CLI" >> "$shell_rc"
  echo "export PATH=\"$INSTALL_DIR:\$PATH\"" >> "$shell_rc"
  success "Added 'epsilon' to PATH (restart terminal or: source $shell_rc)"
}
...
pull_and_start() {
  ...
  if [ "$EPSILON_LOCAL_IMAGES" = "1" ]; then
    if [ "$EPSILON_BUILD_LOCAL_IMAGES" = "1" ]; then
      rebuild_local_images
    fi
  ...
  printf "  ${GREEN}${BOLD}║             Epsilon is running!                  ║${NC}\n"
  printf "  ${GREEN}${BOLD}╚══════════════════════════════════════════════════╝${NC}\n"
  printf "\n"
  ...
  if [ "$DEPLOY_MODE" = "local" ]; then
    ...
    printf "    ${WHITE}Epsilon Cloud${NC}   ${CYAN}https://epsilon.com${NC}     ${FADED}managed, zero setup${NC}\n"
  ...
  printf "    ${CYAN}epsilon start${NC}    Start services\n"
  printf "    ${CYAN}epsilon stop${NC}     Stop services\n"
  printf "    ${CYAN}epsilon setup${NC}    Open sign-in page\n"
  printf "    ${CYAN}epsilon update${NC}   Pull latest & restart\n"
  printf "    ${CYAN}epsilon logs${NC}     Tail service logs\n"
  printf "    ${CYAN}epsilon status${NC}   Show running containers\n"
  printf "\n"
}
...
main() {
  ...
  # manually removed (rm -rf ~/.epsilon) without running `docker compose down -v`.
