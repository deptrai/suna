...
if [ ! -f "$ROOT_DIR/$ENV_FILE" ]; then
  ...
  echo "  Create a root .env with your values (see apps/api/.env.example)"
  exit 1
fi
...
write_env() {
  ...
  if [ -e "$full_path" ]; then
    echo "ERROR: $target already exists. Refusing to overwrite it."
    echo "  Delete it first if you really want scripts/setup-env.sh to regenerate it."
    exit 1
  fi
  {
