...
# ║                                                                            ║
# ║  Requires: Epsilon installed & running (sandbox container active)           ║
# ║                                                                            ║
# ║  Usage:                                                                    ║
...
# ║    OWNER_EMAIL      For auto-login (default: e2e@epsilon.ai)               ║
...
echo "${BOLD}${CYAN}"
echo "  ╔═══════════════════════════════════════════════╗"
echo "  ║  Epsilon — SSH Key E2E Test Suite              ║"
echo "  ╚═══════════════════════════════════════════════╝"
echo "${NC}"
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q 'epsilon-sandbox\|sandbox'; then
...
if echo "$PUBLIC_KEY" | grep -q 'epsilon-sandbox$'; then
  pass "Public key has epsilon-sandbox comment"
else
  fail "Public key has epsilon-sandbox comment"
fi
...
if [ "$IS_REMOTE_PROVIDER" -eq 0 ]; then
  ...
  CONTAINER_KEYS=$(docker exec epsilon-sandbox cat /config/.ssh/authorized_keys 2>/dev/null || docker exec epsilon-sandbox cat /workspace/.ssh/authorized_keys 2>/dev/null || true)
  ...
  PERMS=$(docker exec epsilon-sandbox stat -c '%a' /config/.ssh/authorized_keys 2>/dev/null || docker exec epsilon-sandbox stat -c '%a' /workspace/.ssh/authorized_keys 2>/dev/null || echo "unknown")
  if [ "$PERMS" = "600" ]; then
  ...
  OWNER=$(docker exec epsilon-sandbox stat -c '%U' /config/.ssh/authorized_keys 2>/dev/null || docker exec epsilon-sandbox stat -c '%U' /workspace/.ssh/authorized_keys 2>/dev/null || echo "unknown")
  if [ "$OWNER" = "abc" ]; then
...
if [ "$IS_REMOTE_PROVIDER" -eq 0 ]; then
  KEY_COUNT=$(docker exec epsilon-sandbox cat /config/.ssh/authorized_keys 2>/dev/null | grep -c 'ssh-ed25519' || true)
else
