...
# ║                                                                            ║
# ║  Requires: Epsilon installed & running with sandbox active                  ║
# ║                                                                            ║
# ║  Usage:                                                                    ║
...
echo "${BOLD}${CYAN}"
echo "  ╔═══════════════════════════════════════════════╗"
echo "  ║  Epsilon — Proxy URL E2E Test Suite            ║"
echo "  ╚═══════════════════════════════════════════════╝"
echo "${NC}"
...
if [ -n "$MASTER_HEALTH" ]; then
  ...
  if [ "$MASTER_ROOT_CODE" = "200" ] || [ "$MASTER_ROOT_CODE" = "404" ]; then
    pass "Epsilon Master responds via proxy ($MASTER_ROOT_CODE)"
  else
    fail "Epsilon Master via proxy (got: $MASTER_ROOT_CODE)"
  fi
fi
...
# On VPS, p6080-epsilon-sandbox.localhost should NOT resolve from the host
# (it resolves to 127.0.0.1, so curl from VPS hits VPS itself on port 443)
SUBDOMAIN_CODE=$(curl -s --connect-timeout 3 -o /dev/null -w "%{http_code}" "http://p6080-epsilon-sandbox.localhost:8008/" 2>/dev/null || echo "000")
...
EXPECTED_DESKTOP_URL="${PUBLIC_URL}/v1/p/epsilon-sandbox/6080/"
...
EXPECTED_STATIC_URL="${PUBLIC_URL}/v1/p/epsilon-sandbox/3211/"
