#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Test Suite: Embedded CLI (generated by get-epsilon.sh)                     ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
# Extract the embedded CLI from get-epsilon.sh (between CLIPATH markers)
CLI_CONTENT=$(sed -n "/cat > .*\/epsilon.*<< 'CLIPATH'/,/^CLIPATH/p" "$SCRIPT" | sed '1d;$d')

if [ -z "$CLI_CONTENT" ]; then
  fail "could not extract embedded CLI from get-epsilon.sh"
  echo ""; echo "  0/$TOTAL passed"; echo ""
  exit 1
fi

pass "embedded CLI extracted from get-epsilon.sh"
