#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Test Suite: get-epsilon.sh (unified installer)                             ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

ROOT_DIR="$(cd "$(dirname "$0")/../../.." && pwd)"
SCRIPT="$ROOT_DIR/scripts/get-epsilon.sh"
...
echo ""
echo "  Testing get-epsilon.sh structure"
echo "  ════════════════════════════════"
echo ""
...
if [ -f "$SCRIPT" ]; then
  pass "get-epsilon.sh exists"
else
  fail "get-epsilon.sh exists"
fi
...
if grep -q -- '--local' "$SCRIPT" && grep -q 'EPSILON_LOCAL_IMAGES' "$SCRIPT"; then
...
if grep -q 'POSTGRES_IMAGE\|epsilon/postgres' "$SCRIPT"; then
...
if grep -q 'DATABASE_URL.*postgres' "$SCRIPT"; then
  pass "compose sets DATABASE_URL for epsilon-api"
else
  fail "compose sets DATABASE_URL for epsilon-api"
fi
...
if grep -q 'epsilon/epsilon-frontend' "$SCRIPT" && grep -q 'epsilon/epsilon-api' "$SCRIPT"; then
...
if [ ! -f "$ROOT_DIR/scripts/install.sh" ]; then
  pass "install.sh deleted (unified into get-epsilon.sh)"
else
  fail "install.sh deleted (unified into get-epsilon.sh)"
fi

if [ ! -f "$ROOT_DIR/scripts/epsilon.sh" ]; then
  pass "epsilon.sh deleted (unified into get-epsilon.sh)"
else
  fail "epsilon.sh deleted (unified into get-epsilon.sh)"
fi
