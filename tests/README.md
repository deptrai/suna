...
All tests for the Epsilon platform, centralised in one place.
...
tests/
  ...
  shell/                  # Shell-based tests (no browser)
    ...
    installer/            #   get-epsilon.sh structure validation
      test-install.sh
      test-e2e-install.sh
...
|-------|------------------|
| `installer/test-install.sh` | get-epsilon.sh has correct structure, functions, compose |
...
|----------|---------|-------------|
| `E2E_OWNER_EMAIL` | `test-e2e@epsilon.ai` | Test owner email |
...
`core/epsilon-master/tests/`, `packages/*/test/`) stay in-place. They are
