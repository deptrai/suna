 ...
 *
 * FINDING K-3: Sandbox keys (epsilon_sb_*) have the same access level
 * as user keys (epsilon_*) in all routes that use combinedAuth or apiKeyAuth.
 *
...
describe('Code Review: Sandbox Key Privilege Escalation', () => {
  ...
  describe('[MEDIUM] No key type enforcement in middleware', () => {
    test('apiKeyAuth accepts both epsilon_ and epsilon_sb_ tokens', () => {
  ...
  describe('Routes accessible with sandbox key', () => {
    ...
    for (const route of routesWithCombinedAuth) {
      test(`${route} uses combinedAuth — sandbox key would be accepted`, () => {
        // A sandbox agent (which has a epsilon_sb_ key) can access these routes
