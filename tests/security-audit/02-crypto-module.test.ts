...
function deriveSigningKey(token: string): string {
  return createHmac('sha256', 'epsilon-tunnel-signing-v1').update(token).digest('hex');
}
...
describe('Security Audit: Crypto Module', () => {
  ...
  describe('Random key generation', () => {
    ...
    test('generates correct prefixed key formats', () => {
      const userKey = `epsilon_${randomAlphanumeric(32)}`;
      const sandboxKey = `epsilon_sb_${randomAlphanumeric(32)}`;
      const tunnelKey = `epsilon_tnl_${randomAlphanumeric(32)}`;
      const publicKey = `pk_${randomAlphanumeric(32)}`;

      expect(userKey).toMatch(/^epsilon_[A-Za-z0-9]{32}$/);
      expect(sandboxKey).toMatch(/^epsilon_sb_[A-Za-z0-9]{32}$/);
      expect(tunnelKey).toMatch(/^epsilon_tnl_[A-Za-z0-9]{32}$/);
  ...
  describe('HMAC key hashing', () => {
    test('produces consistent hashes for same input', () => {
      const key = 'epsilon_testkey12345678901234567890';
    ...
    test('produces different hashes for different keys', () => {
      const hash1 = hashSecretKey('epsilon_key1', TEST_SECRET);
      const hash2 = hashSecretKey('epsilon_key2', TEST_SECRET);
      expect(hash1).not.toBe(hash2);
    });

    test('produces different hashes with different secrets', () => {
      const key = 'epsilon_testkey';
    ...
    test('hash output is 64-char hex string (SHA-256)', () => {
      const hash = hashSecretKey('epsilon_test', TEST_SECRET);
      expect(hash.length).toBe(64);
      expect(hash).toMatch(/^[0-9a-f]{64}$/);
    });

    test('hash is not the same as plain SHA-256 (HMAC vs hash)', () => {
      const key = 'epsilon_test';
  ...
  describe('Timing-safe key verification', () => {
    test('verifies correct key successfully', () => {
      const key = 'epsilon_valid_key_12345678901234';
    ...
    test('rejects incorrect key', () => {
      const key = 'epsilon_valid_key_12345678901234';
      const hash = hashSecretKey(key, TEST_SECRET);
      expect(verifySecretKey('epsilon_wrong_key_1234567890123', hash, TEST_SECRET)).toBe(false);
    });
  ...
  describe('Tunnel message signing & verification', () => {
    const token = 'epsilon_tnl_test_token_123456789';
