 ...
 * a epsilon_ token (Mode 3 passthrough).
 *
 * ALL of these should require a epsilon_ token on cloud.
 ...
 * BUT the request still goes through Epsilon's infrastructure:
 * - Bandwidth consumed
 * - Epsilon IP used for the upstream request
...
describe('Cloud Scan: Proxy Route Map — All Should Require epsilon_ Token', () => {
  ...
  describe('These should all return Epsilon 401 instead of upstream errors', () => {
    test('EXPECTED: all non-epsilon requests should get Epsilon 401', () => {
      // Current: request forwards to upstream, upstream auth error leaks through
      // Expected: Epsilon rejects BEFORE forwarding
