...
if [ -d "$HOME/.epsilon" ]; then
  echo "[e2e] Cleaning existing ~/.epsilon stack"
  docker compose -f "$HOME/.epsilon/docker-compose.yml" down --remove-orphans --volumes >/dev/null 2>&1 || true
  rm -rf "$HOME/.epsilon"
fi

echo "[e2e] Running installer (local mode, minimal prompts)"
printf "y\n\n\n\nn\nn\n" | bash "scripts/get-epsilon.sh" >"$INSTALL_LOG" 2>&1
...
docker build -f "apps/web/Dockerfile" \
  ...
  -t "epsilon/epsilon-frontend:latest" . >/dev/null

echo "[e2e] Building local API image with current source"
docker build --build-arg SERVICE=epsilon-api -f "apps/api/Dockerfile" -t "epsilon/epsilon-api:latest" . >/dev/null

docker compose -f "$HOME/.epsilon/docker-compose.yml" up -d epsilon-api frontend >/dev/null
