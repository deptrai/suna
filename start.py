...
def print_manual_instructions():
    """Prints instructions for manually starting ChainLens services."""
    progress = load_progress()
    ...
    print("To start ChainLens, you need to run these commands in separate terminals:\n")
    ...
    print("Once all services are running, access ChainLens at: http://localhost:3000\n")
...
def main():
    ...
    if "--help" in sys.argv:
        ...
        print("Manage ChainLens services based on your setup method")
        print("\nOptions:")
    ...
    if setup_method == "manual":
        ...
        print("Managing all ChainLens services with Docker Compose...\n")
        ...
        if is_up:
            action = "stop"
            msg = "🛑 Stop all ChainLens services? [y/N] "
        else:
            action = "start"
            msg = "⚡ Start all ChainLens services? [Y/n] "
        ...
        if action == "stop":
            subprocess.run(["docker", "compose", "down"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All ChainLens services stopped.{Colors.ENDC}")
        else:
            subprocess.run(["docker", "compose", "up", "-d"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All ChainLens services started.{Colors.ENDC}")
            print(f"{Colors.CYAN}🌐 Access ChainLens at: http://localhost:3000{Colors.ENDC}")
