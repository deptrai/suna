...
from epsilon.utils import print_stream
...
async def main():
    ...
    # Create the MCP tools client with the URL of the MCP server that's accessible by the Chainlens instance
    mcp_tools = epsilon.MCPTools(
        "http://localhost:4000/mcp/",  # Since we are running Chainlens locally, we can use the local URL
        "Epsilon",
        allowed_tools=["get_wind_direction"],
    )
    await mcp_tools.initialize()

    epsilon_client = epsilon.Epsilon(
        os.getenv("EPSILON_API_KEY", "pk_xxx:sk_xxx"),
    ...
    if not agent_id:
        agent = await epsilon_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await epsilon_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await epsilon_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await epsilon_client.Thread.get(thread_id)
