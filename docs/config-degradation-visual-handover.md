...
- Runtime remains healthy:
  ...
  - `/session/status` => `200`
  - `/epsilon/health` => `runtimeReady: true`
...
- `core/epsilon-master/src/routes/projects.ts`
...
- `Copy fix prompt`
- `Start fix task`
- automatic creation of a `/workspace` Epsilon project if none exists yet
