# Epsilon Agent OS Framework + Cloud Platform Spec

**Status:** Draft V0.1  
**Date:** 2026-04-28  
**Repo context:** `chainlens` currently has the right primitives already emerging: `.opencode/` for agents/skills/commands/tools/plugins, `.epsilon/triggers.yaml`, connector CRUD, OCX marketplace routes, and a self-host installer CLI. This spec turns that into a coherent framework + managed cloud product.
...
Epsilon should solve this with one clear promise:
...
### Dogfood Wedge: Epsilon PlatformDev

The strongest first proof is to use Epsilon to run engineering automation for `https://github.com/epsilon-ai/chainlens`.
...
The initial agent system should be **EpsilonPlatformDev**: a repo-backed engineering agent team that watches production signals, GitHub activity, Slack prompts, and scheduled QA/maintenance windows, then investigates issues and opens PRs.
...
5. **Slack-to-task / Slack-to-PR agent**
   - Trigger: tag `@EpsilonPlatformDev` in Slack.
...
The best agents for Epsilon are not generic chatbots. They are background workers with clear event sources, tools, state, and deliverables.
...
Epsilon should be positioned as:
...
Epsilon should become **the fastest way to create, run, and deploy code-first background agents as real git-backed software projects**.

The mental model should be:

> **Next.js/Vercel for agents**: a local git repo defines agent behavior; `epsilon dev` runs the agent OS locally; `epsilon deploy` ships it to managed 24/7 cloud runtime with channels, triggers, integrations, secrets, logs, and persistent memory.

More specifically:

> **A Epsilon project is a git repo. A Epsilon deployment is that repo running inside a managed agent machine.**
...
3. an existing repo,
4. a hand-written `epsilon.toml` manifest,
...
Epsilon should package those pieces into one primitive:

> **An Agent App** — a git repo with a `epsilon.toml` manifest that can run locally or in Epsilon Cloud with the same behavior.

---

## 3. Product Shape

Epsilon becomes two products that reinforce each other:

### 3.0 Repo-Native Project Model

This is the fundamental refactor: **Epsilon projects should stop being primarily database/UI entities and become git repositories that can be cloned, edited, run, tested, deployed, rolled back, and forked.**
...
- `epsilon init` creates a repo.
- `epsilon dev` runs that repo locally.
- `epsilon deploy` deploys that repo to a managed machine/serverless runtime.
- `epsilon dev` should open a local cloud/control-plane UI that shows every local/cloud Epsilon instance the user can access.
...
- Existing ChainLens/Epsilon internals should be refactored so `.opencode`/`.epsilon` are implementation details generated from or contained by the repo.
...
This gives Epsilon the benefits of parallel agents without pretending parallel mutation is safe.
...
Epsilon should default to **one authoritative project brain with coordinated concurrency**, not uncontrolled swarm behavior.
...
This is the same reason human engineering teams use branches and PRs. Epsilon should make that automatic for agents.

CLI/UI implications:

```bash
epsilon task run "Fix BetterStack incident X" --isolate
epsilon task run "Investigate issue #123" --branch issue-123
epsilon sandboxes list
epsilon sandboxes fork production incident-abc123
epsilon sandboxes merge incident-abc123 --pr
```
...
- local runtime with OpenCode/Epsilon Master
- local trigger/webhook simulation
- local marketplace starter installation
- generated `.opencode` and `.epsilon` compatibility with the existing runtime
...
I ran the Prime CLI in a disposable folder at `/tmp/epsilon-prime-cli-smoke` to feel the UX.
...
### 4.2 Issues to Avoid in Epsilon

- `prime lab setup --prime-rl --no-interactive` tried `sudo apt` on macOS and continued with a warning. Epsilon setup should detect OS up front and avoid irrelevant package-manager steps.
- Some list commands hung for me (`rl models`, `rl configs`, `rl list`). Epsilon list/status commands must have aggressive timeouts, streaming progress, and clear offline/failure modes.
- `rl run` would launch paid/real hosted training. Epsilon must make paid cloud actions explicit, with `--dry-run`, cost preview, and a confirmation gate unless `--yes` is passed.
- Setup should be fast. Heavy downloads should be lazy or backgrounded unless required for the current command.

### 4.3 Epsilon CLI Principle

Epsilon should copy the clarity, not the friction:

```bash
epsilon init support-agent --template slack-triage
cd support-agent
epsilon dev
epsilon deploy
```
...
The same repo runs with `epsilon dev` and deploys with `epsilon deploy`.
...
- Building a new agent runtime from scratch. V1 should wrap the existing OpenCode/Epsilon runtime.
- Replacing `.opencode` immediately. V1 should compile `epsilon.toml` into `.opencode`/`.epsilon` structures where needed.
...
## 7. Core Concept: Epsilon Project / Agent App

A **Epsilon Project** is a git repo. An **Agent App** is the deployable runtime described by that repo.

This distinction matters:

- **Project** = source, development workflow, version control, marketplace starter, team collaboration.
- **Deployment** = built project running on Epsilon Cloud.
...
```text
my-agent/
  epsilon.toml                 # canonical app manifest
...
> The repo defines behavior. `epsilon.toml` describes how to run/deploy it. `.epsilon/state` stores local runtime pointers/cache, not necessarily all durable state. Cloud overlays bind secrets, OAuth accounts, deployed URLs, production resources, databases, object stores, and persistent/forked volumes.
...
Epsilon Cloud should feel serverless from the user perspective, but the primitive underneath is a **real machine-like runtime**.
...
- **Agents** — one or more OpenCode/Epsilon agents with system prompts and permissions.
...
```toml
schema = "https://schemas.epsilon.com/agent-app/v1.json"
...
model = "epsilon-yolo/think"
skills = ["epsilon/web-research", "epsilon/slack"]
...
temperature = 0.2
skills = [
  "epsilon/account-research",
  "epsilon/draft-outreach",
  "./skills/company-crm"
]
...
model = "epsilon-yolo/fast"
skills = ["epsilon/account-research", "epsilon/web-research"]
...
web = "allow"
browser = "allow"
filesystem_write = ["./data", "./outputs", ".epsilon"]
...
Current repo alignment: `core/epsilon-master/src/routes/connectors.ts` already has a SQLite connector registry.
...
```bash
curl -fsSL https://epsilon.com/install | bash
```

Alternative package managers:

```bash
uv tool install -U epsilon
brew install epsilon-ai/tap/epsilon
npm i -g @epsilon/cli
```
...
```bash
epsilon init
epsilon init support-agent --template slack-support
epsilon init --from-prompt "Build an agent that watches Linear bugs and posts a daily engineering brief."
epsilon init --repo https://github.com/epsilon-ai/starters/slack-triage
```

`epsilon init` output should always end with exact next commands:
...
```bash
epsilon dev
epsilon dev --port 3737
epsilon dev --runtime persistent
epsilon dev --no-docker # if local process mode is supported later
```

Responsibilities:

- validate `epsilon.toml`,
- generate/refresh `.opencode` runtime config,
- start the local Epsilon control plane UI,
- show all local/cloud instances the user can access,
- start the selected project instance’s Epsilon Master/OpenCode runtime,
...
Try:
  epsilon run "Send me a test daily brief"
  epsilon triggers fire daily-pipeline-review
```

`epsilon dev` should feel like running a mini Epsilon Cloud locally. The UI should feel like “Cowork”: a workspace where the human can see instances, open the sandbox, talk to agents, inspect triggers/channels/logs, and watch background work.

### 11.4.1 Instances

```bash
epsilon instances list
epsilon instances open daily-ops-agent/dev
epsilon instances create preview --from-branch feature/new-agent
epsilon instances clone production preview
epsilon instances stop preview
epsilon instances destroy preview
```
...
```bash
epsilon run "Research Anthropic and draft outbound."
epsilon run /daily-brief
epsilon run --agent researcher "Find new funding news for target accounts."
epsilon run --json "Return account summary for Ramp."
```
...
epsilon doctor
epsilon test
epsilon test --trigger daily-pipeline-review
epsilon test --channel slack --fixture fixtures/slack-message.json
```

`epsilon validate` checks schema and missing files.  
`epsilon doctor` checks Docker, auth, ports, secrets, connectors, runtime image.  
`epsilon test` runs repo-defined tests/evals/smoke triggers.

### 11.7 Deploy

```bash
epsilon deploy
epsilon deploy --env production
epsilon deploy --dry-run
epsilon deploy --yes
```
...
```bash
epsilon logs --follow
epsilon sessions list
epsilon sessions get <id>
epsilon traces get <run-id>
epsilon status
epsilon open
epsilon shell
epsilon restart
...
```bash
epsilon secrets list
epsilon secrets set HUBSPOT_ACCESS_TOKEN
epsilon secrets pull --env development
epsilon secrets check
```
...
```bash
epsilon integrations list
epsilon integrations add slack
epsilon integrations add hubspot --provider pipedream
epsilon integrations test hubspot
epsilon integrations remove hubspot
```

### 11.11 Channels

```bash
epsilon channels add slack
epsilon channels add telegram
epsilon channels add teams
epsilon channels add whatsapp
epsilon channels test slack
```

### 11.12 Triggers

```bash
epsilon triggers list
epsilon triggers add cron daily-brief "0 9 * * 1-5" --prompt "Run /daily-brief"
epsilon triggers add webhook new-lead --path /hooks/new-lead --prompt "Research this lead"
epsilon triggers fire daily-brief
epsilon triggers logs daily-brief
```

### 11.13 Marketplace

```bash
epsilon marketplace browse
epsilon marketplace add slack-support-agent
epsilon marketplace add epsilon/account-research
epsilon marketplace publish
```
...
- plugins/tools,
- `.epsilon/triggers.yaml`,
- `core/epsilon-master` service manager,
- trigger manager,
- connector registry,
- marketplace routes,
...
Epsilon Cloud should be designed around one core abstraction:
...
- GitHub repo,
- GitLab repo,
- local uploaded tarball from `epsilon deploy`,
...
1. **Framework build** — no custom Dockerfile; use Epsilon base runtime, install declared dependencies, compile manifest into runtime config.
2. **Agent machine build** — custom `Agent.Dockerfile`; user controls packages/binaries while Epsilon injects the runtime shim.
...
- “You deploy a repo.”
- “Epsilon gives it a managed machine.”
...
Epsilon should wrap Pipedream into a simple concept: connector bindings.
...
```bash
epsilon init my-support-agent --template slack-support
```
...
```bash
epsilon marketplace add epsilon/daily-brief-command
```
...
- **Vercel for agents. OpenCode-powered. Epsilon-managed.**

Subheadline:

> Define agents, skills, commands, channels, integrations, triggers, and heartbeats in a local repo. Run with `epsilon dev`. Deploy to managed 24/7 cloud with `epsilon deploy`.
...
```text
Create a Epsilon agent that watches Stripe failed payments, researches the customer in HubSpot, drafts a Slack summary, and posts a daily retry plan every weekday at 9am.
```

Then:

```bash
epsilon init --from-prompt "<paste prompt>"
epsilon dev
epsilon deploy
```
...
curl -fsSL https://epsilon.com/install | bash
epsilon login
epsilon init my-agent --template slack-support
cd my-agent
epsilon dev
epsilon deploy
```
...
- **Local dev, cloud deploy** — `epsilon dev` locally, `epsilon deploy` to cloud.
...
- Secret names can be in `epsilon.toml`.
...
- `epsilon deploy --dry-run` previews resources and cost.
...
### Phase -1 — Reframe ChainLens/Epsilon Around Repo-Backed Projects

Before new cloud polish, align the core data model and product language.

- Rename/reframe “project” to mean a git-backed Epsilon repo, not just an internal board/session container.
...
- Treat `.epsilon` runtime state as local/cloud state attached to a deployment.
...
- A Epsilon project can be represented as `repo_url + branch + commit + manifest_path`.
...
- Local `epsilon dev` and cloud `epsilon deploy` operate on the same repo layout.

### Phase 0 — Productize the Existing CLI Baseline

- Keep self-host install working.
- Add `epsilon --plain`/`--json` conventions where possible.
- Make command groups clearer.
- Add `epsilon doctor` for current local stack.

### Phase 1 — Manifest + Local Framework

- Add `epsilon.toml` schema.
- Add manifest validator.
- Add manifest compiler to `.opencode` + `.epsilon/triggers.yaml`.
- Add `epsilon init` starter scaffolding.
- Add `epsilon dev` local runtime wrapper.
- Add `epsilon run` local prompt runner.
- Add `epsilon validate` and `epsilon doctor`.

P0 acceptance:

- A new repo from `epsilon init` runs locally with `epsilon dev`.
...
- `epsilon logs --follow` streams runtime logs.
- `epsilon run` can target cloud deployment.
...
Epsilon Agent OS V1 is done when:

1. `epsilon init my-agent` creates a git-ready agent repo.
2. `epsilon dev` runs the agent locally with a dashboard and hot reload.
3. `epsilon run "..."` executes against the local runtime.
4. `epsilon deploy` deploys the same repo to managed cloud.
...
1. Should the manifest be `epsilon.toml`, `.epsilon.toml`, or `Epsilonfile`? Recommendation: `epsilon.toml` plus optional `Agent.Dockerfile`.
...
4. What is the default runtime mode? Recommendation: persistent for `epsilon deploy` unless starter marks itself ephemeral.
...
Epsilon should become a simple, sharp workflow:
