...
export const tokenTerminalValuationSnapshots = epsilonSchema.table(
  'token_terminal_valuation_snapshots',
  {
    ...
    peerPercentiles: jsonb('peer_percentiles').notNull().default({}).$type<Record<string, number>>(),
    riskFactors: jsonb('risk_factors').notNull().default([]).$type<Array<{ code: string; label: string; severity: 'low' | 'medium' | 'high' | 'critical' }>>(),
    source: varchar('source', { length: 32 }).notNull().default('token_terminal'),
