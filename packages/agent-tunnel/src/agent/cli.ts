...
async function printStartup(config: { tunnelId: string; apiUrl: string }, capabilities: string[], version: string): Promise<void> {
  ...
  const brand = 'created by epsilon';
  ...
  blank();
  console.log(`  ${c.gray}╰${'─'.repeat(brandFill)} ${c.dim}created by ${c.cyan}epsilon${c.reset} ${c.gray}─╯${c.reset}`);
  console.log('');
}
...
function showHelp(): void {
  ...
  console.log('');
  console.log(`  ${c.bold}Usage${c.reset}   ${c.dim}npx @epsilon/agent-tunnel <command> [options]${c.reset}`);
  console.log('');
  ...
  console.log(`  ${c.dim}powered by ${c.cyan}epsilon${c.reset}`);
  console.log('');
}
